...
\vspace{0.9cm}
{\Large\bfseries\color{uzhgreydark} April~2026\par}
\vspace{1.3cm}
{\large Simon Scheidegger\\[0.5em]
\normalsize HEC, University of Lausanne\par}
\vfill
{\small Course materials: \url{https://github.com/sischei/Deep_Learning_for_Solving_And_Estimating_Dynamic_Economic_Models}\\[4pt]
Cloud platform: \url{https://nuvolos.cloud}\par}
\vspace{0.6cm}
{\footnotesize Last updated: April~2026\\[3pt]
\copyright{} 2026 Simon Scheidegger.  Released under the
...
\item \textbf{Physics-Informed Neural Networks} (PINNs), which approximate Hamilton--Jacobi--Bellman equations and Kolmogorov forward equations in continuous time without computing conditional expectations \citep{sirignano2018dgm, raissi2019physics}.  We develop this machinery for consumption-savings problems and option pricing, and compare it with finite-difference HJB--KFE methods for continuous-time heterogeneous-agent models in the tradition of \citet{achdou2022income}.
\item \textbf{Deep surrogate models and Gaussian processes}, which construct fast, differentiable approximations of expensive structural models, enabling rapid estimation, uncertainty quantification, and global sensitivity analysis \citep{rennerscheidegger_2018, SCHEIDEGGER201968, chen2025Deep}.
\item \textbf{GP-based dynamic programming}, which embeds Gaussian process regression into a value function iteration algorithm \citep{engel2005reinforcement, deisenroth2009gaussian} and, when combined with active subspaces, scales to economies with up to 500 continuous state variables \citep{SCHEIDEGGER201968}.
\end{enumerate}
The unifying theme throughout is that \emph{economic structure} (equilibrium conditions, PDEs, Bellman equations) is embedded directly in the neural network or GP loss function, rather than in labeled training data.  This makes the methods fundamentally different from standard supervised learning.

\paragraph{How these notes are organized.}
The first six chapters build the discrete-time toolkit layer by layer: from the foundations of machine learning and deep learning (Chapter~1), through DEQNs for representative-agent models (Chapter~2), to the international real business cycle model (Chapter~3), neural architecture search and loss normalization (Chapter~4), overlapping generations (Chapter~5), and heterogeneous agents via Young's histogram method \citep{young2010} together with a sequence-space DEQN extension \citep{azinovicyangzemlicka2025sequencespace} (Chapter~6).  Chapters~7--8 introduce continuous-time methods: PINNs for economic PDEs and the coupled HJB--Kolmogorov Forward system for heterogeneous-agent models in the tradition of \citet{achdou2022income}.  Chapters~9--10 cover surrogate-based estimation and Gaussian processes, including active subspaces, Bayesian active learning, deep kernel learning, and GP-based value function iteration.  Chapter~11 applies the DEQN and surrogate toolbox to climate economics and deep uncertainty quantification.  Chapter~12 synthesizes the key ideas and discusses open challenges.
...
\item \textbf{Surrogates and estimation:} read Chapter~1 for optimization and approximation, Chapter~9 for deep surrogates, GPs, active subspaces, and GP-based dynamic programming, and Chapter~10 for the SMM pipeline.  Chapter~2 is the minimum DEQN background for the Brock--Mirman examples used there.
...
The script uses three colored callout boxes plus an algorithm box.  Each color is consistent across the entire manuscript and signals a particular kind of content:
...
\begin{tabular}{@{}c L{3cm} L{4.6cm} L{5.0cm}@{}}
\toprule
\textbf{Ch.} & \textbf{Topic} & \textbf{Slides (\texttt{dayN/}prefix)} & \textbf{Notebooks} \\
\midrule
1 & Intro to ML \& DL & \texttt{day1/01\_Intro\_to\_DL} & \texttt{01}--\texttt{08} (8 notebooks) \\
2 & Deep Equilibrium Nets & \texttt{day2/02\_DEQNs} & \texttt{01}--\texttt{04\_DEQN} \\
3 & IRBC Model & \texttt{day3/03\_IRBC} & \texttt{01\_IRBC}, \texttt{05\_IRBC\_Exercise} \\
4 & NAS \& Loss Norm. & \texttt{day3/04\_NAS}, \texttt{day3/05\_Loss} & \texttt{02\_NAS}, \texttt{03\_NAS}, \texttt{04\_Loss} \\
5 & OLG Models & \texttt{day4/07\_OLG} & \texttt{07}--\texttt{08\_OLG}, \texttt{09\_OLG\_Exercise} \\
6 & HA, Young, Seq.\ Space & \texttt{day4/08\_Youngs} (self-study), \texttt{day4/06\_SeqSpace} & \texttt{05}--\texttt{06\_SeqSpace}, \texttt{10\_Youngs}, \texttt{11\_Continuum}, \texttt{12\_KS\_DL}, \texttt{KrusellSmith\_Tutorial\_CPU} \\
7 & PINNs & \texttt{day6/06\_PINNs} & \texttt{01}--\texttt{05\_PINN}, \texttt{09\_PINN\_Exercise} \\
8 & CT Het.\ Agents & \texttt{day6/07\_CT\_Theory}, \texttt{day6/08\_CT\_Num} & \texttt{06}--\texttt{07\_PE\_HJB}, \texttt{08\_Aiyagari} \\
9 & Surrogates, GPs, DKL & \texttt{day7/07\_Surrogates\_GPs} & \texttt{01\_Surrogate}, \texttt{02\_GP\_and\_BAL}, \texttt{04\_GP\_VFI}, \texttt{05}--\texttt{07\_AS}, \texttt{08\_DKL} \\
10 & Structural Estimation & \texttt{day7/08\_Struct\_Est} & \texttt{03\_Structural\_Estimation\_BM} \\
11 & Climate \& Deep UQ & \texttt{day8/08\_Climate}, \texttt{day8/09\_UQ} & \texttt{01\_Climate\_Economics\_DICE\_DEQN}, \texttt{02\_Climate\_Exercise} \\
12 & Synthesis \& Outlook & \texttt{day8/10\_Wrap\_Up} & -- \\
\bottomrule
\end{tabular}
\end{center}

\noindent{\scriptsize\textbf{Path conventions.} Slide and notebook names are abbreviated in the table.  Full paths follow the repository's session-based directory structure, \texttt{lectures/day$N$/slides/} and \texttt{lectures/day$N$/code/}.  Slide-name prefixes such as \texttt{06}, \texttt{07}, and \texttt{08} are local to each day's deck order; the \texttt{dayN/} qualifier disambiguates them.  This naming mirrors the live course schedule, while the manuscript itself is organized by chapter.  See the README for complete file names and links.}

\medskip
\noindent{\scriptsize\textbf{Workshop material.} The live course includes a hands-on workshop on agentic programming (using AI agents as coding partners).  Because this field is evolving quickly, it is presented through slides and examples rather than as a fixed manuscript chapter.  The manuscript remains organized chapter by chapter, with the workshop material collected in the companion Day~5 slides and exercises.}
...
Deep learning has already demonstrated its potential across a broad range of economic applications.
%
In \emph{macroeconomics}, deep neural networks serve as global approximators of policy and value functions in high-dimensional dynamic equilibrium models, enabling researchers to move far beyond the reach of traditional grid-based methods.  Key contributions include \citet{maliar2021deep} and \citet{azinovicDEEPEQUILIBRIUMNETS2022}; for non-neural-network global solution methods, see \citet{ECTA:ECTA1716} and \citet{eftekhari2025dynare}.
%
In \emph{heterogeneous-agent models}, a central focus of modern macro, deep learning handles the effectively infinite-dimensional state spaces that arise from tracking entire wealth distributions: \citet{azinovicDEEPEQUILIBRIUMNETS2022} extend the nonstochastic simulation method of \citet{young2010} to neural networks, encoding distributions as histograms; \citet{han2023deepham} learn \emph{optimal generalized moments} of the distribution via a permutation-invariant architecture; \citet{fernandezvillaverde2023hjb} study financial frictions and the wealth distribution; and \citet{yang2025structural} combine structural economic models with reinforcement learning to solve heterogeneous-agent economies.
%
\emph{Search and matching} models with aggregate shocks, essential for labor-market analysis, have similarly benefited from deep learning approaches \citep{payne2024deepsam}.
%
In \emph{continuous-time settings}, neural networks approximate Hamilton--Jacobi--Bellman equations without computing conditional expectations, opening new avenues for macro-finance and asset pricing \citep{gopalakrishna2024aliens, duarte2024ml}.  Relatedly, deep learning has been used to solve high-dimensional PDEs directly via backward stochastic differential equations \citep{han2018solving}, though this course focuses on the PINN formulation.
%
\emph{Climate economics} poses particularly severe computational challenges: integrated assessment models couple economic dynamics with climate systems across many state variables.  DEQNs have proven effective at solving such models \citep{kubler2025using}, while careful calibration of the underlying climate emulators is itself a substantive prerequisite \citep{Folini_2021}; for a review of how deep learning methods can be applied to integrated assessment models, see \citet{fernandezvillaverde2025climate}.
%
In \emph{finance}, deep surrogate models dramatically accelerate option pricing and risk management \citep{hutchinson1994nonparametric}; in macro-finance and sovereign default settings, they are equally attractive because the equilibrium mapping is highly nonlinear and expensive to solve \citep{arellano2008default}; and GP-based methods with active subspaces enable the solution of dynamic portfolio choice problems with transaction costs \citep{gaegauf2023portfolio} and private asset allocation \citep{chen2025private}.
%
In \emph{structural estimation}, neural networks make it feasible to estimate rich heterogeneous-agent models that were previously computationally prohibitive \citep{kase2022estimating}, while deep surrogates combined with Gaussian processes enable global uncertainty quantification for complex integrated assessment models \citep{friedlDeep2023}.
%
In \emph{monetary economics}, deep learning has been used to compute optimal policy rules under persistent supply shocks \citep{nuno2024monetary}.
%
It is worth noting that neural networks are not new to economics and econometrics: early applications include nonparametric adaptive learning with feedback \citep{chenwhite1998adaptive}, derivative pricing \citep{hutchinson1994nonparametric}, solving stochastic growth models via parameterized expectations \citep{duffy2001approximating}, and structural estimation of discrete choice models \citep{doi:10.1080/07474938.2011.607089}.  \citet{valaitisvilla2024} recently revisited and modernized this parameterized-expectations approach using neural networks in a macro-finance setting.  In this course, we focus on the recent advances enabled by \emph{deep} neural networks, modern hardware, and algorithmic innovations in training.
...
\begin{itemize}\itemsep2pt
    \item \textbf{Clustering:} partitioning data into groups of similar observations.  \emph{Example:} segmenting firms into peer groups based on financial characteriztics such as size, leverage, and profitability.
...
\[
    \max_{\pi}\; \mathbb{E}\!\left[\sum_{t=0}^{\infty} \gamma^t \, r_t\right], \qquad \gamma \in [0,1).
\]
...
For classification tasks with $K$ classes, one typically uses the cross-entropy loss together with a softmax output layer:
\begin{equation}
J = -\frac{1}{m}\sum_{i=1}^{m}\sum_{k=1}^{K} y_k^{(i)}\log \hat{y}_k^{(i)},
\qquad \hat{y}_k = \frac{e^{z_k}}{\sum_{j} e^{z_j}}.
\end{equation}
Here the output layer is interpreted probabilistically: $\hat{y}_k$ is the model's predicted probability that observation $i$ belongs to class $k$.  If the target is encoded as a one-hot vector, then exactly one component of $(y_1^{(i)},\dots,y_K^{(i)})$ equals one and all others are zero.  For an observation whose true class is $c$, the loss contribution reduces to $-\log \hat{y}_c^{(i)}$.  In other words, the model is rewarded for assigning high probability to the correct class and penalized heavily when it assigns a very small probability to the truth.
...
\end{equation*}
Negating and averaging this expression gives the binary cross-entropy.  The $K$-class formula above is the corresponding negative log-likelihood for a categorical distribution with probabilities generated by the softmax layer.  Cross-entropy is therefore the statistically natural loss whenever the network output is meant to represent class probabilities; see again \citet{bishop2006, deisenroth2020mathematics}.
...
A \emph{deep feedforward network} with $L$ layers computes a nested composition:
\begin{equation}
...
For the DEQN and PINN applications in this course, exponential decay or cosine annealing schedules are commonly used to achieve fine-grained convergence in the later stages of training, after the optimizer has found a good basin of attraction.
...
\paragraph{He initialization.}  For ReLU activations, which zero out roughly half the inputs, \citet{he2015delving} showed that the weight variance should be doubled relative to the forward fan-in rule:
\begin{equation}
W_{ij} \sim \mathcal{N}\!\left(0,\; \frac{2}{n_\mathrm{in}}\right).
\label{eq:he_init}
\end{equation}
This correction is based on the second moment: for a centered symmetric pre-activation $z$, $\E{\mathrm{ReLU}(z)^2}=\tfrac12\E{z^2}$.  The exact variance is slightly smaller because $\E{\mathrm{ReLU}(z)}>0$, but the second-moment calculation is the variance-preserving approximation used in the He rule.  He initialization has become the default for ReLU-family networks and is used in most of the implementations in this course.

\begin{remarkbox}[Practical guidance]
The applications in this course use different activations depending on the task:
\textbf{(i)}~the introductory DEQN examples (Brock--Mirman) use \emph{ReLU} for its simplicity and fast training;
\textbf{(ii)}~the multi-country IRBC model and the deep surrogate use \emph{Swish} ($z\sigma(z)$) for its smooth gradients;
\textbf{(iii)}~all PINN examples use \emph{$\tanh$}, whose $C^\infty$ smoothness is essential when the loss involves second-order derivatives.
...
Leaky ReLU  & $\max(\alpha z, z)$, $\alpha\!=\!0.01$     & $(-\infty,\infty)$            & No dead neurons             \\
...
Among the three mitigations listed above, batch normalization \citep{ioffe2015batch} (BN) deserves a closer look because it is simple to state, surprisingly effective in practice, and has become a default building block in supervised deep networks.  At its core BN is the standardisation trick familiar from regression — re-centring each variable to mean zero and rescaling it to unit variance — applied separately to every layer's pre-activations and recomputed on every mini-batch of training data.
...
\paragraph{Why standardisation, layer by layer.}  Without BN, the input distribution to a hidden layer $\ell$ depends on every weight in layers $1,\dots,\ell-1$.  As earlier weights update during gradient descent, the distribution faced by layer $\ell$ \emph{drifts} from one optimization step to the next: each layer therefore chases a moving target — a phenomenon \citet{ioffe2015batch} called \emph{internal covariate shift}.  BN pins the input distribution of every layer to mean zero and unit variance at every step (Figure~\ref{fig:batchnorm_intuition}).  Gradients become better conditioned, and substantially larger learning rates become safe.
...
\begin{axis}[
    ...
    ylabel={Test error},
    xmin=0, xmax=5, ymin=0, ymax=3.3,
    xtick=\empty, ytick=\empty,
    grid=major, grid style={gray!15},
    axis lines=left,
    clip=false,
]
    % Interpolation threshold — vertical dashed marker + horizontal label above the peak
    \draw[dashed, darkred, thick] (axis cs:1,0) -- (axis cs:1,2.8);
    \node[font=\small\bfseries, darkred, anchor=south]
        at (axis cs:1, 2.85) {interpolation threshold ($p\!\approx\!n$)};

    % Single continuous curve: both pieces share the same spike, so the two
    % baselines match at x=1 (peak = 0.9 + 1.7 = 2.6) and the curve connects
    % without a jump.  Classical regime (bias-variance U) + spike rise.
    \addplot[very thick, uzhblue, domain=0.1:1, samples=200]
        {0.5/(x^0.5) + 0.4*(x^1.5) + 1.7*exp(-(x-1)^2/0.01)};
    % Modern (overparameterized) regime + spike fall (identical spike term).
    \addplot[very thick, uzhblue, domain=1:5, samples=200]
        {0.25 + 0.65*exp(-0.8*(x-1)) + 1.7*exp(-(x-1)^2/0.01)};

    % Region labels
    \node[font=\footnotesize, darkred, align=center]
        at (axis cs:0.55, 2.0) {classical\\regime};
    \node[font=\footnotesize, softgreen, align=center]
        at (axis cs:3.5, 1.2) {modern /\\overparameterized regime};
\end{axis}
\end{tikzpicture}
\caption{Schematic of the double-descent phenomenon.  In the classical regime ($p < n$) test error follows the standard bias--variance U-curve; around the interpolation threshold $p \approx n$ test error can peak sharply because the fitted function is highly sensitive to noise; in the modern overparameterized regime ($p \gg n$) test error decreases again \citep{belkin2019reconciling, nakkiran2020deep}.  In some linearized, kernel, max-margin, or least-norm settings, gradient methods exhibit an implicit bias toward particular low-complexity interpolants; in nonlinear finite-width networks this bias depends on architecture, data, optimizer, initialization, and training protocol.  The curve is illustrative, not a measurement.}
...
The double descent phenomenon can be explored interactively in the companion notebook \texttt{03\_Double\_Descent.ipynb}, which reproduces the key experiments of \citet{nakkiran2020deep}.

\subsection{A Pointer to the Theory: NTK and Benign Overfitting}
...
\item Deep networks compose simple nonlinear coordinate transformations; depth, not width, is what gives them efficient representational power for compositional functions \citep{cybenko1989approximation, hornik1989multilayer}.
...
\paragraph{The volume paradox.}  A complementary perspective on the curse of dimensionality comes from geometry.  The ratio of the volume of the unit $d$-ball to the unit $d$-cube is:
\begin{equation}
\frac{V_\mathrm{ball}(d)}{V_\mathrm{cube}(d)} = \frac{\pi^{d/2}}{2^d\,\Gamma(d/2+1)}.
\end{equation}
This ratio decreases rapidly with $d$: for $d=2$ it is $\pi/4 \approx 0.785$, for $d=10$ it drops to $\approx 0.0025$, and for $d=100$ it is less than $10^{-70}$.  In other words, in high dimensions, almost all the volume of the cube is concentrated in its ``corners,'' far from the center.  Uniformly spaced grids waste nearly all their points in these corners, where the economic model's ergodic distribution typically places negligible probability mass.  Figure~\ref{fig:ergodic_vs_grid} visualizes this mismatch.  The geometric insight motivates the DEQN approach of training on the model's own ergodic distribution rather than on a fixed grid.
...
The loss~\eqref{eq:deqn_loss} treats every equilibrium equation symmetrically, but in practice \citet{azinovicDEEPEQUILIBRIUMNETS2022}~\S4.2.2 split the conditions into two groups.  We illustrate the split with the Brock--Mirman model introduced in the next section and indicate how it generalizes.
...
\end{equation}
Finally, substituting $\lambda_t = 1/C_t$ from~\eqref{eq:bm_foc_c} into this expression gives the \emphc{Euler equation}:
\begin{equation}
\frac{1}{C_t}
...
\end{equation}
The derivation proceeds by guessing that the value function takes the form $V(K,z) = V_0 + B\ln K + D\ln z$ (with the constant $V_0$ written as $V_0$ rather than $A$ to avoid clashing with the TFP / cohort-count uses of $A$ elsewhere in the script) and using the envelope theorem.  Substituting this guess into the Bellman equation $V(K,z) = \max_C \{\ln C + \beta \E{V(K',z')}\}$ with $K' = zK^\alpha - C$ yields a system of equations for $V_0$, $B$, and $D$.  The solution implies the linear savings rule $K_{t+1} = \beta\alpha\, z_t K_t^\alpha$, from which~\eqref{eq:bm_analytical} follows immediately via the resource constraint.  Notably, this closed-form solution holds regardless of the shock persistence $\varrho$, because the $z_{t+1}$ terms cancel in the ratio $z_{t+1}/C_{t+1}$ under the linear consumption rule.

\paragraph{Persistent shocks.}  When $\varrho > 0$, productivity shocks are autocorrelated and the state of the economy becomes the pair $(K_t, z_t)$, since the current shock level $z_t$ now carries information about future productivity.  Under log utility with Cobb--Douglas production and full depreciation, the analytical solution~\eqref{eq:bm_analytical} continues to hold regardless of the persistence $\varrho$ (see above).  However, for more general preferences (e.g., CRRA with $\gamma \neq 1$), partial depreciation ($\delta < 1$), or non-Cobb--Douglas production, the closed-form solution breaks down and numerical methods, and the DEQN approach in particular, become essential: the policy function $C(K_t, z_t)$ must be approximated rather than derived analytically.
...
\end{equation}
for smooth $f$.  Trapezoid and Simpson rules refine the same idea by replacing each rectangle with a trapezoid or a parabolic arc and reach $\mathcal{O}(N^{-2})$ and $\mathcal{O}(N^{-4})$ respectively; Gauss--Hermite, the workhorse of \S\ref{sec:gh_tensor_product}, is the optimal rule when the integrand is multiplied by the Gaussian weight $e^{-x^2}$ and reaches degree-$(2Q-1)$ exactness using only $Q$ nodes.  The same tile-the-area idea generalises to higher dimensions by laying out a Cartesian grid.  With $N$ nodes per coordinate the cost is $N^d$; equivalently, with $M=N^d$ total nodes the midpoint rate becomes $\mathcal{O}(M^{-2/d})$.  This is the explicit form of the curse of dimensionality flagged in Section~\ref{sec:curse_of_dim}, and it motivates both the Stroud monomial rules of \S\ref{sec:monomial_cubature} and the randomised methods of \S\ref{sec:qmc_cdf}.
...
There are three generic ways to compute a derivative of a programmable function $f:\R^n \to \R$ at a point $x_0$: \emphc{symbolic}, \emphc{finite differences}, and \emphc{autodiff}.  Each has a distinct profile.
...
\begin{equation}
\Pi(K_t, K_{t+1}, z_t) \;=\; u\!\left(\,z_t K_t^{\alpha} + (1-\delta)K_t \;-\; K_{t+1}\,\right).
\label{eq:ad_pi}
\end{equation}
This is the only place the model enters.  Change $u$, $Y$, or $\delta$ and nothing else in what follows needs to move.  The Bellman equation of \S\ref{sec:bm} then reads
\begin{equation}
...
\begin{lstlisting}[caption={Autodiff Euler residual for Brock--Mirman.  The function \texttt{Pi} is the only model-specific code; the rest is generic.  A full implementation appears in \texttt{lectures/day4/code/02\_Brock\_Mirman\_AutoDiff\_DEQN.ipynb}.}, label=lst:autodiff_euler]
...
\paragraph{Cross-checking the autodiff loss.}  An important pedagogical point is that the autodiff residual~\eqref{eq:ad_euler} and the hand-derived residual of \S\ref{sec:bm} are the same mathematical object.  The Day 4 notebooks implement both expressions on the same network and report the maximum absolute difference: $4.8\cdot 10^{-7}$ in the deterministic case and $8.1\cdot 10^{-6}$ in the stochastic case with Gauss--Hermite quadrature, both consistent with float32-level arithmetic, graph ordering, and quadrature accumulation.  The trained policy from the autodiff loss matches the analytical closed-form $K_{t+1} = \alpha\beta K_t^{\alpha}$ (respectively $\alpha\beta z_t K_t^{\alpha}$) to mean relative error $\sim 10^{-4}$, confirming that minimizing the autodiff residual recovers the true policy when one is available.
...
\paragraph{Companion notebooks.}  Three notebooks on Day 4 put the above into practice:
\begin{itemize}[itemsep=2pt]
...
\end{align}
Investment is irreversible: $I^j = k^{j\prime} - (1-\delta)k^j \geq 0$.
...
\end{equation}
Under this choice, the consumption-sharing condition~\eqref{eq:irbc_consumption} (derived in the next section) combined with $\lambda_\mathrm{ss} = 1$ and $k^j_\mathrm{ss} = 1$ yields $c^j_\mathrm{ss} = A_\mathrm{tfp} - \delta$ for every country, exactly the per-country consumption implied by the resource constraint $c^j_\mathrm{ss} = Y^j_\mathrm{ss} - I^j_\mathrm{ss} = A_\mathrm{tfp} - \delta$.  The symmetric steady state thus serves as a natural anchor for training: the network's initial predictions need only match this point to avoid infeasible economies during the early simulated trajectories.

\paragraph{Reference calibration.}  Throughout the companion notebook \texttt{01\_IRBC\_DEQN.ipynb}, we use the quarterly calibration summarized in Table~\ref{tab:irbc_params}.  The implied total factor productivity and deterministic steady-state quantities can then be computed analytically.
...
\paragraph{Worked steady state.}  Substituting the reference values into~\eqref{eq:atfp} gives
\begin{align}
...
&\sum_{j=1}^{N} \tau^j \, \frac{(c_t^j)^{1-1/\gamma_j}}{1-1/\gamma_j}
...
\end{equation}
The planner chooses $c_t^j$ and $k_{t+1}^j$ for each country $j$ and each date $t$.  The complementary slackness conditions require $\mu_t^j \geq 0$, $I_t^j \geq 0$, and $\mu_t^j \cdot I_t^j = 0$.
...
\paragraph{Relative error form.}  For numerical purposes, dividing~\eqref{eq:irbc_euler_level} through by its left-hand side gives a scale-free formulation:
\begin{equation}
...
% Gradient arrow
\draw[-{Stealth[length=2mm]}, thick, darkgreen] (axis cs:2.2,0.8) -- (axis cs:2.2,0.08);
\node[font=\small, darkgreen, anchor=south] at (axis cs:2.2,0.85) {$\nabla \mathrm{FB}$};
\end{axis}
\end{tikzpicture}
\caption{Exact zero set of the Fischer--Burmeister function in the $(\mu, I)$-plane.  $\mathrm{FB}_0(\mu, I) = 0$ exactly on the positive $I$-axis (investing, multiplier slack) and the positive $\mu$-axis (constraint binding, $I = 0$).  The open first quadrant is infeasible: it violates complementarity.  The exact map is non-differentiable at the origin; the smoothed version $\mathrm{FB}_\varepsilon$ used in code rounds that corner while preserving the same complementarity intuition.}
...
\end{tikzpicture}
\caption{Reference network architecture used for the $N$-country IRBC model in the companion notebook: two hidden layers of 64 Swish units mapping the $2N$-dimensional state to all $2N+1$ policy outputs via a softplus head.  Softplus keeps capital choices and KKT multipliers positive; irreversibility is still enforced through the Fischer--Burmeister residual unless one parameterizes investment directly.}
\label{fig:irbc_nn_arch}
\end{figure}

The hidden layers use the Swish activation $\mathrm{swish}(x) = x \cdot \sigma(x)$, while the output layer employs the softplus function $\ln(1+e^x)$ to keep the multipliers and capital choice positive.  Two approximation caveats deserve emphasis.  First, $\mathrm{softplus}(x) > 0$ for all $x$, so the multipliers $\mu^j$ are strictly positive rather than exactly zero when the constraint is slack; complementarity is enforced only approximately.  Second, irreversibility requires $I^j = k^{j\prime} - (1-\delta)k^j \geq 0$; a softplus on $k^{j\prime}$ alone does \emph{not} enforce this — the network can output a positive $k^{j\prime}$ that nonetheless implies negative investment.  A cleaner alternative is to output investment directly via $I^j = \mathrm{softplus}(r^j)$ and set $k^{j\prime} = (1-\delta)k^j + I^j$, which hard-enforces the constraint by construction.

The total DEQN loss aggregates all three sets of equilibrium conditions:
\begin{equation}
\ell_\rho = \frac{1}{N_s} \sum_{i=1}^{N_s} \left[
\sum_{j=1}^{N} \bigl(\mathrm{Euler}^j(\bm{s}_i)\bigr)^2
+ \bigl(\mathrm{ARC}(\bm{s}_i)\bigr)^2
+ \sum_{j=1}^{N} \bigl(\mathrm{FB}^j(\bm{s}_i)\bigr)^2
\right],
\label{eq:irbc_loss}
\end{equation}
...
\paragraph{Representative implementation.}  The architecture is a 2-hidden-layer Swish network with a softplus output head of dimension $2N + 1$ (the $N$ capital choices, the resource-constraint multiplier $\lambda$, and the $N$ irreversibility multipliers $\mu^j$); softplus on the multiplier outputs enforces $\mu^j \ge 0$ by construction.  The only non-textbook piece of the loss is the Fischer--Burmeister smoothing of the irreversibility complementarity:
\begin{lstlisting}[caption={Fischer--Burmeister smoothing of $\mu \perp I$ (the only non-trivial line of the IRBC loss).}, label=lst:irbc_fb]
def fischer_burmeister(mu, I, eps=1e-4):
    return mu + I - tf.sqrt(mu**2 + I**2 + eps**2)
\end{lstlisting}
Inside the per-batch cost function, this residual is squared and averaged alongside the Euler-equation residual (whose conditional expectation is handled by the Gauss--Hermite quadrature of \S\ref{sec:gh_tensor_product}) and the aggregate-resource-constraint residual.  The companion notebook \tpath{01_IRBC_DEQN.ipynb} implements the full \tpath{compute_cost} routine as a direct TensorFlow transcription of Equations~\eqref{eq:irbc_adjcost}--\eqref{eq:irbc_arc}; nothing in it is conceptually new beyond the FB line above.

\section{Two-Phase Training}

For large-scale IRBC models, a two-phase training strategy can be used to stabilize convergence:

\begin{definitionbox}[Two-Phase Training Protocol]
\textbf{Phase 1, Burn-in:} Train on i.i.d.\ samples drawn uniformly from an enlarged state space $\mathcal{S}_\mathrm{wide}$.  This phase provides a rough initial approximation and is analogous to ``warming up'' the network so that the simulated economy does not diverge.

\smallskip
\textbf{Phase 2, Ergodic:} Simulate the economy forward using the current policy network, collect state trajectories, and train on these endogenously generated samples.  This phase refines the policy on the economically relevant portion of the state space.
\end{definitionbox}

The rationale for the two-phase approach is that in Phase~2, the simulated economy must remain stable for the training distribution to be well-defined.  If the initial network is too poor, the simulation may diverge (e.g., negative capital or consumption).  Phase~1 provides a sufficiently good starting point to avoid this issue.  Note that this two-phase protocol is not strictly necessary; it is a practical stabilization technique that can help when the model is particularly sensitive to the initial policy approximation.

When used, Phase~1 typically runs for $10{,}000$--$50{,}000$ iterations with a larger learning rate ($\eta \sim 10^{-3}$), while Phase~2 refines the solution over $50{,}000$--$200{,}000$ iterations with a decaying learning rate (e.g., cosine annealing from $10^{-3}$ to $10^{-5}$).  The transition from Phase~1 to Phase~2 is typically triggered when the maximum Euler equation error falls below a threshold (e.g., $10^{-2}$), indicating that the policy network is sufficiently accurate for stable simulation.
...
\item \textbf{Complementarity check:} Confirm that $\mathrm{FB}^j \approx 0$ and that the multiplier $\mu^j$ is positive only when investment is at its lower bound.
...
\begin{enumerate}[itemsep=2pt]
\item \textbf{Held-out residual table.}  Evaluate mean and max absolute residuals on an out-of-sample test set for each equation block (Euler, ARC, FB).  In the two-country benchmark, typical values are mean $\sim 10^{-4}$ and max $\sim 10^{-3}$ for Euler/ARC, with smaller FB residuals.
...
\item \textbf{Constraint diagnostics.}  Verify $I^j \ge 0$ everywhere and that $(\mu^j, I^j)$ lies close to the complementarity axes ($\mu^j \approx 0$ when $I^j > 0$).
...
\item Fischer--Burmeister smoothing converts the non-differentiable irreversibility complementarity $0 \le \mu^j \perp I^j \ge 0$ (with multiplier $\mu^j$ and investment $I^j$, not the resource-constraint shadow price $\lambda$) into a smooth squared residual $\Phi_\varepsilon^2$ that is compatible with SGD.
\item Two-phase training (uniform burn-in $\to$ ergodic refinement) is the practical recipe for stability when the simulated economy is fragile under an untrained policy.
\item Gauss--Hermite tensor-product quadrature (\S\ref{sec:gh_tensor_product}, introduced in the previous chapter) handles expectations in low-dimensional shock spaces; once $N \gtrsim 5$ the linear-scaling Stroud-3 monomial rule (\S\ref{sec:monomial_cubature}) is the workhorse for IRBC, with QMC (\S\ref{sec:qmc_cdf}) and sparse grids reserved for high-accuracy or very high-dimensional cases.
\end{itemize}
\end{keyinsightbox}
...
\addcontentsline{toc}{section}{Exercises}
\begin{enumerate}[itemsep=4pt, leftmargin=*]
\item\label{ex:ch3:1} \textbf{Fischer--Burmeister.}  Show that $\Phi(a,b) = a + b - \sqrt{a^2+b^2} = 0 \iff a\ge 0,\, b\ge 0,\, ab=0$.  Then plot the level set $\Phi(a,b)=0$ and a smoothed variant $\Phi_\varepsilon(a,b) = a + b - \sqrt{a^2+b^2+\varepsilon^2}$ for $\varepsilon=10^{-3}$ in the $(a,b)$ plane.  (The sign convention $a+b-\sqrt{a^2+b^2}$ matches the chapter, the slide deck, and the IRBC notebook; the alternative $\sqrt{a^2+b^2}-a-b$ has the same zero set but the opposite gradient sign.)
\item\label{ex:ch3:2} \textbf{State-space scaling.}  For an IRBC with $N$ symmetric countries, write down the state, policy, and equation dimensions.  At what $N$ does a tensor-product Gauss--Hermite rule with $Q=3$ nodes per dimension exceed $10^4$ evaluations per Euler equation?  At what $N$ does the Stroud-3 monomial rule of \S\ref{sec:monomial_cubature} stay below $100$ nodes?
\item\label{ex:ch3:3} \textbf{Two-phase training.}  Justify why Phase~1 (uniform burn-in) is sometimes essential by constructing a state in which a randomly initialized network produces an infeasible $k_{t+1}^j$ for some country, causing the simulation to diverge in Phase~2.
\item\label{ex:ch3:4} \textbf{Adjustment-cost partials and Tobin's Q.}  Starting from the quadratic adjustment cost~\eqref{eq:irbc_adjcost}, $\Gamma^j = (\kappa/2)\,k^j (k^{j\prime}/k^j - 1)^2$, derive the partial derivatives $\partial\Gamma^j/\partial k^{j\prime}$ and $\partial\Gamma^j/\partial k^j$ shown in~\eqref{eq:irbc_adjcost_derivs}.  Show that both partials vanish at the steady state $k^{j\prime} = k^j$, so the deterministic steady-state allocation is identical to the one of the frictionless model.  Define the \emph{net investment rate} $g^j = k^{j\prime}/k^j - 1$ and re-express the marginal adjustment cost $\partial\Gamma^j/\partial k^{j\prime}$ as $\kappa\,g^j$; this is the Tobin's-Q-style wedge that the planner must balance against the marginal product of capital in the Euler equation.  Discuss qualitatively how raising $\kappa$ changes the speed of convergence to the steady state under a positive productivity shock.
\item\label{ex:ch3:5} \textbf{Complete-markets risk sharing.}  Use the consumption-sharing condition~\eqref{eq:irbc_consumption} to derive the cross-country consumption ratio $c_t^i / c_t^j$ as a function of the Pareto weights $(\tau^i, \tau^j)$, the IES parameters $(\gamma_i, \gamma_j)$, and the shadow price $\lambda_t$.  (i)~Show that with homogeneous preferences ($\gamma_i = \gamma_j = \gamma$), the ratio $c_t^i/c_t^j$ is \emph{constant in time}, reproducing the perfect-risk-sharing result of \citet{backus1992international}.  (ii)~Show that with heterogeneous IES, the ratio fluctuates with $\lambda_t$, but consumption growth rates are still scalar multiples of the same aggregate shadow-price growth when all $\gamma_j>0$.  What does this imply for cross-country consumption-growth correlations in this planner allocation?  (iii)~Explain in two sentences why heterogeneous IES alone does not resolve the empirical consumption-correlation puzzle.
\item\label{ex:ch3:6} \textbf{Notebook --- depreciation sensitivity.}  Open \texttt{05\_IRBC\_Exercise.ipynb}.  Vary $\delta$ between $0.01$ and $0.05$ in five evenly spaced steps; for each value, retrain the network and report (a)~the deterministic steady-state $k_\mathrm{ss}$ implied by the calibration and (b)~the mean and standard deviation of the ergodic capital distribution.  Plot both quantities against $\delta$ and explain qualitatively why the standard deviation of capital rises with $\delta$.
\item\label{ex:ch3:7} \textbf{Notebook --- loss-component weighting.}  In the same notebook, replace the equal $1/n$ component weights of the IRBC loss with inverse-loss weights $\lambda_i^{(t)} \propto 1/\bar{\ell}_i^{(t-1)}$, where $\bar\ell_i^{(t-1)}$ is the moving average of the $i$-th component loss over the previous $100$ training steps.  Compare convergence speed (epochs to reach the residual tolerance) and final residual to the equal-weight baseline.  In which regime do you expect inverse-loss weighting to help most, and when can it hurt (think of a component that is small because it is genuinely satisfied by construction, e.g., a hard-coded resource constraint)?
\end{enumerate}
...
To appreciate the scale of the search problem, consider a typical DEQN setup where we must select: the number of hidden layers ($L \in \{2, 3, 4, 5\}$), neurons per layer ($n \in \{32, 64, 128, 256\}$), activation function (ReLU, Swish, Tanh), learning rate ($\eta \in \{10^{-4}, 5\times 10^{-4}, 10^{-3}, 5\times 10^{-3}\}$), batch size ($B \in \{64, 128, 256, 512\}$), and weight decay ($\lambda \in \{0, 10^{-5}, 10^{-4}, 10^{-3}\}$).  The total number of configurations is $4 \times 4 \times 3 \times 4 \times 4 \times 4 = 3{,}072$.  If each configuration requires 30 minutes to train, exhaustive evaluation would take 64 days on a single GPU.  With additional choices (optimizer type, learning rate schedule, dropout rate), the space easily exceeds $10^4$ configurations.
...
\begin{tikzpicture}[scale=1.0]
    ...
    \end{scope}
\end{tikzpicture}
\caption{Why random search beats grid search when only one hyperparameter matters.  Both panels show 9 evaluations.  Projected onto the ``important'' axis (below each panel), the grid contributes only three distinct values, while the random design contributes nine.  Because typically only 2--3 out of many hyperparameters drive performance (\citealt{bergstra2012random}), random sampling is strictly more informative in the dimensions that matter.}
\label{fig:grid_vs_random}
\end{figure}

\section{Bayesian Optimization}

Bayesian optimization treats the validation loss $\ell(\bm{h})$ as an expensive black-box function of the hyperparameter vector $\bm{h}$, and uses a probabilistic surrogate model, typically a Gaussian process (see Chapter~\ref{ch:gp}) \citep{snoek2012practical, garnett2023bayesian}, to guide the search.  After evaluating $n$ configurations $\{(\bm{h}_i, \ell_i)\}_{i=1}^n$, the GP posterior provides both a prediction $\mu(\bm{h})$ and an uncertainty estimate $\sigma(\bm{h})$ at any untried configuration.

The next configuration to evaluate is selected by maximizing an \emph{acquisition function}.  The most common choice is Expected Improvement (EI):
\begin{equation}
...
\begin{tikzpicture}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Bayesian optimization in one dimension.  Coordinates produced by an actual Gaussian-process fit with an RBF kernel (length scale $\ell = 1.2$, signal variance $\sigma_f^2 = 0.25$); compare with the companion notebook.  \emph{Top:} the GP posterior after five evaluations: the mean $\mu(h)$ (solid blue) \emph{interpolates} all observations, and the $\pm 2$ s.d.\ band collapses to zero at each observation and balloons in the gaps; the unknown true validation loss (dashed red) has a hidden dip near $h = 4.2$ that the five observations fail to reveal.  \emph{Bottom:} the Expected-Improvement acquisition function $\mathrm{EI}(h) = \E[\max(\ell^\star - \ell(h), 0)]$ peaks at $h \approx 3.75$, the largest gap with a promising predicted mean, which is picked as the next evaluation point.  EI balances \emph{exploitation} (low predicted mean) and \emph{exploration} (high predicted variance).}
\label{fig:bayesopt}
\end{figure}
...
\begin{algorithmic}
\small
\STATE \textbf{Input:} Budget $B$, initial candidates $n$, reduction factor $\gamma = 3$
\STATE Allocate each candidate a budget of $r = B/(n \lceil\log_\gamma n\rceil)$
\FOR{round $s = 0, 1, \ldots, \lfloor\log_\gamma n\rfloor$}
    \STATE Train all remaining candidates for $r$ additional resources (epochs)
    \STATE Keep the top $1/\gamma$ fraction; discard the rest
    \STATE $r \leftarrow r \cdot \gamma$
\ENDFOR
\STATE \textbf{Output:} Best surviving configuration
\end{algorithmic}
\end{definitionbox}

For example, with $n = 81$ initial candidates and $\gamma = 3$: round~0 trains all 81 for $r$ epochs and keeps the top 27; round~1 trains these 27 for $3r$ epochs and keeps the top 9; round~2 trains 9 for $9r$ epochs and keeps 3; round~3 trains 3 for $27r$ and selects the winner.  The total budget is $81r + 27\cdot 3r + 9\cdot 9r + 3\cdot 27r = 4 \cdot 81 r = 324\,r$, equivalent to about a dozen full $R = 27r$ trainings rather than the $81$ that naive parallel evaluation would require.  Figure~\ref{fig:hyperband} visualizes this resource-allocation cascade.
...
\end{tikzpicture}
\caption{Successive Halving with 81 initial candidates and reduction factor $\gamma = 3$.  Each round trains the surviving configurations for $\gamma$ times the previous budget, then discards the bottom $(1-1/\gamma)$ fraction.  Total compute is only $\mathcal{O}(B)$ rather than $\mathcal{O}(nB)$ for training every candidate to completion.  Hyperband runs several such brackets in parallel with different $(n,r)$ trade-offs to hedge against unknown early-vs-late performance correlations \citep{li2018hyperband}.}
\label{fig:hyperband}
\end{figure}

Hyperband extends SHA by running multiple SHA brackets with different trade-offs between the number of candidates $n$ and the per-candidate budget $r$, ensuring robustness to the unknown early-stopping behavior of different hyperparameter configurations.
...
\end{equation}
From a multi-objective-optimization standpoint, the vector $(\ell_1, \dots, \ell_K)$ is the object of interest: the equilibrium is defined by all $K$ residuals being zero, and any nonzero weight vector $\bm{w}$ picks a particular scalarisation of the same underlying Pareto problem.  When the components are on comparable scales, uniform weights work; when they are not, the scalarised problem is dominated by a single component and the optimizer effectively ignores the others.  Adaptive weighting methods (discussed below) can be seen as online strategies for traversing the Pareto front rather than committing to a single scalarisation up front.
...
\citet{heydari2019softadapt} proposed a more principled approach based on the \emph{rates of change} of the loss components.  Define $\Delta_k^{(t)} = \ell_k^{(t)} - \ell_k^{(t-1)}$.  The SoftAdapt weights are:
\begin{equation}
w_k^{(t)} = \frac{\exp(\Delta_k^{(t)}/\tau)}{\sum_{j=1}^K \exp(\Delta_j^{(t)}/\tau)},
\end{equation}
where $\tau > 0$ is a temperature parameter.  Components that are decreasing slowly (or increasing) receive higher weight, directing the optimizer's attention to the lagging components.  In practice, SoftAdapt uses smoothed rates (averaged over a window of recent iterations) for stability.
...
\addplot[harvardcrimson, domain=0:1000, samples=100] {0.5*exp(-x/300)};
...
\end{axis}
\end{tikzpicture}
\caption{Illustrative multi-component loss trajectories in a two-country IRBC experiment.  At initialization the three residuals differ by up to two orders of magnitude.  Under uniform weighting, the optimizer drives down $\ell_1$ (blue) fastest because it dominates the gradient, while $\ell_2$ (red), the smaller-scale but equally important country-2 Euler residual, decays roughly five times more slowly.  Adaptive loss balancing (e.g., ReLoBRaLo) re-weights the components so that all three decrease at comparable rates.  Curves shown are schematic exponential fits; see \texttt{04\_Loss\_Normalization.ipynb} for the actual trajectories.}
\label{fig:multi_component_loss}
\end{figure}

Figure~\ref{fig:multi_component_loss} illustrates the typical behavior: without adaptive reweighting, the optimizer focuses almost exclusively on $\ell_1$ (the largest component), allowing $\ell_2$ to stagnate; with adaptive loss balancing (e.g., ReLoBRaLo, GradNorm), all components converge at comparable rates.  Reported convergence-speed improvements vary across schemes and benchmarks; multi-physics PINN benchmarks have shown substantial gains with ReLoBRaLo \citep{bischof2025relobralo}, while gains on DEQN-style Euler-equation losses tend to be smaller and problem-specific (the multi-component scale gap there is usually one to two orders of magnitude rather than the four to six common in PINN systems).  A complementary line uses Neural-Tangent-Kernel diagnostics to choose the weights \citep{wang2022when}, and an older multi-task baseline weights losses by their predictive uncertainty \citep{kendall2018multi}.
...
\addcontentsline{toc}{section}{Exercises}
\begin{enumerate}[itemsep=4pt, leftmargin=*]
\item\label{ex:ch4:1} \textbf{Random vs.\ grid.}  Reproduce the random-vs-grid figure of \citet{bergstra2012random}.  With $9$ evaluations and only one ``important'' axis out of two, suppose the near-optimal region occupies a fraction $p$ of that axis and its location relative to the grid is unknown.  What is the hit probability for a $3\times 3$ grid, and what is the hit probability for random search?
\item\label{ex:ch4:2} \textbf{Bayesian optimization toy problem.}  Implement a Gaussian-process-based BO loop on the 1D function $f(x) = -\sin(3x) - x^2 + 0.7x$ over $x \in [-1,2]$.  How many BO evaluations are typically needed before it matches the best value found by a grid with step size $0.01$?
\item\label{ex:ch4:3} \textbf{Hyperband budget allocation.}  For a Hyperband run with maximum resource $R=81$ and $\eta=3$, list the brackets $(n_i, r_i)$ used and the total resource consumed.  Compare with a naive ``train each candidate to $R$'' strategy at fixed candidate count $n_0 = 27$.
\item\label{ex:ch4:4} \textbf{Loss balancing.}  In a multi-component PINN loss with three terms of magnitude $10^0$, $10^{-2}$, $10^{-4}$, write down what fixed weights $\lambda_i$ would be needed to equalise their gradient contributions.  Why does this become impractical when the gradients are correlated?
\item\label{ex:ch4:5} \textbf{Pareto front geometry.}  Consider the toy two-component loss $\mathcal{L}(\theta;\lambda) = \lambda\,(\theta - a)^2 + (1-\lambda)(\theta - b)^2$ with $\theta \in \mathbb{R}$, $a < b$, and $\lambda \in [0,1]$.  (i)~Solve for the minimizer $\theta^\star(\lambda)$ in closed form.  (ii)~Compute the per-component residuals $\ell_1^\star(\lambda) = (\theta^\star - a)^2$ and $\ell_2^\star(\lambda) = (\theta^\star - b)^2$.  (iii)~Eliminate $\lambda$ to express the Pareto frontier in the $(\ell_1, \ell_2)$ plane and show it is the curve $\sqrt{\ell_1} + \sqrt{\ell_2} = b - a$ for $\ell_1, \ell_2 \ge 0$, hence convex.  (iv)~Sketch the front and identify which point on it corresponds to the equal-weight choice $\lambda = 1/2$.  (v)~In higher-dimensional parameter spaces, explain why nonzero gradient inner products $\langle\nabla \ell_1, \nabla \ell_2\rangle$ make fixed scalar weights fragile.  Contrast ReLoBRaLo's relative-loss progress rule with GradNorm's direct gradient-norm balancing.
\item\label{ex:ch4:6} \textbf{ReLoBRaLo vs.\ GradNorm.}  In notebook \texttt{04\_Loss\_Normalization}, swap the ReLoBRaLo balancer for a GradNorm balancer (\citealt{chen2018gradnorm}; see the chapter for the per-component gradient-norm equalization rule).  Run both on the same multi-scale regression target with components of magnitude $10^0, 10^{-2}, 10^{-4}$.  Report (i)~wall-clock training time per epoch, (ii)~final residual on each component, (iii)~the gradient-balance ratio $\|\nabla\ell_k\|/\sum_j \|\nabla\ell_j\|$ at convergence.  Comment on the cost--benefit trade-off: under what circumstances is the extra per-step cost of GradNorm worth it?
\item\label{ex:ch4:7} \textbf{HPO vs.\ full NAS decision.}  You have access to either (a)~a single consumer GPU (RTX 3060, $\sim 12$ GB) or (b)~one A100 ($80$ GB), for one week of compute.  Your search problem is either (i)~a fixed-topology MLP with unknown depth $\in \{1,\ldots,5\}$, width $\in \{32,\ldots,512\}$, activation $\in \{\mathrm{ReLU}, \mathrm{Swish}, \tanh\}$, learning rate (log-uniform), or (ii)~a flexible network that can be MLP / RNN / shallow Transformer with unknown layer connectivity (graph-level NAS).  For each of the four (hardware $\times$ problem) cells, recommend in three to five sentences whether to use Random Search with Successive Halving, Bayesian Optimization, or full graph-level NAS.  Justify by referencing the per-method overhead and search-space size.
\end{enumerate}
...
In Chapters~\ref{ch:deqn}--\ref{ch:irbc} all agents were infinitely lived.  We now extend the DEQN framework to \emph{overlapping generations} (OLG) models \citep{diamond1965national}, where $A$ finitely-lived cohorts coexist in every period.  OLG models introduce lifecycle savings, intergenerational transfers, age-dependent heterogeneity, and borrowing constraints, phenomena that are central to fiscal policy analysis, pension reform, and demographic modeling.  Computationally, the main new challenge is the kink induced by borrowing constraints, which we handle by combining softplus output activations (for non-negativity) with squared product residuals for the orthogonality conditions in the loss.  We follow the formulation in \citet{Krueger20041411} and \citet{azinovicDEEPEQUILIBRIUMNETS2022}.

\section{Why Overlapping Generations?}
\label{sec:olg_why}

In the Brock--Mirman and IRBC models of Chapters~\ref{ch:deqn}--\ref{ch:irbc}, all agents are infinitely lived.  This assumption simplifies the analysis but rules out several economically important phenomena:
\begin{itemize}[itemsep=2pt]
...
\section{OLG Model Setup}
\label{sec:olg_setup}

Consider an economy with $A$ overlapping cohorts, indexed by age $h \in \{1,\ldots,A\}$.  Time is discrete and infinite.
...
A representative firm operates a Cobb--Douglas technology $Y_t = \eta_t K_t^\alpha L_t^{1-\alpha}$, where $\eta_t$ is a TFP shock and $L_t = \sum_{h=1}^A \ell^h$.  Competitive factor markets imply:
\begin{equation}
...
\section{Mapping OLG to DEQNs}
\label{sec:olg_deqn}

The mapping follows the same ``states $\to$ network $\to$ loss'' structure as Brock--Mirman (Chapter~\ref{ch:deqn}).  We now write each block explicitly for the analytic 6-agent OLG of \S\ref{sec:olg_analytic}; this is exactly what slides II.7--II.9 of \texttt{day4/slides/07\_OLG\_Models\_DEQNs.tex} render in pictures.  The 56-agent benchmark of \S\ref{sec:olg_56} extends the same template with two extra policy blocks (multipliers, bond price) and an additional market-clearing residual; we write that version out in \S\ref{sec:olg_56}.

\paragraph{State $\x_t$ entering the network.}
The \emph{informational} state of the analytic model is the pair
\begin{equation}
\bigl(z_t,\,\bm{k}_t\bigr) \;\in\; \{1,\ldots,4\}\times\R^A
\qquad\text{where}\qquad \bm{k}_t = (k_t^1,\ldots,k_t^A),
\label{eq:olg_state_min}
\end{equation}
i.e.\ the current shock index plus the cross-sectional capital distribution; this is the minimal vector that summarizes the equilibrium and is what slide II.8 displays in the FREE signature.  Inside the notebook we feed the network an \emph{extended state} of dimension $8+4A$ that pre-computes aggregates and per-agent income blocks the network would otherwise have to learn from scratch:
\begin{equation}
\x_t \;=\; \bigl(\,\underbrace{z_t,\eta_t,\delta_t,K_t,L_t,r_t,w_t,Y_t}_{8\text{ aggregate}},\;
\underbrace{k_t^{1:A}}_{A\text{ capital}},\;
\underbrace{\mathrm{fw}_t^{1:A},\,\mathrm{linc}_t^{1:A},\,\mathrm{inc}_t^{1:A}}_{3A\text{ per-agent income}}\bigr) \;\in\; \R^{8+4A},
\label{eq:olg_state}
\end{equation}
with $K_t=\sum_h k_t^h$, $L_t=\sum_h \ell^h$, $(r_t,w_t)$ from~\eqref{eq:olg_prices}, $Y_t$ from the production function, and the per-agent blocks $\mathrm{fw}_t^h = r_t k_t^h$, $\mathrm{linc}_t^h = w_t\,\ell^h$, $\mathrm{inc}_t^h = \mathrm{fw}_t^h + \mathrm{linc}_t^h$.  The map $(z_t,\bm{k}_t)\mapsto\x_t$ is deterministic, so~\eqref{eq:olg_state_min} and~\eqref{eq:olg_state} carry the same information; the augmentation is pure feature engineering and does not change the equilibrium map.

\paragraph{Policies approximated by the network.}
A single multilayer perceptron with softplus output head approximates the equilibrium policy as a function of the state.  (Throughout this OLG chapter we use $\theta$ for the network parameters rather than the $\rho$ of Chapters~\ref{ch:deqn}--\ref{ch:irbc}; both refer to the same object, and the switch follows the convention of the public OLG reference implementation.)
\begin{equation}
\boxed{\;\mathcal{N}_\theta:\;\R^{8+4A} \;\longrightarrow\; \R^{A-1},\qquad
\mathcal{N}_\theta(\x_t) \;=\; \bigl(\hat a^1(\x_t),\ldots,\hat a^{A-1}(\x_t)\bigr)\;}
\label{eq:olg_policy}
\end{equation}
where $\hat a^h(\x_t)$ is the savings of cohort $h$ at state $\x_t$ (slide II.9, output column).  Cohort $A$ saves nothing by terminal boundary, so the network has $A-1$ outputs rather than $A$.  Two by-construction guarantees follow from the softplus head:
\begin{itemize}[itemsep=2pt]
    \item Non-negativity $\hat a^h \ge 0$ holds at every iteration, so the borrowing constraint~\eqref{eq:olg_kkt} is satisfied without an explicit Lagrange multiplier (in this calibration $\lambda^h \equiv 0$ on the ergodic set; see \S\ref{sec:olg_fb} for the multiplier-based variant used in the 56-agent benchmark).
...
Each cohort $h\in\{1,\ldots,A-1\}$ contributes one \emph{relative} Euler-equation residual.  Define the implied current consumption from the budget~\eqref{eq:olg_budget},
\(
\hat c^h(\x_t) := \mathrm{inc}_t^h - \hat a^h(\x_t),
\)
the next-state map $\Phi$ that combines the policy with a fresh shock to produce $\hat\x_{t,+}=\Phi(\x_t,z_{t+1};\theta)$ (more on $\Phi$ in the next paragraph), and the implied next-period consumption $\hat c^{h+1}(\hat\x_{t,+})$ of the agent that just aged from $h$ to $h+1$.  The relative Euler-equation residual is then
\begin{equation}
e_{\mathrm{REE}}^h(\x_t)
\;:=\;
...
\label{eq:olg_loss}
\end{equation}
\emph{(matching slide II.7).}  Two small additive penalties on $\max(-\hat c^h,\,0)^2$ and $\max(-K_{t+1},\,0)^2$ are summed in alongside~\eqref{eq:olg_loss} to keep training numerically robust away from convergence; both penalties are identically zero on the equilibrium ergodic set, so they vanish as $\mathcal{L}\to 0$ and do not bias the solution.\footnote{The 56-agent benchmark of \S\ref{sec:olg_56} adds two genuine extras to~\eqref{eq:olg_loss}: KKT product residuals (because the borrowing and collateral constraints actually bind) and an explicit bond-market-clearing residual (because the network outputs each agent's bond holding independently).  An orthogonal extension is to encode capital-market clearing \emph{exactly} via a dedicated output layer that rescales unnormalised cohort savings so that $\sum_{h=2}^{A} k_{t+1}^{h} = K_{t+1}$ holds by construction; \citet{azinoviczemlicka_2024} adopt this design in an OLG economy with rare disasters.}

\section{Borrowing Constraints and KKT Complementarity}
\label{sec:olg_fb}

The borrowing constraint $k'^h \geq 0$ introduces a complementarity condition via the Karush--Kuhn--Tucker (KKT) system:
\begin{equation}
...
\end{itemize}
This product form is what the public reference implementation accompanying \citet{azinovicDEEPEQUILIBRIUMNETS2022} uses, and what we use in the \emph{56-agent benchmark} of \S\ref{sec:olg_56} (Notebook~\tpath{08_OLG_Benchmark_DEQN.ipynb}); in the 6-agent analytic calibration of \S\ref{sec:olg_analytic} the borrowing constraint is non-binding everywhere on the ergodic set, so $\lambda^h \equiv 0$ and the multipliers (and the KKT residual) are dropped from both the network output and the loss, yielding the simpler $\mathcal{N}_\theta: \mathbb{R}^{8+4A} \to \mathbb{R}^{A-1}$ described in \S\ref{sec:olg_deqn}.  The smoother \emph{Fischer--Burmeister} (FB) reformulation, $\Phi(a,b) = a + b - \sqrt{a^2 + b^2}$, is an alternative used in the IRBC notebook of Chapter~\ref{ch:irbc} for the investment-irreversibility constraint.

\paragraph{When to choose product form vs.\ Fischer--Burmeister.}  The product form $(k'^h \lambda^h)^2$ is simpler, gradient-cheaper, and sufficient whenever the constraint is rarely active on the ergodic set, since the optimizer just needs to verify slackness in expectation.  The Fischer--Burmeister residual $\Phi(a,b)^2$ keeps gradient information \emph{on both sides} of the active set: when the constraint is frequently binding (e.g.\ the IRBC irreversibility constraint on a non-trivial fraction of states), product-form gradients vanish whenever the constraint is locally inactive, which can stall training; FB does not have this pathology.  As a rule of thumb: product form for occasionally-binding KKT, FB for frequently-binding KKT.  In the OLG benchmark of \S\ref{sec:olg_56} the borrowing constraint binds on a thin slice of the ergodic set, so the product form was sufficient; the IRBC application of the previous chapter binds more often and benefits from FB.

\paragraph{The two OLG models we solve, side by side.}
The remainder of this chapter develops two OLG instances that anchor the rest of the manuscript: a 6-agent analytic model used to validate the DEQN against a closed form, and a 56-agent benchmark that mirrors the leading example of \citet{azinovicDEEPEQUILIBRIUMNETS2022}.  Table~\ref{tab:olg_6_vs_56} summarizes the structural and computational gap between them.
...
Constraints                   & None binding in calibration                             & Borrowing $k'^h\!\ge\!0$ and collateral $k'^h\!+\!\kappa b'^h\!\ge\!0$ \\
Adjustment cost               & None                                                    & Quadratic $\tfrac{\zeta}{2}(k'^h\!-\!rk^h)^2$   \\
Network input dim             & 32 (extended; minimal 7)                                & 236 (extended; minimal 113)                     \\
Output dim                    & 5 (savings of cohorts 1--5)                             & 221 ($4(A-1)+1$: policies, multipliers, price)  \\
Loss terms                    & 5 Euler $+$ market clearing by construction             & 221: $4(A-1)$ Euler/KKT $+$ 1 bond clearing     \\
Network                       & $100\!\times\!50\!\times\!5$                            & $128^2$ (classroom) / $1000^2$ (production)     \\
Validation target             & Closed-form $\beta_h$ of \citet{Krueger20041411}        & Mean Euler residual on simulated trajectory     \\
Notebook                      & \tpath{07_OLG_Analytic_DEQN.ipynb}                      & \tpath{08_OLG_Benchmark_DEQN.ipynb}             \\
\bottomrule
\end{tabular}
\caption{The two OLG models solved in this chapter, side by side.  The economic richness of the 56-agent benchmark adds two assets, two binding constraints, persistent shocks, lifecycle labor, and adjustment costs, raising the network input dimension from 32 to 236 and the output dimension from 5 to 221.  The DEQN training loop is structurally identical in both cases.}
\label{tab:olg_6_vs_56}
\end{table}

\section{The 6-Agent Analytic OLG}
...
\item \textbf{Loss.}  Stack 5 Euler-equation residuals into a single mean-square loss; softplus on each savings output enforces $k'^h > 0$ exactly, and capital-market clearing is satisfied by construction (aggregate $K_{t+1}$ is read off as the sum of cohort savings).  Small additive penalties on negative $c$ and $K$ keep training numerically robust early on but vanish at convergence.
\item \textbf{Training distribution.}  Simulate the economy under the current policy for $T_{\text{sim}} = 10\,000$ periods; sample mini-batches of states from this trajectory.
\item \textbf{Optimization.}  Adam with the default $(\beta_1,\beta_2)=(0.9,0.999)$ and learning rate $10^{-5}$ (the value used in the analytic-OLG notebook).  In the 56-agent benchmark of \S\ref{sec:olg_56}, the production schedule is $10^{-5}$ for the first $60{,}000$ episodes followed by $10^{-6}$ for the remaining episodes.
\item \textbf{Diagnostics.}  At convergence, check (i) the average savings rate by cohort against the closed-form $\beta_h$ in Eq.~\eqref{eq:olg_savings_rate} below, (ii) Euler residuals across the simulated ergodic set, and (iii) (in the 56-agent benchmark only) the bond-market-clearing residual.  In the 6-agent analytic model, capital-market clearing is satisfied by construction, since aggregate next-period capital is taken as the sum of cohort savings.
\end{enumerate}
The notebook \tpath{07_OLG_Analytic_DEQN.ipynb} ships with a \texttt{MODE} switch: \texttt{"smoke"} ($\sim$30~s, sanity check), \texttt{"classroom"} ($\sim$5~min, savings close to the closed form but Euler residuals still diagnostic), and \texttt{"production"} (longer run, Table~3-level Euler accuracy).  The 56-agent benchmark of \S\ref{sec:olg_56} (notebook \tpath{08_OLG_Benchmark_DEQN.ipynb}) runs in a few hours on a GPU with the \emph{same} code template.
\end{remarkbox}

\citet{Krueger20041411} proposed a deliberately simple OLG model with a \emph{closed-form solution}, making it an ideal validation benchmark for the DEQN approach.

\paragraph{Calibration.}
The model has $A=6$ agents with log utility ($\gamma = 1$), Cobb--Douglas production ($\alpha = 0.3$), and discount factor $\beta = 0.7$.  Only agent~1 works ($\ell = (1,0,0,0,0,0)$).  Four exogenous shock states combine TFP $\eta \in \{0.95,1.05\}$ and depreciation $\delta \in \{0.5,0.9\}$, with i.i.d.\ transitions ($\pi_{ss'} = 0.25$).
...
\end{equation}
The optimal policy is then $k'^h = \beta_h \cdot \mathrm{inc}^h$: each agent saves a \emph{fixed fraction} of total income, regardless of the shock.  For $A=6$, $\beta=0.7$, Table~\ref{tab:olg6_savings_rates} reports the resulting savings rates.
\begin{table}[ht]
\centering
\small
...
\end{table}
Young agents save more (more periods ahead); old agents save less; Figure~\ref{fig:olg6_savings} plots the same numbers across $h$.
...
The network takes a 32-dimensional input ($8 + 4 \times 6$, the extended state of \S\ref{sec:olg_deqn}) and outputs 5 savings decisions via a $32 \to 100 \to 50 \to 5$ architecture with ReLU hidden layers and softplus output (8,605 parameters).  Training uses the episode-based procedure from Chapter~\ref{ch:deqn}: the current network generates a capital path (episode), equilibrium residuals are computed and used for SGD updates, and a new episode is simulated periodically.  The companion notebook exposes a \texttt{MODE} switch with three calibrated budgets: \texttt{"smoke"} (100 episodes, $\sim$30~s on CPU; loss only reaches $\sim 10^{3}$ -- a sanity check, \emph{not} converged), \texttt{"classroom"} (1\,000 episodes, $\sim$5~min on CPU; savings rates are already close to the closed form, while mean Euler errors remain around $10^{-1}$), and \texttt{"production"} (5\,000 episodes with longer trajectories, several hours on CPU; mean Euler errors $\sim 10^{-3}$, matching Table~3 of \citet{azinovicDEEPEQUILIBRIUMNETS2022}).  In all three modes Adam with $\textrm{lr} = 10^{-5}$ is used; the production run additionally benefits from a learning-rate decay to $10^{-6}$ after the first batch of episodes, mirroring the schedule used for the 56-agent benchmark.
...
The benchmark model of \citet{azinovicDEEPEQUILIBRIUMNETS2022} scales the OLG framework to $A = 56$ agents (ages 25--80) with several realistic features:
...
\item \textbf{Borrowing constraint:} $k'^h \geq 0$ (no short-selling of capital).
...
\begin{equation}
c^h + k'^h + p\cdot b'^h + \Psi^h = r\cdot k^h + \min(\kappa r,1)\cdot b^h + w\cdot e^h.
\end{equation}
...
\),
where $\bm{k}_t = (k_t^1,\ldots,k_t^A)$ and $\bm{b}_t = (b_t^1,\ldots,b_t^A)$ are the cross-sectional capital and bond distributions.  As in the analytic case, the notebook feeds the network an extended state of dimension $1+2A+\text{(aggregates)}$ that pre-computes $K_t$, $L_t$, prices, per-agent income blocks, and the row of next-period transition probabilities $\pi(z_t,\cdot)$ (used by the conditional-expectation block of the loss); concretely $236 = 8 + 4\times 56 + 4$.  This is the analogue of slide III.8.
...
\begin{equation}
\boxed{\;\mathcal{N}_\theta:\;\R^{236} \;\longrightarrow\; \R^{4(A-1)+1},\qquad
\mathcal{N}_\theta(\x_t) \;=\; \bigl(\hat k'^{1:A-1},\;\hat \lambda_b^{1:A-1},\;\hat q^{1:A-1},\;\hat \mu^{1:A-1},\;\hat p\bigr)(\x_t)\;}
\label{eq:olg56_policy}
\end{equation}
where $\hat k'^h$ are capital savings, $\hat\lambda_b^h$ the borrowing-constraint multipliers, $\hat q^h \equiv \hat k'^h + \kappa\,\hat b'^h$ the collateral requirement (from which bond holdings are recovered as $\hat b'^h = (\hat q^h - \hat k'^h)/\kappa$), $\hat\mu^h$ the collateral-constraint multipliers, and $\hat p$ the equilibrium bond price.  Softplus on every output enforces the four non-negativity inequalities $\hat k'^h\ge 0$, $\hat\lambda_b^h\ge 0$, $\hat q^h\ge 0$, $\hat\mu^h\ge 0$ \emph{by construction}, leaving the orthogonality conditions of the KKT systems to be enforced softly in the loss (next paragraph).  The production network uses $1000\times 1000$ hidden units ($\sim\!1.5$M parameters); the classroom version uses $128\times 128$.
...
\begin{equation*}
\begin{aligned}
\mathcal{N}^h_k(\x_t) &:= \beta\,\E{r_{t+1}\,u'(\hat c^{h+1}_{t+1})} + \hat\lambda_b^h + \hat\mu^h, \quad &
...
\end{equation}
On top of these per-agent residuals the bond market must clear, contributing one aggregate residual:
\begin{equation}
e_{\mathrm{MC},b}(\x_t) \;:=\; \sum_{h=1}^{A} \hat b'^h(\x_t) \;=\; \frac{1}{\kappa}\sum_{h=1}^{A-1}\bigl(\hat q^h - \hat k'^h\bigr).
\label{eq:olg56_mc}
\end{equation}
...
\label{eq:olg56_loss}
\end{equation}
\emph{(matching slide III.6).}  With $A=56$ this is $4\times 55 + 1 = 221$ squared residuals per training state.  Comparison with~\eqref{eq:olg_loss}: the analytic case is the special instance of~\eqref{eq:olg56_loss} in which the borrowing constraint never binds (so $\lambda_b^h\equiv 0$), there are no bonds (so all $b$- and collateral-related blocks drop out), and $4(A-1)+1$ collapses to $A-1$.  The two losses are the same template instantiated at different complexity.  Table~\ref{tab:olg56_residual_count} unpacks the residual blocks.
...
Production training uses 60,000 episodes at lr $= 10^{-5}$ followed by 140,000 episodes at lr $= 10^{-6}$, with runtime of several hours on GPU.  The classroom version (2,000 episodes, 128-128 hidden units) runs in $\sim$30 minutes on CPU and is meant to show the mechanics and qualitative lifecycle patterns, not final accuracy.  The loss trajectory typically exhibits oscillations, caused by re-simulation of the capital path at each episode, but the overall trend is steadily downward.

\paragraph{Lifecycle diagnostics.}
The trained model produces economically plausible lifecycle patterns.  Capital savings $k'^h$ follow a hump shape that mirrors the labor income profile: young agents save little (borrowing constraint binds), mid-career agents accumulate rapidly, and older agents decumulate.  Bond holdings $b'^h$ are initially negative (young agents borrow against future income) and increase with age as agents shift from illiquid capital to liquid bonds.  Bond prices vary across shock states, with higher prices in high-TFP states reflecting stronger demand for savings.  In the classroom run the Euler residuals are still large enough to treat the output as diagnostic; in production runs the mean Euler equation errors are of order $10^{-3}$ for both capital and bond equations, corresponding to a $\sim$0.1\% deviation in consumption.  Market clearing residuals are comparably small.

\begin{keyinsightbox}[Scalability of the DEQN framework]
The \emph{same algorithm} that solved the 6-agent model scales to 56 agents with two assets and inequality constraints.  Only the network size and training duration change; the DEQN framework itself is unchanged.  The 56-agent benchmark has a 113-dimensional minimal state and a 236-dimensional engineered network input, both infeasible for traditional grid-based methods.
\end{keyinsightbox}

\begin{keyinsightbox}[Chapter Summary]
\begin{itemize}[itemsep=2pt, leftmargin=*]
\item OLG models discretise the cross-section into a finite number of cohorts ($A$); the minimal state vector contains the aggregate shock and the cohort asset holdings, hence has dimension $\mathcal{O}(A)$.
\item Market clearing $\sum_h k_{t+1}^h = K_{t+1}$ closes the model and is the natural place to encode aggregation \emph{exactly} via a market-clearing output layer (Chapter~\ref{ch:deqn}, footnote on Azinovic-Yang \& \v{Z}emli\v{c}ka 2024).
\item Borrowing and collateral constraints introduce KKT complementarity, which we enforce by combining softplus output activations (for non-negativity) with squared product residuals $(a \cdot b)^2$ in the loss.
\item The 6-agent analytic OLG and the 56-agent IER benchmark cover the spectrum from textbook validation to research-scale scalability with the \emph{same} training loop.
\end{itemize}
\end{keyinsightbox}
...
The OLG models of Chapter~\ref{ch:olg} featured a finite number of agent types, so the cross-sectional state was simply a vector $(k^1,\ldots,k^A)$.  Many important macroeconomic applications, notably the \citet{Aiyagari} model and the \citet{krusell1998income} economy, instead require a \emph{continuum} of agents subject to idiosyncratic shocks, so the entire wealth distribution $\mu_t$ becomes a state variable.  Incomplete markets prevent full insurance, making explicit distribution tracking essential.  Why represent $\mu_t$ as a histogram on a discrete grid rather than as a Monte Carlo panel?  Two reasons motivate the choice up front: a histogram is \emph{deterministic}, so re-running the same equilibrium gives identical aggregates and the loss is a smooth function of the network weights, and it is \emph{noise-free}, so Euler-equation residuals reflect approximation error rather than $\mathcal{O}(1/\sqrt N)$ Monte Carlo sampling noise (Figure~\ref{fig:young_vs_mc} makes this contrast quantitative).  This chapter develops Young's non-stochastic simulation method \citep{young2010} for representing $\mu_t$ as a histogram and shows how to embed that method within the DEQN framework of \citet{azinovicDEEPEQUILIBRIUMNETS2022} to solve heterogeneous-agent economies with neural network policy functions.

\paragraph{How this chapter maps onto the slides and notebooks.}
The Day~4 in-class lecture covers Automatic Differentiation, Sequence-Space DEQNs, and OLG models; this chapter together with the companion deck \texttt{day4/slides/08\_Heterogeneous\_Agents\_Youngs\_Method.tex} is offered as a self-study extension.  Two notebooks accompany Sections~\ref{sec:young_method}--\ref{sec:young_deqn}: \tpath{day4/code/10_Youngs_Method_Examples.ipynb} isolates Young's redistribution operator on toy examples, and \tpath{day4/code/11_Continuum_of_Agents_DEQN.ipynb} embeds the same operator inside the Appendix~A.5 endowment-economy DEQN.  Section~\ref{sec:ks_alternatives} on alternative deep-learning approaches is paired with \tpath{day4/code/12_KrusellSmith_DeepLearning.ipynb}, a classroom-scale all-in-one DL solver in the spirit of \citet{maliar2021deep}.

\paragraph{The Bewley--Huggett--Aiyagari lineage.}  The continuum-agent framework that this chapter operationalises has three foundational sources.  \citet{bewley1986stationary} introduced stationary equilibrium with a continuum of agents subject to uninsurable income shocks and self-insurance through borrowing constraints.  \citet{huggett1993riskfree} cast the same idea as a tractable endowment economy with a single risk-free asset, focusing on the equilibrium interest rate.  \citet{Aiyagari} added neoclassical production, closing the model in general equilibrium with capital accumulation; this is the canonical incomplete-markets economy.  \citet{krusell1998income} layered aggregate productivity shocks on top, producing the modern Krusell--Smith economy that this chapter targets, and its continuous-time reformulation is developed by \citet{achdou2022income} and revisited in Chapter~\ref{ch:ct_theory}.
...
The mathematical challenge is that the \emph{entire distribution} $\mu_t$ over individual states is a state variable.  Since $\mu_t$ is a measure (an infinite-dimensional object), it cannot be placed on a grid without encountering the curse of dimensionality.

\paragraph{Two approaches.}
There are two main strategies for handling heterogeneous agents in discrete time.  Table~\ref{tab:finite_vs_continuum_ha} summarizes the distinction that matters for this chapter: finite-agent methods keep a high-dimensional vector of individual states, while continuum-agent methods replace agent labels by a distributional state.
...
\toprule
& \textbf{Finitely many agents} & \textbf{Continuum of agents} \\
\midrule
State & $(k_1,\ldots,k_N)$, finite-dimensional & $\mu_t(\cdot)$, infinite-dimensional \\
Pro & Exact, no approximation of $\mu_t$ & Law of large numbers eliminates sampling noise; no permutation problem \\
Con & Permutation problem; scales poorly with $N$ & Must approximate $\mu_t$ \\
Examples & OLG (Ch.~\ref{ch:olg}); Maliar et al.\ (2010) & Krusell--Smith (1998); Young (2010) \\
\bottomrule
\end{tabular}
\caption{Two discrete-time representations of household heterogeneity.  A finite-agent representation tracks each labelled agent and must deal with permutation symmetry and growth in $N$; a continuum-agent representation tracks the law $\mu_t$ directly, eliminating sampling noise but requiring an approximation to an infinite-dimensional object.}
\label{tab:finite_vs_continuum_ha}
\end{table}

This chapter pursues Approach~2: tracking the distribution via a histogram, with Young's histogram method \citep{young2010} providing the distribution update.
...
\end{equation}
where $m_1 = \int k\, d\mu$ is mean capital and $a$ is the aggregate shock, achieves $R^2 > 0.999$ in practice.
...
Two cautions are essential.  First, this is a property of the \emph{Krusell--Smith model class} (one risky asset, modest wealth dispersion, moderate income risk) and is not a general theorem; in models with multiple assets, large MPC heterogeneity, or occasionally binding constraints that bite on a non-trivial mass of agents, the forecasting rule typically requires higher moments or a more flexible function approximator (Chapters~\ref{ch:gp},~\ref{ch:climate}).  Second, $R^2 > 0.999$ on simulated mean dynamics does \emph{not} imply that the distribution itself is well summarized by its mean: tails, percentiles, and the borrowing-constrained mass continue to matter for welfare and policy analysis.  Approximate aggregation is therefore best read as a \emph{computational} result, and Sections~\ref{sec:young_method}--\ref{sec:young_deqn} accordingly track the full histogram and use the mean only as one of the network's input features rather than as a state-space substitute.
\end{remarkbox}

\paragraph{Equilibrium.}
A recursive competitive equilibrium consists of:
\begin{enumerate}[itemsep=2pt]
\item Individual optimization: each agent chooses savings $k'$ to maximize utility given the forecasting rule.
\item Market clearing: aggregate demand equals supply.
\item Consistent distribution evolution: $\mu_{t+1} = T(\mu_t, a_t)$ given optimal policies.
...
\medskip\noindent
\textbf{Step 3, Interpolation weights.}  Since the grid spacing is $\Delta k = 1.0$, each off-grid $k'$ is bracketed by $[k_n, k_{n+1}]$ with weight $\omega = 1 - (k' - k_n)/\Delta k$:
\begin{center}\small
\begin{tabular}{l cccc}
\toprule
& $k=1.0$ & $k=2.0$ & $k=3.0$ & $k=4.0$ \\
...
A reader implementing the method should recognize three properties from the figure: (i)~mass is conserved bin-by-bin because $\omega + (1-\omega) = 1$ and $\sum_{\varepsilon'} \pi_{\varepsilon\varepsilon'} = 1$; (ii)~the capital lottery's expected next-period $k$ equals the policy choice $k'$ by Eq.~\eqref{eq:young_meanpreserve}; (iii)~the entire update is \emph{linear} in $G_t$, so it is a sparse matrix-vector product $G_{t+1} = T(\rho)\,G_t$ where the transition operator $T$ depends on the current policy.  This linearity is what makes the histogram update fully differentiable when we embed it inside a neural-network training loop in \S\ref{sec:young_deqn}.
...
Young's method produces \emph{zero sampling noise} (deterministic), preserves the mean \emph{exactly}, requires only $\sim$100--5,000 grid points (versus $>$50,000 agents for Monte Carlo), and is fully reproducible.  The trade-off is that it approximates higher moments and requires a grid that is wide enough to contain all mass.  The following table summarizes the comparison:
\begin{center}
\small
\begin{tabular}{lcc}
\toprule
...
A remarkable empirical finding is that the log-linear forecasting rule~\eqref{eq:ks_forecast} achieves $R^2 > 0.999$ in the standard Krusell--Smith economy: the first moment of the wealth distribution is a nearly sufficient statistic for predicting next-period prices.  Adding higher moments (variance, skewness) to the forecasting rule barely improves the fit.  This ``approximate aggregation'' result does not hold universally; it relies on the specific features of the Krusell--Smith calibration (small aggregate shocks, moderate borrowing constraint), but it is a useful benchmark against which richer models can be compared.
...
\end{itemize}
Both networks use two hidden layers with 500 units (production) or fewer (classroom).
...
\begin{equation}
\mathcal{L} = \frac{1}{N}\sum_{i=1}^{N}\left[\mathrm{EE}_i^2 + \mathrm{BE}_i^2 + n_Z\cdot\mathrm{MC}_i^2 + \mathrm{KKT}_i^2 + \mathrm{CB}_i^2\right],
\label{eq:young_loss}
\end{equation}
where EE is the Euler equation residual, BE is the Bellman equation consistency condition, MC is the bond market clearing condition, KKT is the borrowing complementarity, and CB penalizes negative consumption.  The factor $n_Z$ on market clearing ensures comparable magnitude across terms.

\paragraph{Market clearing via histogram.}
A key advantage of the histogram representation is that market clearing can be computed \emph{exactly} (no Monte Carlo):
\begin{equation}
\mathrm{MC} = \left|\sum_{\eta}\sum_{j=1}^{N_b} h_t(\eta, b_j)\cdot b'(\eta, b_j, x_t^{agg}) - \bar B\right|,
\label{eq:young_mc}
\end{equation}
where $h_t(\eta, b_j)$ is the histogram mass, $b'(\cdot)$ is the policy network output, and $\bar B$ is aggregate net bond supply.  The Appendix~A.5 notebook normalizes $\bar B=1$, which is the source of the ``$-1$'' term sometimes seen in code and slides.

\paragraph{Young's method inside the training loop.}
During each training episode, the histogram is propagated forward using Young's method (Algorithm~\ref{alg:young}) with the current neural network providing the policy function.  This creates a sequence of aggregate states $(x_0^{agg}, x_1^{agg}, \ldots, x_T^{agg})$ on which the equilibrium residuals~\eqref{eq:young_loss} are evaluated.  The distribution update is fully differentiable (linear interpolation), so gradients flow through the histogram propagation during backpropagation.  Crucially, the distribution co-evolves with the network: early in training, when the policy network is inaccurate, the simulated distribution will be ``wrong,'' but market-clearing residuals provide corrective feedback through the loss.  As the network improves, the distribution converges toward the ergodic steady state.

\section{Results and Discussion}
\label{sec:young_results}

In the Appendix~A.5 teaching model, the DEQN with histogram encoding achieves competitive accuracy: Euler equation errors are of order $10^{-3}$, and market-clearing residuals are comparably small.  The broader lesson for the Krusell--Smith benchmark is conceptual rather than model-specific.  Compared to the traditional KS algorithm, the DEQN approach has two advantages: (i) the neural network can condition on the \emph{full} distribution rather than just its mean, providing a richer approximation that can capture situations where higher moments of the distribution matter for prices, and (ii) there is no need for a separate outer loop to update forecasting coefficients, since equilibrium conditions are enforced directly through the loss function.

\begin{remarkbox}[Connection to continuous time]
The discrete-time histogram approach presented here has a natural continuous-time counterpart.  In Chapter~\ref{ch:pinn}, we will see that the Kolmogorov forward equation (KFE; Fokker--Planck) describes the evolution of the wealth \emph{density} in continuous-time heterogeneous-agent models such as \citet{achdou2022income}.  While the mathematical formulation differs (histogram update vs.\ PDE), the economic question is the same: how does the wealth distribution evolve, and how does it feed back into equilibrium prices?
\end{remarkbox}
...
DeepHAM \citep{han2023deepham}
 ...
 & Bellman residual + value-function error
...
Interpretability of distributional state
 ...
 & Medium (moments learned, but low-dim) \\
\addlinespace
...
\addlinespace
Main loss terms
 & Euler + Bellman + market clearing + FB
 & Euler + Bellman + market clearing
 & Bellman + value-function error + market clearing \\
\addlinespace
Reported accuracy (baseline KS)
 & Euler errors $\sim 10^{-3}$--$10^{-4}$
 & Approximation errors $< 1\%$ with $>10^3$ agents
 & Bellman error reduced by $54.2\%$ vs.\ KS with one learned moment \\
\bottomrule
\end{tabular}
\caption{Three deep-learning approaches to heterogeneous-agent equilibria, all applied to the Krusell--Smith benchmark.  They share the same residual-minimization philosophy but differ in how the cross-sectional distribution is encoded as input to the networks.}
...
\end{equation}
where $s = (k_t,\varepsilon_t; \bm{S}_t)$ is the individual-plus-aggregate state and $\bm{S}_t$ is an $N$-agent panel of capital holdings.  The policy network outputs $c_t = \pi_\rho(s)$; the Euler residual is evaluated at simulated states and the expectation by Monte Carlo over next-period idiosyncratic and aggregate shocks.
...
\item \textbf{Borrowing constraint} enforced either architecturally (softplus on savings) or via a Fischer--Burmeister term.
\end{itemize}
...
\citet{maliar2021deep} demonstrate the approach across several increasingly demanding setups: a Krusell--Smith variant with $N \geq 1{,}000$ explicit agents and $2{,}001$ state variables (one per agent plus one aggregate) in the baseline parameterization, scaled up to $N = 10{,}000$ agents in a moments-reduced variant where the cross section is summarized by $10$--$20$ aggregate moments; and on a one-agent consumption-savings problem with kinked policies they report approximation errors of at most a fraction of one percentage point.  A Python/TensorFlow replication is available at \url{https://github.com/marcmaliar/deep-learning-euler-method-krusell-smith} \citep{marcmaliar2022dlks}; it is the basis for the companion notebook \tpath{day4/code/12_KrusellSmith_DeepLearning.ipynb} (described below).
...
\citet{han2023deepham} ask a more ambitious question: can the network \emph{learn} which summary statistics of the cross-sectional distribution are relevant for individual decisions, rather than having the researcher commit to tracking a histogram or the first moment?  They propose replacing the distribution~$\mu_t$ with a small set of \emph{generalized moments}
\begin{equation}
\bm{m}_t \;=\; \bigl(m_t^1, m_t^2, \ldots, m_t^M\bigr) \;=\; \bigl(\textstyle\sum_i g_\theta^1(s_t^i),\; \ldots,\; \sum_i g_\theta^M(s_t^i)\bigr),
\label{eq:deepham_moments}
\end{equation}
where $g_\theta^m : \R^{d} \to \R$ are neural networks trained jointly with the policy and value networks.  Equation~\eqref{eq:deepham_moments} is the canonical DeepSets architecture of permutation-invariant set functions \citep{zaheer2017deep}: sum pooling over the agent population makes $\bm{m}_t$ invariant to permutations of the agents, and the $g_\theta^m$ encoders are flexible enough (by universal approximation on the permutation-invariant functions) to represent any fixed-arity moment.
...
\end{equation}
and all networks $(V_\rho, \pi_\rho, g_\theta)$ are trained jointly to minimize the squared Bellman residual on simulated paths.  Because the generalized moments are parameters of the optimization (not hyperparameters), the method automatically discovers the minimal set of distributional statistics required for equilibrium pricing.

\paragraph{Reported accuracy.}
On the baseline Krusell--Smith model, \citet{han2023deepham} report:
\begin{itemize}[itemsep=2pt]
\item DeepHAM with \emph{only} the first moment in the state vector reduces the Bellman error by \textbf{37.5\%} compared to the classical KS algorithm.
\item DeepHAM with \emph{one} learned generalized moment (on top of, or replacing, the first moment) reduces the error by \textbf{54.2\%}.
...
The accompanying Jupyter notebook \tpath{day4/code/12_KrusellSmith_DeepLearning.ipynb} (introduced in §\ref{sec:young_results} and described in detail in the README) implements a classroom-scale version of the all-in-one DL approach of \citet{maliar2021deep} on the Krusell--Smith benchmark with the parameters of \citet{krusell1998income}.  It uses a single policy network $\pi_\rho(k, \varepsilon, m_1, a)$ parameterized by TensorFlow/Keras, Euler-residual + Bellman-residual + market-clearing loss terms, and an explicit panel of $N$ agents as the distributional input.  The notebook is annotated cell-by-cell with the correspondence to the equations in this chapter, and is designed to converge in under ten minutes on a standard CPU.  For the production-scale counterpart (up to $N = 10^4$ agents, GPU acceleration, richer shock structure), we refer the reader to the replication repository of \citet{marcmaliar2022dlks}.
...
\[
x_t^{\mathrm{seq}} = z_t^T = (z_{t-T+1}, \ldots, z_t), \qquad K_{t+1} = \mathcal{N}_\rho(z_t^T), \qquad C_t = z_t K_t^\alpha - K_{t+1}.
\]
...
\item the \textbf{network output} changes from current consumption $C_t$ to next-period capital $K_{t+1}$ in the warm-up notebook;
...
The shape-preserving idea is concrete enough that we can write it down.  Let $\{k_n\}_{n=0}^N$ be a fixed log-spaced wealth grid and let $B \in \mathbb{R}^{J \times (N+1)}$ be a precomputed I-spline basis evaluated on it,
\[
...
\]
which is a complementarity condition.  The Fischer--Burmeister envelope
\begin{equation}
\mathrm{FB}(g, s) \;=\; \sqrt{g^2 + s^2 + \epsilon_{\text{fb}}} \;-\; g \;-\; s
\label{eq:fb}
\end{equation}
is smooth and satisfies $\mathrm{FB}(g,s) = 0$ if and only if $\min(g, s) = 0$ with both non-negative; the small constant $\epsilon_{\text{fb}}$ (set to $10^{-12}$ in the notebooks) is a numerical stabilizer for the square root.  This sign convention is the one used in the upstream JAX tutorial code; it is the negative of the form $a+b-\sqrt{a^2+b^2}$ adopted in Ch.~\ref{ch:irbc} and Ch.~\ref{ch:olg}, but has the same zero set when squared.  The training loss is the buffer-and-grid average of $\mathrm{FB}^2$,
\[
...
The Krusell--Smith tutorial assembles the encoder, the I-spline policy head, Young's distribution step, and the Fischer--Burmeister loss into a single replay-buffer training loop.  Algorithm~\ref{algo:ks_seqspace} states it explicitly; the parameters in brackets are the defaults used in \tpath{KrusellSmith_Tutorial_CPU.ipynb} (\texttt{full} mode).
...
\medskip
A third notebook, \tpath{KrusellSmith_Tutorial_CPU.ipynb}, is a JAX/optax port of the upstream pedagogical tutorial released by the paper's authors.  It exposes the same shape-preserving I-spline MPC parameterization, the same Young step inside the simulator, and the same Fischer--Burmeister KKT loss as the TensorFlow notebook, but in the original JAX form, with a \texttt{TUTORIAL\_MODE} switch (\texttt{quick}/\texttt{full}) for classroom-friendly run times.  It is adapted from the upstream tutorial \texttt{01\_KrusellSmith\_Tutorial\_CPU.ipynb} in the companion code repository \citep{azinovicyangzemlicka2025sequencespacecode}, available at \url{https://github.com/azinoma/DeepLearningInTheSequenceSpace}; the local adaptation only adds the tutorial-mode switch, an explicit shape-guarantee diagnostic, and additional inline commentary, leaving the algorithm unchanged.

\begin{definitionbox}[Math-to-code glossary for the KS sequence-space tutorial]
\footnotesize
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{@{}L{2.6cm} L{6.2cm} L{4.4cm}@{}}
\toprule
...
\begin{keyinsightbox}[Chapter Summary]
\begin{itemize}[itemsep=2pt, leftmargin=*]
\item Continuum-agent equilibria require explicit distribution tracking; Young's (2010) histogram is a deterministic mass-redistribution scheme on a fixed grid that converges to the ergodic distribution exactly as the grid is refined.
\item Embedding Young's update inside a DEQN training loop yields fully differentiable heterogeneous-agent solutions: gradients flow through the histogram and through the policy network simultaneously.
\item The sequence-space DEQN \citep{azinovicyangzemlicka2025sequencespace} is the natural alternative when the entire path of aggregate variables (rather than the cross-section) is the state of interest.
\item Bewley--Huggett--Aiyagari--Krusell--Smith is the foundational lineage; Achdou et al.\ (2022) supply the continuous-time counterpart taken up in Chapter~\ref{ch:ct_theory}.
\end{itemize}
\end{keyinsightbox}
...
Day 5 introduced agentic programming as a tool for supervising and debugging research code at scale.  Day 6 returns to numerical methods, this time applying neural networks to partial differential equations rather than equilibrium fixed-point conditions.  The PINN loss is structurally analogous to the DEQN loss of Chapter~\ref{ch:deqn}, but the equations are now in continuous time, which lets us solve a single PDE globally over the state space rather than time-step it.

Chapters~\ref{ch:deqn}--\ref{ch:young} operated in discrete time, where the equilibrium conditions take the form of expectational equations (Euler equations).  We now shift to \emph{continuous time}, where the optimality conditions are partial differential equations, Hamilton--Jacobi--Bellman (HJB) equations and Kolmogorov forward equations.  Physics-Informed Neural Networks (PINNs) approximate the solution of these PDEs by minimizing the PDE residual at collocation points, using automatic differentiation to compute the required derivatives \citep{sirignano2018dgm, raissi2019physics}.  This chapter develops the PINN methodology from simple ODEs through boundary condition strategies to the HJB equation for consumption-savings problems, with applications to macro-finance.
...
The resulting framework, known as \emph{Physics-Informed Neural Networks} (PINNs), was introduced by \citet{raissi2019physics}.  In economics and finance, continuous-time models governed by HJB and related PDEs arise naturally in asset pricing under climate uncertainty \citep{barnett2020pricing}, portfolio choice \citep{duarte2024ml}, and heterogeneous-agent economies with aggregate shocks \citep{gopalakrishna2024aliens}.  \citet{duarte2024ml} apply machine-learning methods to continuous-time finance problems, while \citet{gopalakrishna2024aliens} develop the ALIENs framework for solving continuous-time economies with deep learning.  PINNs provide a natural and scalable computational framework for such problems.  Figure~\ref{fig:deqn_pinn_comparison} summarizes the discrete-time DEQN template and its continuous-time PINN analogue.
...
\begin{tikzpicture}[
    ...
    \node[deqnbox] (d2) at (-4.2,1.4) {$\mathcal{N}_\rho(\x_t) \approx p(\x_t)$};
    \node[deqnbox] (d3) at (-4.2,0.2) {$\ell = \|G(\x, \mathcal{N}_\rho)\|^2$};
    \node[deqnbox] (d4) at (-4.2,-1.0) {SGD $\to$ $\rho^\star$};
    \draw[deqnarr] (d1) -- (d2);
    \draw[deqnarr] (d2) -- (d3);
    \draw[deqnarr] (d3) -- (d4);
    ...
    \node[pinnbox] (p2) at (4.2,1.4) {$\mathcal{N}_\rho(\x) \approx u(\x)$};
    \node[pinnbox] (p3) at (4.2,0.2) {$\ell = |\mathcal{D}[\mathcal{N}_\rho]|^2 + \lambda|\mathrm{BC}|^2$};
    \node[pinnbox] (p4) at (4.2,-1.0) {SGD $\to$ $\rho^\star$};
    \draw[pinnarr] (p1) -- (p2);
    \draw[pinnarr] (p2) -- (p3);
    \draw[pinnarr] (p3) -- (p4);
...
To build intuition, consider the simple boundary value problem:
\begin{equation}
y''(x) + y(x) = 0, \quad x \in [0, \pi/2], \quad y(0) = 0, \quad y(\pi/2) = 1.
\end{equation}
The analytical solution is $y(x) = \sin(x)$.  Using a hard BC trial solution:
\begin{equation}
\hat{y}(x) = \underbrace{\frac{2x}{\pi}}_{A(x)} + \underbrace{x\!\left(\frac{\pi}{2} - x\right)}_{B(x)} \cdot \mathcal{N}_\rho(x),
\label{eq:1d_ode_trial}
\end{equation}
which satisfies $\hat{y}(0) = 0$ and $\hat{y}(\pi/2) = 1$ for any network output.  The loss function requires only the PDE residual:
\begin{equation}
\ell = \frac{1}{N}\sum_{i=1}^{N}\big(\hat{y}''(x_i) + \hat{y}(x_i)\big)^2,
\end{equation}
where $\hat{y}''(x_i)$ is computed via two applications of \texttt{torch.autograd.grad}.  Collocation points $x_i$ are drawn uniformly from $[0, \pi/2]$.
...
\end{axis}
\end{tikzpicture}
\caption{PINN solution of the 1D ODE $y''+y=0$ on $[0,\pi/2]$ with hard-BC trial solution~\eqref{eq:1d_ode_trial}. The analytical solution $\sin(x)$ (solid blue) is recovered to plotting accuracy by the converged network (dotted green); the dashed red curve illustrates a typical early-training iterate. Tick marks on the $x$-axis are the uniformly drawn collocation points.}
\label{fig:pinn_1d_ode}
\end{figure}

Figure~\ref{fig:pinn_1d_ode} shows the same calculation graphically.  This simple example illustrates the key ingredients of every PINN: (i)~a neural network that approximates the unknown function, (ii)~automatic differentiation to compute derivatives, (iii)~a loss function built from the PDE residual, and (iv)~collocation points sampled from the domain interior.  The same machinery extends directly to PDEs in two or more dimensions, as we demonstrate in the following applications.
...
\paragraph{Soft enforcement.}  Boundary conditions are penalized as an additional loss term, weighted by $\lambda$.  The difficulty is that $\lambda$ must be tuned: too small and the BCs are violated; too large and the optimizer ignores the PDE interior.
...
\begin{equation}
\hat{y}(x) = A(x) + B(x) \cdot \mathcal{N}_\rho(x),
\label{eq:trial}
\end{equation}
...
\begin{tikzpicture}
    ...
    \begin{axis}[
        ...
        name=plotBN,
        width=4.0cm, height=4.0cm,
        title={\small $B(x)\cdot\mathcal{N}_\rho(x)$},
    ...
    \end{axis}
\end{tikzpicture}
\caption{Hard boundary-condition decomposition for the trial solution $\hat y(x) = A(x) + B(x)\cdot\mathcal{N}_\rho(x)$ with Dirichlet data $\hat y(0)=1$, $\hat y(1)=2$. \emph{Left:} anchor $A(x)=1+x$ matches the boundary values exactly. \emph{Centre:} mask times network, $B(x)\cdot\mathcal{N}_\rho(x)$ with mask $B(x)=x(1-x)$, which vanishes at $x\in\{0,1\}$ regardless of $\mathcal{N}_\rho$. \emph{Right:} their sum $\hat y(x) = A(x) + B(x)\cdot\mathcal{N}_\rho(x)$ is a candidate solution that satisfies both BCs by construction, so the training loss reduces to the interior PDE residual alone.}
\label{fig:trial_solution_decomposition}
\end{figure}

Hard enforcement eliminates the boundary loss term entirely, reducing the number of hyperparameters and improving accuracy near the boundaries.

\paragraph{Transfinite interpolation for 2D domains.}
The 1D hard-enforcement idea ($\hat{y} = A + B \cdot \mathcal{N}_\rho$) extends naturally to rectangular 2D domains, but the anchor function $A$ must now interpolate prescribed boundary data along entire \emph{edges}, not just at two endpoints.  The classical technique for this is \emphc{transfinite interpolation} (also known as Gordon--Coons blending): it constructs a function over the rectangle that exactly matches given boundary curves, in much the same way that bilinear interpolation matches four corner values, except that here we match four continuous edge functions rather than four discrete values.
...
\smallskip\noindent\textbf{Step 3, Add and correct.}  Summing these two would double-count the corner values (e.g., $f_L(c)$ and $f_B(a)$ both contribute at $(a,c)$).  We subtract a bilinear interpolant through the four corner values to compensate.  The result is the \emphc{blending (anchor) function}:
\begin{equation}
\begin{split}
...
\end{split}
\end{equation}
where $f_{BL} = f_B(a) = f_L(c)$, etc., are the four corner values (which must be consistent).  By construction, $A$ matches all four edge functions exactly.

The \emphc{mask function} must vanish on all four edges so that the network can modify the interior without disturbing the boundaries:
\begin{equation}
...
\begin{equation}
\hat{u}(x,y) = A(x,y) + B(x,y) \cdot \mathcal{N}_\rho(x,y).
\end{equation}
On any boundary edge, $B=0$ and $\hat{u}$ reduces to $A$, which matches the prescribed data.  In the interior, $B > 0$ and the network $\mathcal{N}_\rho$ is free to learn whatever shape is needed to satisfy the PDE.  This construction satisfies all four Dirichlet conditions exactly for \emph{any} network output $\mathcal{N}_\rho$.
...
As a pedagogical 2D benchmark, the accompanying notebooks include a Poisson equation:
\begin{equation}
-\nabla^2 u(x,y) = f(x,y), \qquad (x,y) \in [0,1]^2, \qquad u\big|_{\partial\Omega} = 0,
\end{equation}
with manufactured solution $u^\star(x,y) = \sin(\pi x)\sin(\pi y)$, yielding $f = 2\pi^2\sin(\pi x)\sin(\pi y)$.  Using hard boundary conditions via the mask $B(x,y) = x(1-x)y(1-y)$, the PINN achieves pointwise errors below $10^{-4}$ with a modest network and a few thousand collocation points.  This benchmark is the clean 2D extension of the 1D ODE example and a useful bridge before turning to economic applications.
...
\centering
\small
\begin{tabular}{@{} L{3.0cm} L{4.0cm} L{5.5cm} @{}}
\toprule
...
Slow or stalled optimization & Gradient imbalance across loss terms & Normalize loss components and monitor per-term gradients; use adaptive balancing schemes \citep{wang2021understanding, bischof2025relobralo}. \\
...
\end{equation}
where $\rho > 0$ is the subjective discount rate, $\gamma > 0$ ($\gamma \neq 1$) is the coefficient of relative risk aversion (CRRA), and $r$ is the interest rate on wealth.  The budget constraint says that wealth grows at rate $r$ minus consumption; this is the ``cake-eating'' problem because the household is consuming out of a finite asset.
The dynamic-programming perspective is classical in economics; see \citet{StokeyLucasPrescott89} for the standard textbook treatment.
...
\end{equation}
Dividing through by $a^{1-\gamma}/(1-\gamma)$ and solving for $\Lambda$ gives $\Lambda = \kappa^{-\gamma}$ with $\kappa = (\rho - (1-\gamma)r)/\gamma$, so $V^\star(a) = \kappa^{-\gamma}\,a^{1-\gamma}/(1-\gamma)$ and the optimal consumption rule is $c^\star(a) = \kappa\, a$.  This solution is well-defined provided $\kappa > 0$, i.e., $\rho > (1-\gamma)r$, a standard transversality condition ensuring that the agent discounts future utility sufficiently relative to wealth growth.  This closed form is the natural validation target for the PINN in notebook \texttt{04\_Cake\_Eating\_HJB\_PINN}: the trained network should recover $V(a)$ to mean relative error well below $10^{-3}$, and any larger discrepancy signals an under-trained network or a malformed loss.

\paragraph{PINN formulation and PDE residual.}
To solve the HJB equation~\eqref{eq:cake_hjb} with a PINN (rather than using the closed-form above), we proceed as follows.  A neural network $\hat{V}(a) = \mathcal{N}_\rho(a)$ approximates the value function.  Its derivative $\hat{V}'(a)$ is computed by automatic differentiation up to floating-point precision, so no finite differences are needed.  From this derivative, we reconstruct the optimal consumption via the FOC~\eqref{eq:cake_foc}:
\begin{equation}
\hat{c}(a) = \bigl(\hat{V}'(a)\bigr)^{-1/\gamma}.
\end{equation}
Substituting $\hat{V}$, $\hat{V}'$, and $\hat{c}$ into the HJB~\eqref{eq:cake_hjb} and moving everything to one side defines the \emph{PDE residual}:
\begin{equation}
\mathcal{R}(a) = \rho\,\hat{V}(a) - \left[\frac{\hat{c}(a)^{1-\gamma}}{1-\gamma} + \hat{V}'(a)\,(ra - \hat{c}(a))\right].
...
\end{equation}
The DGM (Deep Galerkin Method) architecture of \citet{sirignano2018dgm}, which provides LSTM-style gating and skip connections from the input to every hidden layer, may be used in place of the plain MLP below to improve expressivity for this type of PDE problem; the listing keeps a plain MLP for clarity.  In low dimension the trial-solution MLP of notebook \texttt{04\_Cake\_Eating\_HJB\_PINN} typically outperforms a soft-BC DGM, because the boundary loss no longer competes with the interior residual; DGM becomes worthwhile primarily in higher dimensions or for PDEs with sharp internal features.

\begin{remarkbox}[Residual loss alone is not enough]
A small HJB residual is necessary but not sufficient for an economically meaningful solution.  The same residual minimum can correspond to wildly different value-function levels when the boundary anchor is weak, and to spurious non-monotonic policies when $\hat V'(a) \le 0$ is left unpenalized.  Practical safeguards used in notebook \texttt{04\_Cake\_Eating\_HJB\_PINN}: (i)~hard-BC trial solution that fixes both endpoints exactly, removing the boundary--interior trade-off; (ii)~a softplus-transformed derivative $\widetilde V_a=\operatorname{softplus}(\hat V_a)+\varepsilon$ inside the FOC inversion, eliminating the negative-derivative pathology in training; (iii)~checking the implied consumption policy, the raw derivative $\hat V_a$, and the HJB residual against the closed-form benchmark.  When the residual, boundary conditions, and policy diagnostics agree, the solution is also economically sensible; when only the HJB residual is small, it usually is not.
\end{remarkbox}

\begin{remarkbox}[Optimizer pipeline and double precision]
The PINN notebooks of this chapter use a two-stage pipeline: \emph{Adam} on resampled collocation points to find a basin of attraction, then a \emph{deterministic L-BFGS} polish on a fixed grid in float64.  Two practical points are not cosmetic.  First, switching collocation points each L-BFGS evaluation breaks the strong Wolfe line search; the polish requires a deterministic objective.  Second, second derivatives of a $\tanh$ MLP lose substantial precision in float32, which is exactly the scale L-BFGS probes when comparing successive line-search points.  FP64 is therefore a stability device, not a guarantee of machine-precision residuals: in the classroom runs, the one-dimensional cake HJB reaches final HJB loss about $3\times 10^{-4}$, while the heterogeneous-agent examples still require residual, policy, density, and aggregate diagnostics.  See, for example, \href{https://arxiv.org/abs/2505.10949}{arXiv:2505.10949} for a recent quantitative discussion of PINN precision.
\end{remarkbox}
...
Many frontier models in macroeconomics feature a continuum of agents who face idiosyncratic risk and interact through equilibrium prices.  In Chapter~\ref{ch:young}, we studied the discrete-time version of this problem using Young's histogram method.  In continuous time, the same economic question leads to a coupled PDE system: an HJB equation for individual optimization and a Kolmogorov forward equation (KFE; Fokker--Planck equation) for the cross-sectional distribution.  Following \citet{achdou2022income} for the model setup (their numerical solution uses finite differences), we present a PINN-based solver for the same coupled system.

\subsection{The Huggett/Aiyagari Economy in Continuous Time}
\label{sec:ct_hank_setup}

Consider a continuum of agents with CRRA utility $u(c) = c^{1-\gamma}/(1-\gamma)$, discount rate $\rho$, and two income states $z \in \{z_1, z_2\}$ that switch according to a Poisson process with intensity $\lambda_{zz'}$.  Agents hold a single asset $a \in [a_{\min}, a_{\max}]$ with return $r$ and face a borrowing constraint $a \geq a_{\min}$.  The wage is $w$, so an agent in state $z$ earns flow income $wz + ra$.

\subsection{The HJB Equation}
\label{sec:ct_hjb}

Individual optimality is characterized by the Hamilton--Jacobi--Bellman equation.  For each income state $z$:
\begin{equation}
\rho V(a,z) = \max_c \left\{u(c) + V'(a,z)\big(w z + r a - c\big) + \sum_{z' \neq z}\lambda_{zz'}\big(V(a,z') - V(a,z)\big)\right\},
\label{eq:hjb_hank}
\end{equation}
where $V'(a,z) \equiv \partial V / \partial a$ is the marginal value of wealth (we use the prime notation throughout this chapter and the next, consistent with the continuous-time HA literature; the subscript form $V_a$ is equivalent and used interchangeably in some sources).  The first-order condition for consumption gives:
\begin{equation}
c^\star(a,z) = \big(V'(a,z)\big)^{-1/\gamma}.
\label{eq:ct_foc}
\end{equation}
Substituting back into~\eqref{eq:hjb_hank} eliminates the maximization, yielding a nonlinear PDE in $V(a,z)$.  At the borrowing constraint $a = a_{\min}$, the drift must be non-negative, which imposes a state-constraint boundary condition.

\subsection{The Kolmogorov Forward Equation}
\label{sec:ct_kfp}

The stationary cross-sectional density $g(a,z)$ of agents over wealth and income states satisfies the Kolmogorov forward equation (KFE):
\begin{equation}
0 = -\partial_a\!\left[s(a,z)\,g(a,z)\right]
    + \sum_{z' \neq z}\left[\lambda_{z'z}g(a,z') - \lambda_{zz'}g(a,z)\right],
\label{eq:kfp_hank}
\end{equation}
where $s(a,z) = wz + ra - c^\star(a,z)$ is the savings (drift) function derived from the HJB solution.  Two conditions close the system:

\paragraph{No-flux boundary conditions.}
At the borrowing constraint, no mass can flow below $a_{\min}$:
\begin{equation}
s(a_{\min}, z)\, g(a_{\min}, z) = 0 \quad \text{for each } z.
\label{eq:kfp_noflux}
\end{equation}
Similarly, at the upper boundary $a_{\max}$ (chosen large enough that it does not bind in equilibrium), we impose zero flux.

\paragraph{Mass normalization.}
The total mass of agents across all income states equals one:
\begin{equation}
\sum_{z}\int_{a_{\min}}^{a_{\max}} g(a,z)\,da = 1.
\label{eq:kfp_mass}
\end{equation}

\subsection{General Equilibrium: Market Clearing}
\label{sec:ct_ge}

In partial equilibrium, the interest rate $r$ and wage $w$ are exogenous.  In general equilibrium (Aiyagari-type), they are determined by aggregate capital supply and a production function.  Aggregate capital and effective labor are:
\begin{equation}
K = \sum_z \int_{a_{\min}}^{a_{\max}} a\, g(a,z)\, da,
\qquad
L = \sum_z z\int_{a_{\min}}^{a_{\max}} g(a,z)\, da,
\end{equation}
and with a Cobb--Douglas production function $Y = K^\alpha L^{1-\alpha}$, factor markets clear at:
\begin{equation}
r = \alpha K^{\alpha-1}L^{1-\alpha} - \delta, \qquad w = (1-\alpha)K^\alpha L^{-\alpha}.
\label{eq:ct_prices}
\end{equation}
Finding the equilibrium requires jointly solving the HJB, the KFE, and the market clearing condition.  In the traditional approach this is done via fixed-point iteration over $r$ (or $K$); the PINN approach described below trains all components jointly.
...
\begin{itemize}[itemsep=2pt]
\item $\hat{V}_\rho(a,z)$: approximates the value function.  Its derivative $\hat{V}_a$ is computed via automatic differentiation.
...
\label{eq:ct_loss}
\end{equation}
where $\ell_{\mathrm{HJB}}$ and $\ell_{\mathrm{KFE}}$ are mean squared PDE residuals computed on collocation points, $\ell_{\mathrm{flux}}$ enforces the no-flux boundary conditions~\eqref{eq:kfp_noflux}, and the mass term enforces normalization~\eqref{eq:kfp_mass}.  The integral is evaluated numerically (quadrature or Monte Carlo on the collocation points).

Balancing the four loss components is critical: the HJB and KFE residuals can differ by orders of magnitude, so adaptive loss balancing (ReLoBRaLo, Chapter~\ref{ch:nas}) is strongly recommended.  Practical considerations include using separate learning rate schedules for the two networks, concentrating collocation points near the borrowing constraint where the density can become sharply peaked, and verifying that the consumption policy $c^\star$ from~\eqref{eq:ct_foc} satisfies $c^\star > 0$ everywhere.  If the true stationary distribution contains an atom at the borrowing constraint, a continuous density network alone cannot represent it; one must add a separate boundary-mass variable or use a discretization that permits point mass.

\subsection{Connection to Discrete-Time Methods}
\label{sec:ct_comparison}

The continuous-time PINN approach and the discrete-time DEQN with Young's method (Chapter~\ref{ch:young}) address the \emph{same} economic question, namely how heterogeneous agents interact through prices when markets are incomplete, but differ in the mathematical formulation summarized in Table~\ref{tab:discrete_continuous_ha}.
...
\begin{tabular}{lll}
\toprule
& \textbf{Discrete time (Ch.~\ref{ch:young})} & \textbf{Continuous time (this section)} \\
\midrule
...
Both approaches can incorporate aggregate shocks, multiple assets, and general equilibrium; the choice between them depends primarily on whether the underlying economic model is formulated in discrete or continuous time.
...
The PINN approach approximates $V(S,t) \approx \mathcal{N}_\rho(S,t)$ and computes the partial derivatives $V_t$, $V_S$, and $V_{SS}$ via automatic differentiation.  The total PINN loss for the Black--Scholes problem has four terms:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
The loss has four terms (PDE residual, terminal condition, lower BC, upper BC) and three relative penalty weights $(\lambda_\mathrm{TC}, \lambda_0, \lambda_\infty)$; the PDE residual coefficient is normalized to one.  These weights must be tuned or adaptively balanced via ReLoBRaLo from Chapter~\ref{ch:nas}.  In notebook \texttt{05\_Black\_Scholes\_PINN}, the benchmark parameters are $K=50$, $T=1$, $r=0.05$, $\sigma=0.2$, and $S_\mathrm{max}=100$.  The current classroom run reports a max absolute error around $0.13$ and a mean absolute error around $0.04$ against the analytical Black--Scholes formula; accuracy is therefore a diagnostic to check after training, not a guaranteed property of the architecture.  The choice of $\tanh$ is natural here: it is $C^\infty$ (so $V_{SS}$ is well-defined everywhere), and its bounded range $(-1,1)$ provides a stable starting point for learning the option price surface.
...
\begin{keyinsightbox}[Chapter Summary]
\begin{itemize}[itemsep=2pt, leftmargin=*]
\item PINNs approximate PDE solutions by minimizing the residual at collocation points; automatic differentiation provides the required derivatives algorithmically, up to floating-point precision \citep{raissi2019physics}.
\item Hard vs.\ soft enforcement of boundary conditions is the design lever: trial-function constructions like $\hat y = A(x) + B(x)\mathcal{N}_\rho(x)$ enforce BCs by construction; soft penalties cover the cases where hard enforcement is intractable.
\item For second-order PDEs (HJB, Black--Scholes, Poisson), the activation must be at least $C^2$; ReLU is excluded, $\tanh$ and Swish are the standard choices.
\item Operator learning (DeepONet, FNO) is the natural generalization when one wants to amortise across a parametric family rather than solve one instance.
\end{itemize}
\end{keyinsightbox}
...
\addcontentsline{toc}{section}{Exercises}
\begin{enumerate}[itemsep=4pt, leftmargin=*]
\item\label{ex:ch7:1} \textbf{Trial-function BC enforcement.}  For the BVP $y'' + y = 0$ on $[0,\pi/2]$ with $y(0)=0$, $y(\pi/2)=1$, verify that $\hat y(x) = 2x/\pi + x(\pi/2-x)\,\mathcal{N}_\rho(x)$ satisfies both BCs for any network output.  Why is this preferable to a soft penalty?
\item\label{ex:ch7:2} \textbf{ReLU pathology.}  Explain why a ReLU network is unsuitable for the strong-form Black--Scholes residual.  Then sketch a weak-form formulation that would work with ReLU activations.
\item\label{ex:ch7:3} \textbf{Discrete $\to$ continuous bridge.}  Take the discrete-time Bellman operator
\[
...
\]
Show that as $\Delta t \to 0$ it formally yields the HJB $\rho V = \max_c [u(c) - V'(a)\,c]$.
\item\label{ex:ch7:4} \textbf{BC penalty weight $\lambda$ tuning.}  In notebook \texttt{02\_ODE\_PINN\_SoftVsHardBCs}, run the soft-BC variant with PDE residual $\ell_\mathrm{int}$ and BC residual $\ell_\mathrm{BC}$ combined as $\mathcal{L} = \ell_\mathrm{int} + \lambda\,\ell_\mathrm{BC}$ for $\lambda \in \{10^{-1}, 10^0, 10^1, 10^2, 10^3,10^4\}$.  For each, train to a fixed budget of $5{,}000$ iterations and record (i)~the BC violation $|\hat y(0) - y_0|$ at convergence, (ii)~the maximum interior residual, and (iii)~training wall time.  Plot all three on log--log axes against $\lambda$, identify the elbow at which boundary fit stops improving cheaply, and compare against the hard-BC variant.
\item\label{ex:ch7:5} \textbf{Collocation strategy comparison.}  In notebook \texttt{03\_PDE\_PINN\_Poisson2D} solving $\Delta u = f$ on $[0,1]^2$ with Dirichlet BCs, replace the default uniform-random collocation with (a)~Latin Hypercube sampling, (b)~a Sobol low-discrepancy sequence, (c)~residual-based adaptive sampling that draws each batch of collocation points proportional to the residual magnitude at the current iterate.  Train each variant to the same residual tolerance ($10^{-4}$) and report (i)~the number of collocation points needed, (ii)~the wall time, (iii)~the spatial distribution of final residuals (compute $\sup_x |\Delta u_\theta - f|$ on a fine $200\times 200$ test grid).  Discuss when each strategy is preferred: uniform for smooth problems, low-discrepancy for moderately rough, adaptive for problems with localized features (e.g., near boundary layers).
\item\label{ex:ch7:6} \textbf{Strong vs.\ weak forms.}  Extend the Black--Scholes notebook.  First compare the current strong-form $\tanh$ PINN with a ReLU version and explain why the second derivative term is problematic for ReLU.  Then derive a weak-form variant: multiply the residual by a smooth test function $\varphi$ supported inside $[0,S_\mathrm{max}]$, integrate by parts to move the second derivative onto $\varphi$, and state which derivatives of $V$ remain.  If you implement the weak-form ReLU variant, compare its held-out pricing error with the strong-form $\tanh$ baseline.  Connect the result to Exercise~\ref{ex:ch7:2}.
\item\label{ex:ch7:7} \textbf{Operator learning vs.\ PINN.}  For an option pricing problem where the strike $K$ varies across $\{45,46,\ldots,55\}$, count the training cost of (a)~eleven independent PINN runs vs.\ (b)~one operator-learning or parametric-PINN run that takes $K$ as an input; see Section~\ref{sec:operator_learning_bridge} and Chapter~\ref{ch:outlook}.  At what cost ratio does amortized training win?
\end{enumerate}
...
Building on the PINN foundations of Chapter~\ref{ch:pinn}, this chapter develops the full continuous-time heterogeneous-agent framework: the coupled system of the Hamilton--Jacobi--Bellman equation (for individual optimization) and the Kolmogorov forward equation (for the stationary wealth distribution), closed by a market-clearing condition.  This is the continuous-time analogue of the discrete-time Krusell--Smith economy of Chapter~\ref{ch:young}.  The primary reference is \citet{achdou2022income}; for pedagogical background on the continuous-time methods, see also Moll's lecture notes.\footnote{\url{https://benjaminmoll.com/lectures/}}
...
\paragraph{Companion notebooks.}  The chapter's three companion notebooks build up to general equilibrium incrementally.  \tpath{06_PE_Discrete_HJB_PINN.ipynb} solves the partial-equilibrium HJB with two discrete income states (Poisson regime switches) at fixed prices.  \tpath{07_PE_Diffusion_HJB_PINN.ipynb} replaces the discrete shock with a diffusion process, exposing the second-order term that distinguishes continuous-time HA from its discrete cousin.  \tpath{08_Aiyagari_Continuous_Time_FD_and_PINN_PyTorch.ipynb} closes the loop with a full GE solver, comparing a finite-difference upwind benchmark against a PINN trained on HJB and KFE residuals jointly.
...
\begin{itemize}[itemsep=2pt]
\item \textbf{Analytical tractability.}  It\^o calculus provides clean first-order conditions, and the separation between the backward HJB and the forward KFE is sharper than in discrete time.
\item \textbf{No expectations operator.}  Conditional expectations are replaced by differential operators, avoiding numerical integration over shock distributions.
...
\end{axis}
\end{tikzpicture}
\caption{Three simulated standard Brownian sample paths $\{B_t\}_{t\in[0,1]}$, generated with discretization step $\Delta t = 0.05$ and Gaussian increments $B_{t+\Delta t} = B_t + \sqrt{\Delta t}\,\varepsilon_t$, $\varepsilon_t \sim \mathcal{N}(0,1)$. Paths are jagged at the chosen $\Delta t$; limiting Brownian paths are continuous almost surely but nowhere differentiable, with $\mathrm{Var}(B_t) = t$.}
...
The derivation proceeds in four steps.  (i) For any smooth test function $\varphi(x)$, $\E{\varphi(X_t)} = \int \varphi(x)\,g(x,t)\,dx$.  (ii) Differentiate with respect to $t$ and apply It\^o's lemma: $\tfrac{d}{dt}\int \varphi\,g\,dx = \int [\mu\,\varphi' + \tfrac{\sigma^2}{2}\varphi'']\,g\,dx$.  (iii) Integrate by parts (assuming $g \to 0$ at $\pm\infty$): $\int \mu\,\varphi'\,g\,dx = -\int \varphi\,\mu\,\partial_x g\,dx$ and $\int \tfrac{\sigma^2}{2}\varphi''\,g\,dx = \int \varphi\,\tfrac{\sigma^2}{2}\partial_{xx}g\,dx$.  (iv) Since the identity holds for all test functions $\varphi$, we obtain:
...
\end{equation}
where $J = \mu g - \tfrac{1}{2}\partial_x(\sigma^2 g)$ is the probability flux.  The identity $\partial_t g + \partial_x J = 0$ is a conservation law: probability is neither created nor destroyed.
...
\paragraph{Ornstein--Uhlenbeck process.}  For $dX_t = \eta(\bar{x} - X_t)\,dt + \sigma\,dB_t$, the KFE is $\partial_t g = \eta\,\partial_x[(x - \bar{x})\,g] + \tfrac{\sigma^2}{2}\partial_{xx}g$.  Setting $\partial_t g = 0$ yields the stationary distribution $g^*(x) = \mathcal{N}(\bar{x}, \sigma^2/(2\eta))$: mean reversion concentrates mass around $\bar{x}$, while diffusion spreads it, and the balance produces a Gaussian steady state.
...
Individual optimality in continuous-time heterogeneous-agent models is characterized by the HJB equation.  This section gives the full five-step derivation from Itô's lemma; readers who have already worked through the application-driven HJB statement in \S\ref{sec:ct_hank} can treat the present section as the formal counterpart that justifies the equation used there.
...
\label{eq:hjb_agent}
\end{equation}
subject to $da_t^i = (wn_t^i + ra_t^i - c_t^i)\,dt$ and $a_t^i \geq \underline{a}$, where $u(c) = c^{1-\gamma}/(1-\gamma)$ is CRRA utility, $\rho > 0$ is the discount rate, and $(r,w)$ are factor prices.
...
\paragraph{Deriving the HJB.}  The derivation follows five steps.  (i) \emph{Dynamic programming principle}: for small $h > 0$, $V(a,n) = \max_c\{\int_0^h e^{-\rho t}u(c_t)\,dt + e^{-\rho h}\E{V(a_h, n_h)}\}$.  (ii) Apply It\^o's lemma to $V(a_t, n_t)$ between income jumps: $dV = V'(a,n)\cdot s(c,a,n)\,dt$ (no diffusion term since $da = s\,dt$ is deterministic between jumps).  (iii) Take expectations accounting for Poisson jumps: $\E{dV} = [V'(a,n)\cdot s + \lambda(n)(V(a,\hat{n}) - V(a,n))]\,dt$.  (iv) Substitute into the DPP, divide by $h$, and let $h \to 0$.  (v) Impose optimality over $c$:
\begin{equation}
...
The solution method for the stationary equilibrium (no aggregate shocks) iterates: guess $r$ $\to$ solve HJB for $V, c^*$ $\to$ solve KFE for $g^*$ $\to$ compute $K = \sum_n \int a\,g^*\,da$ $\to$ update $r$ from the firm FOC.
...
\label{eq:master_eq}
\end{equation}
where $\mu^g_j$ is the KFE drift computed from the optimal savings policy, and the last line encodes how changes in the distribution affect the value function through prices.
...
\WHILE{$\mathcal{E} > \varepsilon$}
    ...
    \STATE Compute master equation residual $\hat{\mathcal{L}}(\cdot;\Theta)$ via automatic differentiation
    ...
    \STATE Update: $\Theta \leftarrow \Theta - \eta\,\nabla_\Theta \mathcal{E}$ \;\textit{($\eta$: learning rate)}
\ENDWHILE
\end{algorithmic}
\end{algorithm}
...
% ============================================================================
\chapter{Deep Surrogate Models and Gaussian Processes}
\label{ch:gp}

The DEQNs and PINNs of previous chapters solve a single model configuration at a time.  Day~6 solved one such configuration to high accuracy; Day~7 instead solves a \emph{family} of configurations cheaply.  Surrogates produce thousands of evaluations per second, enabling parametric estimation, sensitivity analysis, and uncertainty quantification that would be infeasible if every evaluation required a full numerical solve.  In many applications (structural estimation, uncertainty quantification, optimal policy design) one needs to evaluate the model at thousands or millions of parameter configurations.  This chapter develops two complementary \emph{surrogate} strategies that replace the expensive model with a cheap-to-evaluate approximation: deep neural network surrogates for high-dimensional settings \citep{chen2025Deep}, and Gaussian process surrogates for moderate-dimensional settings where data efficiency matters.  The methodological foundations are the GP textbook of \citet{Rasmussen:2005:GPM:1162254} and the Bayesian-active-learning ideas dating back to \citet{mackay1992information} and \citet{krause2008near}.  Embedding GPs inside dynamic programming was pioneered by \citet{deisenroth2009gaussian} (GPDP) and \citet{engel2005reinforcement} (GP temporal-difference learning); within economics, applications include high-dimensional growth models \citep{SCHEIDEGGER201968}, dynamic incentive problems \citep{rennerscheidegger_2018}, and deep uncertainty quantification for integrated assessment models \citep{friedlDeep2023}.  We also cover Bayesian active learning for optimal training-point selection, sparse approximations \citep{titsias2009variational, hensman2013gaussian} that extend GPs beyond their cubic scaling limit, active subspaces for dimensionality reduction, GP-based value function iteration for dynamic programming, and deep kernel learning for combining neural network feature extraction with GP inference.

\section{Motivation: The Computational Bottleneck}

Many tasks in quantitative economics and finance, such as structural estimation, uncertainty quantification, and optimal policy design, require evaluating a structural model at thousands or millions of parameter configurations.  When each model evaluation involves solving a Bellman equation, a PDE, or running a Monte Carlo simulation, the total computational cost can be prohibitive.
...
\item \textbf{Deep neural network (DNN) surrogates} are best suited for \emph{high-dimensional} settings ($d \gg 10$ inputs) where training data can be generated in large quantities, for example when each model solve takes seconds or when closed-form solutions exist.  DNNs scale gracefully with dimensionality, can be trained via mini-batch SGD on millions of samples, and provide exact gradients via automatic differentiation.  \citet{chen2025Deep} formalize this approach and demonstrate speedups of several orders of magnitude for option pricing (the same surrogate-for-finance idea was implemented earlier with adaptive sparse grids by \citet{scheideggertreccani_2018}); \citet{friedlDeep2023} apply it to uncertainty quantification in high-dimensional integrated assessment models.
...
Data efficiency & needs $10^4$--$10^6$ points & effective with $10^1$--$10^3$ points \\
...
\paragraph{Speed gains.}  Regardless of whether a DNN or GP is used, once the surrogate is trained (a one-time cost), downstream tasks such as estimation, sensitivity analysis, and optimal policy design become computationally trivial.  \citet{chen2025Deep} report speedups of several orders of magnitude for option pricing, where evaluating the DNN surrogate replaces expensive FFT-based Fourier inversion (their Bates-model benchmark documents two-to-three orders of magnitude over the numerical pricing baseline).  As a rough rule of thumb, the gain scales with the cost of the underlying pricing routine: the orders-of-magnitude gains arise for models requiring a PDE solve (roughly $1$\,ms/eval $\to$ $1$\,$\mu$s/eval through a surrogate) or high-dimensional Monte Carlo, regimes in which $10^3$--$10^4\times$ speedups are typical.  The gains are even larger for gradient computations: while finite-difference gradients require $d+1$ model evaluations (one per parameter), the gradient through the surrogate (autograd for DNNs, closed-form for GPs) requires only a single pass, regardless of the number of parameters.

\paragraph{Historical lineage: from adaptive sparse grids to learned surrogates.}  Before neural-network surrogates, the same surrogate philosophy already drove an earlier econ-finance literature based on \emph{adaptive sparse grids}: replace an object that depends on a state vector or parameter vector by a fast functional approximant that can be evaluated repeatedly without re-solving the underlying problem.  \citet{ECTA:ECTA1716} construct adaptive sparse-grid approximants of the equilibrium policy as a function of the high-dimensional state, eliminating the curse of dimensionality of tensor-product grids.  \citet{scheideggertreccani_2018} carry the same idea into derivative pricing: they build adaptive sparse-grid \emph{surrogates of the conditional-expectation function} (the expected value as a function of the underlying state) and call this a \emph{precomputation}, so that one offline construction replaces repeated online evaluation in the recursive American-option pricing step.  The point worth holding onto is that in both cases the surrogate object \emph{is a function of the state or parameter vector}: the GP and DEQN approaches developed in this script inherit exactly the same blueprint, and only swap grid-based hierarchical refinement for sample-based learning.

\paragraph{Economics and finance applications.}  In economics, GPs and active learning are especially useful when each model solve is expensive and the geometry is irregular: value-function iteration with GPs \citep{SCHEIDEGGER201968}, dynamic incentive problems on irregular state spaces \citep{rennerscheidegger_2018}, and surrogate-based structural estimation (Chapter~\ref{ch:estimation}).  For uncertainty quantification and uncertainty propagation with GP surrogates, the general methodological reference is \citet{Bilionis:2016wc}; for deep-network surrogates with active subspaces, see \citet{tripathy2018deep}; for integrated assessment models specifically, the relevant deep-UQ reference is \citet{friedlDeep2023}.  For global sensitivity analysis of economic models via polynomial chaos expansion and Sobol' indices, see \citet{harenberg2019uncertainty}.  In finance, the same logic applies to derivative pricing and calibration \citep{hutchinson1994nonparametric}; in sovereign default and incomplete-markets models, surrogate-based estimation is attractive because the equilibrium mapping is highly nonlinear \citep{arellano2008default}.

\section{Pseudo-States: Parameters as Network Inputs}
...
\citet{scheideggertreccani_2018} achieve the surrogate-for-finance idea with adaptive sparse grids; \citet{friedlDeep2023} apply the surrogate idea to uncertainty quantification in integrated assessment models of climate change; \citet{chen2025Deep} demonstrate speedups of several orders of magnitude for option pricing with the deep-surrogate approach.
...
The squared exponential (RBF) kernel is the most widely used choice; we briefly preview the practical recommendation here so the reader can keep it in mind through the derivations: in economic applications, the target function (value, policy, equilibrium price) often has \emph{kinks} from occasionally-binding constraints, and the Mat\'ern-$3/2$ kernel introduced in \S\ref{sec:matern} below is then a better default than RBF, since it does not oversmooth at non-differentiable features.  The RBF kernel remains the right default when the target is genuinely smooth (e.g.\ in the unconstrained interior of an ergodic set).
\begin{equation}
...
\]
since $x_\star = \pi/2$ is equidistant from $0$ and $\pi$.  Because $\bm y = (0,0)^\top$, the posterior mean~\eqref{eq:gp_mean} is exactly $\bar f_\star = 0$.  For the variance,
\[
...
\begin{equation}
\log p(\bm{y} | \X, \bm{\vartheta}) = -\frac{1}{2}\bm{y}^\top K_y^{-1}\bm{y} - \frac{1}{2}\log|K_y| - \frac{n}{2}\log 2\pi,
\end{equation}
...
\addplot[mark=*, mark size=2pt, only marks, softgreen]
       coordinates {(0.21,-3.34)};
\node[font=\scriptsize, anchor=south, softgreen]
       at (axis cs:0.21, -3.34) {optimum};
...
When model evaluations are expensive, we wish to select training points that provide maximal information.  Bayesian Active Learning (BAL) uses the GP posterior variance to guide this selection.  The information-theoretic foundations go back to \citet{mackay1992information}, with submodular guarantees for variance-based sensor placement established by \citet{krause2008near} and the now-standard upper-confidence-bound formulation by \citet{srinivas2010gaussian}.

\paragraph{Connection to Bayesian optimization (Day~3).}  BAL is the active-learning twin of the Bayesian-optimization (BO) recipe introduced in Chapter~\ref{ch:nas}: same GP surrogate, same acquisition-function machinery.  The difference is the target.  BO seeks a \emph{scalar optimum} of the surrogate (e.g.\ Expected Improvement steers samples toward $\argmax \hat f$), so its acquisition trades exploration against the chance of beating the current best.  BAL instead targets \emph{global function approximation}: it allocates samples wherever posterior variance is largest, irrespective of the predicted value.  Both reduce to the same primitive (fit GP, maximise acquisition, evaluate, refit), but the choice of acquisition reflects whether one wants the best point or the best surrogate.

\begin{definitionbox}[BAL Acquisition Function]
\begin{equation}
U(\x) = w_{\mathrm{exp}} \cdot \mu(\x) + \frac{w_{\mathrm{var}}}{2}\log\sigma^2(\x),
\end{equation}
where $\mu(\x)$ and $\sigma^2(\x)$ are the GP posterior mean and variance, $w_{\mathrm{exp}}$ is the exploitation weight, and $w_{\mathrm{var}}$ controls exploration.  This logarithmic-variance form is the one used by \citet{rennerscheidegger_2018} and \citet{SCHEIDEGGER201968} in economic applications; it is a non-standard relative of GP-UCB \citep{srinivas2010gaussian}.  When the goal is global surrogate accuracy rather than optimization, integrated posterior variance or expected error reduction can be more natural objectives.  The weights $w_{\mathrm{exp}}$ and $w_{\mathrm{var}}$ are local to the BAL context and distinct from the discount factor $\beta$ and shock-persistence notation used elsewhere in the script.
\end{definitionbox}

\begin{definitionbox}[Algorithm: Bayesian Active Learning]
\begin{algorithmic}
\small
\STATE \textbf{Input:} Initial design $\mathcal{D}_0 = \{(\x_i, y_i)\}_{i=1}^{n_0}$, budget $N$, kernel $k$, acquisition weights $(w_{\mathrm{exp}}, w_{\mathrm{var}})$
\STATE Fit GP on $\mathcal{D}_0$: learn hyperparameters via marginal likelihood
\FOR{$n = n_0+1, \ldots, N$}
    \STATE Compute acquisition function: $U(\x) = w_{\mathrm{exp}} \cdot \mu_{n-1}(\x) + \frac{w_{\mathrm{var}}}{2}\log\sigma_{n-1}^2(\x)$
...
The BAL algorithm concentrates training points near kinks, boundary layers, and other regions where the function is hardest to approximate, achieving the same accuracy as uniform sampling with far fewer evaluations \citep{rennerscheidegger_2018, SCHEIDEGGER201968}.  The exploration--exploitation trade-off controlled by $(w_{\mathrm{exp}}, w_{\mathrm{var}})$ ensures that the algorithm balances refining the approximation in already well-sampled regions against exploring uncharted territory.

The intuition behind the acquisition function is as follows.  The first term $w_{\mathrm{exp}} \cdot \mu(\x)$ favors regions where the predicted function value is large (exploitation: sample where the function is interesting).  The second term $\frac{w_{\mathrm{var}}}{2}\log\sigma^2(\x)$ favors regions of high uncertainty (exploration: sample where we know least).  This acquisition function belongs to the Upper Confidence Bound (UCB) family \citep{srinivas2010gaussian}: with $w_{\mathrm{exp}} = 0$ it has the same maximizers as pure posterior-variance sampling, while the logarithmic variance weighting provides a more conservative exploration bonus than the standard-deviation weighting used in GP-UCB when it is combined with exploitation.  By adjusting $w_{\mathrm{exp}}$ and $w_{\mathrm{var}}$, the practitioner can control the balance.  For economic applications where the entire domain is relevant (e.g., approximating a policy function), a pure exploration strategy ($w_{\mathrm{exp}} = 0$, $w_{\mathrm{var}} > 0$) is often appropriate, reducing BAL to an uncertainty sampling scheme that minimizes the integrated posterior variance.  Figure~\ref{fig:bal-iterations} illustrates two BAL iterations on a 1D toy.
...
Prediction cost & $\mathcal{O}(N)$ per test point & $\mathcal{O}(\text{weights})$ per forward pass; independent of $N$ \\
...
% data sample as small gray dots
\addplot[only marks, mark=*, mark size=1.4pt, gray!65]
  coordinates {
   (-3.0,-0.32) (-2.7,-0.66) (-2.4,-0.85) (-2.1,-0.83) (-1.8,-0.62)
   (-1.5,-0.30) (-1.2,0.12) (-0.9,0.62) (-0.6,0.85) (-0.3,0.78)
   (0.0,0.50) (0.3,0.18) (0.6,-0.30) (0.9,-0.65) (1.2,-0.83)
   (1.5,-0.78) (1.8,-0.50) (2.1,-0.10) (2.4,0.20) (2.7,0.42)
   (3.0,0.45)};
...
\end{itemize}
The eigendecomposition of $\hat{C}$ is a standard $d \times d$ matrix operation and is cheap relative to the model evaluations.
...
\paragraph{Further applications.}  The GP framework with active learning has been applied to a range of economic settings.  \citet{rennerscheidegger_2018} develop GP-based methods for dynamic incentive problems in mechanism design, where the policy function exhibits kinks due to incentive compatibility constraints, and demonstrate that the Mat\'ern-3/2 kernel combined with BAL provides accurate approximations even at these non-smooth features.  \citet{gaegauf2023portfolio} extend the \emph{Active-Subspace GP} (ASGP) framework, developed in \S\ref{sec:gp_dp} below, to dynamic portfolio choice with transaction costs, showing that the combination of GPs, active subspaces, and value function iteration can handle the high-dimensional state spaces and non-smooth value functions that arise from no-trade regions.
...
% Deep AS validation MSE: elbow at d=1
\addplot[color=harvardcrimson, mark=*, mark size=3pt, thick]
  coordinates {(1,1.0e-4) (2,3.0e-5) (3,6.0e-5) (4,3.0e-5) (5,3.0e-5)};
...
We now show how the GP and active subspace machinery developed above can be embedded into a \emph{value function iteration} (VFI) algorithm to solve high-dimensional dynamic stochastic models.
...
Notebook 04 compares an active-learning design (starting from $n^s=6$ points and growing to $n^s=10$) against a naive uniform Latin-hypercube design at the same final budget $n^s = 10$.  At convergence, the active design achieves a smaller maximum Bellman residual on a fine validation grid \emph{and} a smaller LOO-RMSE, confirming that the points placed by \eqref{eq:bal_vfi} are doing their job.  Figure~\ref{fig:gp_vfi_2d_active} shows the corresponding behaviour on the 2D separable benchmark.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.92\linewidth]{gp_vfi_2d_active_benchmark.pdf}
\caption{GP-based value function iteration on the separable 2D benchmark $V_2(k_1, k_2) = V^*(k_1) + V^*(k_2)$, where $V^*$ is the closed-form Brock--Mirman value function with full depreciation.  The GP posterior mean (left panel of each row) and the absolute error against the analytical solution (right panel) are shown for a naive uniform Latin-hypercube design (top row) and an active-learning design (bottom row) at the same budget.  Active design points (red) are placed by maximising the posterior standard deviation \eqref{eq:bal_vfi} subject to a minimum-spacing constraint, and concentrate where the GP is most uncertain; this halves the maximum absolute error relative to the naive design.  Generated by notebook \texttt{04\_GP\_Value\_Function\_Iteration.ipynb}, cell~25.}
\label{fig:gp_vfi_2d_active}
\end{figure}
...
Notebook \tpath{04_GP_Value_Function_Iteration.ipynb} implements GP-based VFI for the 1D Brock--Mirman model with cheap-BAL active-learning design \eqref{eq:bal_vfi} and Cholesky-trick LOO diagnostics \eqref{eq:gp_loo}, demonstrating convergence, value-function 95\% credible bands, a verification run against the $\delta = 1$ analytical solution, and a 2D separable benchmark (Figure~\ref{fig:gp_vfi_2d_active}).  The active-subspace notebooks, \tpath{05_Active_Subspace_2D.ipynb}, \tpath{06_Active_Subspace_10D.ipynb}, and \tpath{07_Active_Subspace_Nonlinear.ipynb}, illustrate the same pipeline on 2D, 10D, and nonlinear test functions.
\end{remarkbox}
...
\item Bayesian active learning closes the surrogate loop: train, evaluate uncertainty, request next design point where it's largest, repeat.  The Scheidegger--Bilionis pipeline is the canonical example.
...
\addcontentsline{toc}{section}{Exercises}
\begin{enumerate}[itemsep=4pt, leftmargin=*]
\item\label{ex:ch9:1} \textbf{Posterior on three points.}  Fit a GP with RBF kernel ($\ell=1$, $\sigma_f=1$, $\sigma_y=0.1$) to three points $(0,0), (1,0.8), (2,0.3)$.  Compute the posterior mean and variance at $x^\star = 1.5$ in closed form.
\item\label{ex:ch9:2} \textbf{Marginal likelihood Occam.}  For the same three points, plot the log marginal likelihood as a function of the length scale $\ell \in [0.1, 5]$.  Identify the optimum and explain it in terms of the data-fit / complexity decomposition.
\item\label{ex:ch9:3} \textbf{Active subspace by hand.}  For $f(\x) = (x_1 + x_2 + x_3)^2 + 0.01(x_1 - x_2)^2$ on $[-1,1]^3$, compute the gradient outer product matrix $\hat C$ on a uniform sample.  Show that the leading eigenvector identifies the ``aggregate'' direction $(1,1,1)/\sqrt 3$.
\item\label{ex:ch9:4} \textbf{Deep vs.\ linear active subspace on a radial ridge.}  Take the target $y(\xi) = \exp\!\bigl(-[(w_1^\top\xi)^2 + (w_2^\top\xi)^2]\bigr)$ with $\xi \sim \mathcal{N}(0, I_{20})$ and $w_1, w_2 \in \R^{20}$ fixed orthonormal.  (i)~Compute $\hat C$ from $4\,000$ samples and confirm that it has two nonzero eigenvalues.  (ii)~Train the Tripathy--Bilionis surrogate~\eqref{eq:deep_as_ansatz} with widths~\eqref{eq:tb_widths}, Swish activation~\eqref{eq:swish}, and elastic-net loss~\eqref{eq:tb_loss} for $d \in \{1,2,3,4\}$; plot held-out MSE on a log scale and identify the elbow.  (iii)~Explain why linear AS needs $d \ge 2$ while deep AS already captures the response at $d = 1$ (cf.~notebook~\texttt{09\_Deep\_Active\_Subspace\_Ridge}).
\item\label{ex:ch9:5} \textbf{BAL on a 2D function.}  Modify notebook \texttt{02\_GP\_and\_BAL} so that one acquisition uses pure variance, $U_{\mathrm{var}}(\x)=\sigma^2(\x)$, and another uses the mixed log-variance score $U_{\mathrm{mix}}(\x)=w_{\mathrm{exp}}\mu(\x)+\tfrac{w_{\mathrm{var}}}{2}\log\sigma^2(\x)$ with $w_{\mathrm{exp}}>0$.  Compare the resulting designs and comment on which gives smoother domain coverage.  Explain why pure $\sigma^2(\x)$ and pure $\log\sigma^2(\x)$ have identical maximizers.
\item\label{ex:ch9:6} \textbf{Sobol sensitivity with a GP surrogate.}  Write a short script, or extend notebook \tpath{02_GP_and_BAL.ipynb}, to train a GP surrogate on the $4$-dimensional Genz product-peak function $f(\bm x) = \prod_{i=1}^{4}(c_i^{-2} + (x_i - w_i)^2)^{-1}$ on $[0,1]^4$ with $c = (1, 2, 0.5, 1.5)$, $w = (0.4, 0.6, 0.3, 0.7)$, using $N \in \{50, 100, 200\}$ training points.  For each $N$, compute the Sobol first-order and total-effect indices in two ways: (i)~direct on the true $f$ via $10{,}000$ Monte Carlo samples (reference), (ii)~on the GP surrogate via $10^6$ samples (cheap thanks to the surrogate).  Plot the relative error in each Sobol index against $N$.  Verify that the surrogate-based Sobol estimates converge to the reference as $N$ grows, and identify the $N$ at which all four first-order indices match the reference within $5\%$.  Discuss why surrogate-based sensitivity analysis is the workhorse for expensive simulators (climate models, structural macro models) where direct $10^6$-sample Monte Carlo is infeasible.
\item\label{ex:ch9:7} \textbf{GP overconfidence outside the convex hull.}  Train a GP with RBF kernel on $20$ points sampled uniformly from $[0, 1]$ from the function $f(x) = \sin(2\pi x)\,e^{-x}$.  (i)~Plot the posterior mean and $\pm 2$ standard deviation band on the training interval $[0,1]$; verify the posterior tracks the true function and the band is narrow.  (ii)~Now extrapolate to $x \in [1.5, 3.0]$ and plot the posterior mean and band over the extended interval.  Show analytically that for an RBF kernel with length scale $\ell$, the posterior at $x$ far from any training point ($|x - x_i| \gg \ell$ for all $i$) reverts to the prior: posterior mean $\to 0$, posterior variance $\to \sigma_f^2$.  (iii)~Verify numerically: at $x = 3$, the posterior mean is essentially zero (independent of the training data), and the posterior standard deviation has expanded back to the prior $\sigma_f$.  (iv)~Discuss the danger: the GP looks ``confident'' (small variance) inside the convex hull but its uncertainty estimate is unreliable outside, and naive Bayesian active learning may fail to flag this.  Mitigation: use a Mat\'ern-$\nu$ kernel with smaller $\nu$ (heavier-tailed), or a boundary-aware acquisition function.
\end{enumerate}

% ============================================================================
% CHAPTER 10: STRUCTURAL ESTIMATION
% ============================================================================
\chapter{Structural Estimation via SMM}
\label{ch:estimation}

With the surrogate machinery of Chapter~\ref{ch:gp} in hand, we now turn to one of its most important applications: \emph{structural estimation}.  The chapter's companion notebooks use the Brock--Mirman growth model to estimate first the productivity persistence parameter $\varrho$, and then the pair $\theta=(\beta,\varrho)$, by Simulated Method of Moments (SMM).  The key computational idea is the same in both cases: train one policy surrogate with the structural parameter as an additional input, then reuse that trained network inside the estimator instead of re-solving the dynamic program at every candidate parameter value.  The econometric foundations are \citet{mcfadden1989method}, \citet{pakes1989simulation}, \citet{lee1991simulation}, \citet{duffie1993simulated}, and \citet{gourieroux1993indirect}; the surrogate logic follows the deep-surrogate and GP-surrogate pipelines in \citet{chen2025Deep} and \citet{SCHEIDEGGER201968}.
...
The network outputs a savings rate.  In the single-parameter exercise,
\[
s_t = \mathcal{N}_{\rho}(z_t,K_t,\varrho) \in (0,1),
\]
with $\beta$ calibrated to $0.96$.  In the joint exercise the input becomes $(z_t,K_t,\beta,\varrho)$.  In either case,
\begin{align}
C_t &= Y_t(1-s_t), \\
K_{t+1} &= (1-\delta)K_t + Y_t s_t.
\end{align}
This output parameterization enforces $C_t>0$ and $I_t=Y_t s_t\geq0$ by construction.
...
\]
For a sampled state--parameter pair $(z_i,K_i,\theta_b)$, where $\theta_b=\varrho_b$ in the scalar exercise and $\theta_b=(\beta_b,\varrho_b)$ in the joint exercise, the residual can be written as
\begin{equation}
G_i(\theta_b)
=
1
-
\beta_b C_i
\E{\frac{1-\delta+\alpha z_{i,t+1}(K_i')^{\alpha-1}}{C_{i,t+1}}},
\label{eq:smm_euler_residual}
\end{equation}
where $K_i'=(1-\delta)K_i+z_iK_i^\alpha s_i$, $C_i=z_iK_i^\alpha(1-s_i)$, and $C_{i,t+1}$ is computed by feeding $(z_{i,t+1},K_i',\theta_b)$ back through the same network.  The expectation over $z_{i,t+1}$ is approximated by Gauss--Hermite quadrature (Section~\ref{sec:quadrature_rules}).  A representative training loss is
\begin{equation}
\ell_\rho
=
\frac{1}{N_sN_\theta}
...
\end{equation}
where $W \in \R^{q \times q}$ is a symmetric positive definite \emphc{weighting matrix}.  In code, this objective is simply the weighted inner product \texttt{(m\_sim - m\_data) @ W @ (m\_sim - m\_data)} of the moment deviations, with \texttt{m\_sim = simulate\_moments(theta)} the model-implied moments and \texttt{m\_data} the empirical ones.
...
\item \textbf{Diagonal weighting} ($W = \mathrm{diag}(1/\hat{\sigma}_1^2, \ldots, 1/\hat{\sigma}_q^2)$): each moment is scaled by the inverse of its estimated variance.  This corrects for differing units and precision.  In code: \texttt{W = np.diag(1.0 / np.var(m\_bootstrap, axis=0))}.
...
\noindent With identity weighting, the criterion is simply \texttt{diff @ diff}.  With inverse discrepancy-covariance weighting, it is the squared Mahalanobis distance between simulated and empirical moments.
...
\end{equation}
which is necessary for the parameter to be identified from the chosen moments.  Let $M=\partial m/\partial\theta'|_{\theta_0}$, and let $\Omega$ denote the asymptotic covariance of $\sqrt{T}\hat g(\theta_0)$.  The large-sample distribution is
\begin{equation}
...
Notebook \tpath{03_Structural_Estimation_BM.ipynb} calibrates $\beta=0.96$, sets $\varrho_{\mathrm{true}}=0.90$, and estimates $\varrho\in[0.50,0.99]$.  Let $\{C_t(\varrho),I_t(\varrho),Y_t(\varrho)\}_{t=1}^T$ denote a simulated sample at candidate persistence $\varrho$.  The estimator uses three moments:
\begin{align}
...
\end{align}
The output autocorrelation is the most direct persistence moment.  The volatility moment should be interpreted as an empirical simulated moment, not as the level-variance formula.  For the AR(1) shock,
\[
\mathrm{Var}(\log z_t)=\frac{\sigma_z^2}{1-\varrho^2},
\qquad
\mathrm{Var}(\Delta\log z_t)=\frac{2\sigma_z^2}{1+\varrho},
\]
so the familiar $1/(1-\varrho^2)$ amplification applies to the \emph{level} of log productivity, not to first differences.  The notebook also reports the mean savings rate as a diagnostic and correctly treats it as nearly uninformative for $\varrho$.

\paragraph{Joint exercise.}
Notebook \tpath{03b_Structural_Estimation_BM_Joint.ipynb} estimates $\theta=(\beta,\varrho)$, with $\beta\in[0.92,0.99]$ and $\varrho\in[0.50,0.99]$.  It uses mean savings, growth volatility, consumption-growth autocorrelation, and output autocorrelation.  The just-identified specification uses mean savings and consumption-growth autocorrelation to make the partial-identification ridge visible; the over-identified specification uses all four moments and collapses the ridge around the synthetic truth.

\paragraph{A deterministic objective.}
Because the same initial condition and random seed are used for every candidate $\theta$, the map $\theta\mapsto m(\theta)$ is deterministic in the notebooks.  This is still standard SMM; the fixed seed is a common-random-numbers device that removes irrelevant simulation noise while we study identification and optimization.  In more realistic estimation exercises one averages over multiple replications or increases the simulation length to make Monte Carlo noise negligible.

\paragraph{Implementation.}
The single-parameter estimation routine proceeds in two steps:
\begin{enumerate}[itemsep=3pt]
\item Evaluate the SMM objective on a coarse grid over $\varrho \in [0.50,0.99]$ to verify that the criterion is well behaved and to visualize identification.
\item Refine the minimizer with a bounded scalar optimizer such as \texttt{minimize\_scalar}.
\end{enumerate}
Listing~\ref{lst:smm_rho} shows the resulting Python recipe.  The joint notebook maps the 2D criterion on a grid, then refines from the grid minimizer using bounded Nelder--Mead.  Since the policy surrogate has already been trained, each evaluation of $m(\theta)$ requires only a forward simulation, not a full re-solution of the dynamic program.

\begin{lstlisting}[caption={Direct SMM estimation of TFP persistence in the Brock--Mirman exercise.},label={lst:smm_rho}]
def smm_objective(rho, policy, m_data, seed=123):
    C, I, Y = simulate_economy(policy, rho, T_burn=200, T_sim=1000, seed=seed)
    m_sim = compute_moments(C, I, Y)
    diff = m_sim - m_data
    return diff @ diff

rho_grid = np.linspace(0.50, 0.99, 81)
obj_grid = np.array([smm_objective(r, policy, m_data) for r in rho_grid])

result = minimize_scalar(
    lambda r: smm_objective(r, policy, m_data),
    bounds=(0.50, 0.99),
    method="bounded",
)
rho_hat = result.x
...
\paragraph{Why the surrogate still matters.}  Even though the estimation step is standard SMM, the pseudo-state surrogate changes the computational bottleneck.  Without it, each candidate $\theta$ would require re-solving the dynamic program.  With it, the optimization reduces to repeated forward simulations under an already-trained policy rule.  This is the same computational leverage exploited in recent applications of deep surrogates to dynamic structural models, both in macro-finance \citep{kase2022estimating, payne2024deepsam, chen2025Deep, kubler2025using} and in climate-economics integrated assessment \citep{friedlDeep2023}.
...
The Cholesky-trick LOO formula~\eqref{eq:gp_loo} of \S\ref{sec:gp_loo} delivers a held-out predictive error for each moment GP at zero marginal cost beyond the existing posterior factorisation.  Notebook \tpath{03_Structural_Estimation_BM.ipynb} reports
\[
\mathrm{LOO\text{-}RMSE}_j \;=\; \sqrt{\frac{1}{n}\sum_{i=1}^{n}\bigl(\widehat m_j^{-i}(\theta^{(i)}) - m_j(\theta^{(i)})\bigr)^2}
\]
for every moment $j$ and every design size $n$, and notebook \tpath{03b_Structural_Estimation_BM_Joint.ipynb} tracks it as the design grows under active learning.  As an independent sanity check, both notebooks also evaluate the GP at a \emph{fresh} interior holdout point $\theta_\mathrm{holdout}$ that is never trained on, and report the resulting predictive RMSE; agreement with LOO is the criterion for declaring the moment surrogate trustworthy.

\subsection{Active Learning of the Moment Surrogate}

Two acquisition strategies are illustrated in the notebooks, matched to the dimensionality of the parameter:

\paragraph{Single-parameter case (notebook 03).}  With a scalar $\theta=\varrho\in[0.50,0.99]$, the design starts from a coarse uniform grid of $n_0=8$ points and is enriched by $n_\mathrm{add}=6$ active points placed sequentially at locations of largest standardised moment-GP posterior uncertainty,
\[
\theta^{\mathrm{next}} \in \argmax_{\theta \in \mathcal{X}^\mathrm{cand}}\;\Bigl\|\boldsymbol\sigma_m(\theta) \,/\, \bar{\boldsymbol\sigma}_m\Bigr\|_2,
\]
subject to a minimum-spacing constraint of $1.5\times$ grid spacing against existing design points.  This is the same pure-exploration acquisition we used for VFI~\eqref{eq:bal_vfi}, modulo the per-moment normalisation that prevents one large-magnitude moment from dominating the objective.

\paragraph{Joint-parameter case (notebook 03b).}  With $\theta=(\beta,\varrho)$ on a 2D rectangle, pure exploration is wasteful because most of the rectangle sits far from the SMM minimiser.  The notebook switches to a BoTorch implementation of an Upper-Confidence-Bound (UCB) acquisition on the transformed score $\widetilde Q(\theta) := -\log_{10}(Q(\theta)+\varepsilon)$, multiplicatively weighted by the moment-GP posterior uncertainty:
\begin{equation}
a(\theta)
=
...
\end{equation}
Here $\widetilde{\mathrm{UCB}}_{\widetilde Q}$ denotes the quantile-scaled and clipped UCB score used in the notebook.  The first factor exploits, biasing the design toward $(\beta,\varrho)$ pairs with small SMM criterion, while the second explores, requiring the design to also hit places where the moment GP is uncertain.  The additive constant $0.25$ keeps the exploration term active even where the scaled UCB is zero, preventing pathological degeneracy in the acquisition.

\paragraph{Three-way comparison.}  At a fixed design budget, notebook 03b compares pilot grid / naive Latin-hypercube / BoTorch-BAL designs along three axes: (i) leave-one-out error on the moment GPs; (ii) error on the recovered SMM criterion against a fresh reference grid; (iii) accuracy of the recovered estimate $(\hat\beta,\hat\varrho)$.  In the reported synthetic run, the active design gives the most stable local moment surrogate.
...
First, the criterion has a long, shallow ridge along the $\beta$ direction in the just-identified specification: the data are nearly uninformative about $\beta$ once $\varrho$ is fixed, so a wide range of $\beta$-values fits almost equally well.  This is partial identification, in textbook form, visualised on the criterion surface.

Second, the ridge collapses to a localized minimum in the over-identified specification.  In the synthetic CRN run, the recovered estimate sits very close to $(\beta_{\mathrm{true}},\varrho_{\mathrm{true}})=(0.96,0.90)$.  This makes a useful pedagogical point: identification is a property of the moment selection, not of the estimator.

\begin{figure}[ht]
\centering
\includegraphics[width=0.92\linewidth]{joint_criterion_contour.pdf}
\caption{Direct SMM criterion for the joint Brock--Mirman estimation.  The left panel uses the just-identified two-moment specification and displays a shallow ridge along $\beta$, showing weak identification of patience from those moments.  The right panel uses the over-identified four-moment specification, which adds volatility and output-persistence information and produces a localized minimum near the synthetic truth.  Generated by notebook 03b.}
\label{fig:smm_2d_criterion}
\end{figure}

Figure~\ref{fig:smm_gp_objective} shows the second-layer GP surrogate fitted to the joint moment map near the estimator.  This is a different object from Figure~\ref{fig:smm_2d_criterion}: the figure validates the GP approximation used for cheap downstream objective evaluations, not the raw direct-SMM contour itself.

\begin{figure}[ht]
\centering
\includegraphics[width=0.92\linewidth]{joint_gp_objective_surrogate.pdf}
\caption{BoTorch-BAL GP surrogate for the joint SMM objective.  The left panel shows the GP posterior mean of $\log_{10}Q(\beta,\varrho)$; the right panel shows standardized posterior uncertainty of the moment vector.  White dots are the training design, orange squares are active-learning additions, red stars are held-out validation points and the synthetic truth, the white circle is the direct-SMM estimate, and the black cross is the GP-SMM estimate.  Generated by notebook 03b.}
\label{fig:smm_gp_objective}
\end{figure}

\begin{remarkbox}[Hands-on]
Notebook \tpath{03_Structural_Estimation_BM.ipynb} fits the scalar persistence exercise and the local GP moment-map surrogate with $8$ pilot plus $6$ BAL points.  Notebook \tpath{03b_Structural_Estimation_BM_Joint.ipynb} performs joint $(\beta,\varrho)$ estimation, visualises the partial-identification ridge in Figure~\ref{fig:smm_2d_criterion}, and compares pilot / naive / active GP designs at a common budget.
\end{remarkbox}
...
\item \citet{kase2022estimating, chen2025Deep}, modern deep-surrogate-based estimation in macro-finance.
...
\addcontentsline{toc}{section}{Exercises}
\begin{enumerate}[itemsep=4pt, leftmargin=*]
\item\label{ex:ch10:1} \textbf{Identification.}  In notebook \tpath{03_Structural_Estimation_BM.ipynb}, choose two candidate moments and compute the finite-difference Jacobian $\partial m/\partial\varrho$ at $\varrho_{\mathrm{true}}=0.90$.  Which moment provides stronger local identification?
\item\label{ex:ch10:2} \textbf{Optimal weighting.}  Show that under standard regularity conditions, $W^\star=\Omega^{-1}$ minimizes the asymptotic variance of $\hat\theta_{\mathrm{SMM}}$, where $\Omega$ is the covariance of the moment discrepancy.  Why does identity weighting still appear in the first stage?
\item\label{ex:ch10:3} \textbf{Common random numbers.}  In the scalar Brock--Mirman exercise, plot the SMM objective as a function of $\varrho$ with and without common random numbers.  Quantify the objective noise across repeated Monte Carlo panels under the same candidate grid.
\item\label{ex:ch10:4} \textbf{SMM vs.\ SBI.}  Outline the algorithmic difference between estimating $\theta$ by SMM (one optimization per dataset) and by neural simulation-based inference (one training run plus one posterior evaluation per dataset).  In which regime does SBI dominate?
\item\label{ex:ch10:5} \textbf{$J$-statistic and overidentification.}  In notebook \tpath{03b_Structural_Estimation_BM_Joint.ipynb}, use the over-identified specification with $q=4$ moments and $p=2$ parameters.  (i)~State the asymptotic distribution of the $J$-statistic under correct specification when $W=\Omega^{-1}$.  (ii)~Compute $J(\hat\theta)$ on the original synthetic sample and report whether the $\chi^2_2$ test rejects at $\alpha=0.05$.  (iii)~Repeat across Monte Carlo samples generated at the truth and compare the empirical distribution with $\chi^2_2$.  (iv)~Introduce a structural break in one model parameter and verify that the $J$ distribution shifts to the right.
\item\label{ex:ch10:6} \textbf{Bootstrap confidence intervals.}  Compare three confidence-interval procedures for $\hat\theta_\mathrm{SMM}$: (a)~plug-in sandwich standard errors; (b)~moving-block or stationary bootstrap of the time-series sample; (c)~parametric bootstrap, drawing new samples under the simulated model at $\hat\theta$.  Report the confidence intervals and coverage across repeated Monte Carlo replications.
\item\label{ex:ch10:7} \textbf{Maximum likelihood vs.\ SMM efficiency.}  Suppose the productivity process $\log z_t$ is observed.  Implement the Gaussian AR(1) MLE for $\varrho$ and compare it with the SMM estimator based on the moments in notebook \tpath{03_Structural_Estimation_BM.ipynb}.  On Monte Carlo replications at several sample sizes, report bias, variance, and MSE.  Explain why MLE is efficient for this observed-shock likelihood, while SMM reaches the GMM efficiency bound only for the chosen moment vector.  As an optional extension, repeat the beta-only MLE comparison in the full-depreciation log-utility special case where the policy has a closed form.
\end{enumerate}
...
\item[RICE.]  Twelve-region extension of DICE with trade.  Used for regional SCC and equity questions.
...
\paragraph{Why ACE is a useful complement.}  We will develop the full CDICE numerical model in \S\ref{sec:dice_deqn}; before doing so, it is worth flagging the role of the Analytic Climate Economy of \citet{traeger2018ace} (\textsc{ACE}).  ACE makes carefully chosen log-linear approximations to the carbon cycle, temperature dynamics, and damage function so that the planner's first-order conditions admit a \emph{closed-form} optimal carbon tax.  This pays a small accuracy price relative to the numerical solution of DICE-class models but it delivers a rare object: an analytic SCC formula in terms of preference and climate parameters.  In our deep-learning workflow, ACE serves two roles, (i) a sanity check for the DEQN-trained CDICE policies (the numerical SCC should converge to the analytic SCC as $\Delta t \to 0$ and as the linearization domain widens), and (ii) a transparent decomposition of which structural parameters drive the SCC, complementing the Sobol/Shapley sensitivity analysis of \S\ref{sec:deep_uq}.  In short: CDICE (introduced next) is the model we solve; ACE is the analytic yardstick we calibrate against.

The \emph{Dynamic Integrated model of Climate and the Economy} (DICE), developed by \citet{nordhaus1994managing}, is the most influential IAM; in this chapter we follow the variant of \citet{nordhausRevisitingSocialCost2017}.  It couples a neoclassical growth model with a reduced-form climate module in a single global framework.

\paragraph{State variables and controls.}
The economic state is the capital stock $K_t$.  The climate state comprises three carbon reservoirs (atmosphere, upper ocean, and lower ocean) and two temperature layers (atmospheric $T^{\mathrm{AT}}_t$ and oceanic $T^{\mathrm{OC}}_t$).  Exogenous trends include population $L_t$, TFP $A_t$, emissions intensity $\sigma_t$, land-use emissions $E_{\mathrm{Land},t}$, and exogenous forcing $F^{\mathrm{EX}}_t$.  The planner controls the savings rate $s_t$ and the emissions abatement rate $\mu_t \in [0,1]$.

\subsection{Carbon Cycle}

DICE models the carbon cycle as a three-box linear system.  Let $M_t = (M_{\mathrm{AT},t},\, M_{\mathrm{UO},t},\, M_{\mathrm{LO},t})^\top$ be the vector of carbon concentrations.  The transition is:
\begin{equation}
M_{t+1} = (I + \Delta_t \cdot B)\, M_t + \Delta_t \cdot E_t,
\label{eq:carbon_cycle}
\end{equation}
where $B$ is the transfer matrix governing exchange rates between reservoirs, $\Delta_t$ is the time step, and $E_t = (E_{\mathrm{ind},t} + E_{\mathrm{Land},t},\, 0,\, 0)^\top$.  Industrial emissions depend on economic output and abatement:
\begin{equation}
E_{\mathrm{ind},t} = (1 - \mu_t)\,\sigma_t\, K_t^\alpha (A_t L_t)^{1-\alpha}.
\end{equation}
...
\begin{align}
T^{\mathrm{AT}}_{t+1} &= T^{\mathrm{AT}}_t + \Delta_t \cdot c_1 \bigl(F_t - \lambda\, T^{\mathrm{AT}}_t - c_3(T^{\mathrm{AT}}_t - T^{\mathrm{OC}}_t)\bigr), \label{eq:temp_at} \\
T^{\mathrm{OC}}_{t+1} &= T^{\mathrm{OC}}_t + \Delta_t \cdot c_4 \bigl(T^{\mathrm{AT}}_t - T^{\mathrm{OC}}_t\bigr), \label{eq:temp_oc}
\end{align}
where radiative forcing is
\begin{equation}
F_t = F_{\mathrm{2\times CO_2}} \frac{\log(M^{\mathrm{AT}}_t / M^{\mathrm{AT}}_{\mathrm{EQ}})}{\log 2} + F^{\mathrm{EX}}_t.
\label{eq:forcing}
\end{equation}
The parameter $\lambda = F_{\mathrm{2\times CO_2}} / \Delta T_{\mathrm{AT},\times 2}$ is determined by the \emph{equilibrium climate sensitivity} (ECS), defined as the long-run atmospheric warming from a doubling of CO$_2$ concentration.  ECS is one of the most consequential and uncertain parameters in climate science \citep{roe2007climate, knutti2017beyond}.  Observational and model-based estimates place ECS in a \emph{likely} (66\,\%) range of 2.5\textdegree C--4\textdegree C and a \emph{very likely} (90\,\%) range of 2\textdegree C--5\textdegree C, with a best estimate of approximately 3\textdegree C \citep[IPCC AR6 Synthesis Report;][]{calvinIPCC2023Climate2023a}; ECS uncertainty is one of the largest single sources of variance in the SCC.

\subsection{Damage Functions}

Climate damages reduce gross output multiplicatively.  \citet{nordhaus2008question} proposed the quadratic specification
\begin{equation}
\Omega^N_t(T_{\mathrm{AT},t}) = \frac{1}{1 + \pi_1 T_{\mathrm{AT},t} + \pi_2 T_{\mathrm{AT},t}^2},
\label{eq:damage_nordhaus}
\end{equation}
which is relatively benign for moderate warming.  \citet{weitzman2012ghg} argued that catastrophic tipping points require a steeper damage function:
\begin{equation}
\Omega^W_t(T_{\mathrm{AT},t}) = \frac{1}{1 + \bigl(\tfrac{1}{\psi_1} T_{\mathrm{AT},t}\bigr)^2 + \bigl(\tfrac{1}{2 \cdot TP_t} T_{\mathrm{AT},t}\bigr)^{6.754}},
\label{eq:damage_weitzman}
\end{equation}
where $TP_t$ is a stochastic tipping-point threshold.  The degree of convexity of the damage function is one of the most important determinants of the optimal carbon tax.

\subsection{CDICE: Recalibration}
...
\end{tabular}
\end{table}
This calibration ensures that the reduced-form climate module is consistent with state-of-the-art earth system models.  CDICE also introduces a transparent time-step formulation, $X_{t+\Delta t} = X_t + \Delta t \cdot f(X_t, u_t; \theta)$, that allows coherent implementation at annual, 5-year, or 10-year resolution within a single generic framework.

\section{Solving DICE with DEQNs}
\label{sec:dice_deqn}

The DEQN framework from Chapters~\ref{ch:deqn}--\ref{ch:irbc} extends naturally to the stochastic IAM.  We follow \citet{friedlDeep2023} and \citet{Folini_2021} for the full model formulation.

\subsection{Epstein--Zin Preferences}

Chapters~\ref{ch:deqn}--\ref{ch:ct_theory} used CRRA preferences, in which risk aversion and the intertemporal elasticity of substitution (IES) are bundled as $1/\gamma$.  Climate economics requires separating the two: a 100-year climate disaster demands both consumption smoothing across decades (high IES) and aversion to catastrophic tail risk (high risk aversion).  Recursive (Epstein--Zin) preferences allow exactly this decoupling.  Using the normalized Bansal--Yaron convention and writing $\beta^{\mathrm{EZ}}_t := \exp(-\rho \Delta_t)$ for the one-period discount factor, the planner maximizes
\begin{equation}
V(\bm{X}_t)^{1-1/\psi} = \max_{C_t, K_{t+1}, \mu_t}\left\{(1-\beta^{\mathrm{EZ}}_t)\left(\frac{C_t}{L_t}\right)^{1-1/\psi} L_t + \beta^{\mathrm{EZ}}_t\,\mathbb{E}_t\!\left[V(\bm{X}_{t+1})^{1-\gamma}\right]^{\frac{1-1/\psi}{1-\gamma}}\right\},
\label{eq:ez_iam}
\end{equation}
For a deterministic walkthrough, see notebook \texttt{05\_DICE\_DEQN\_EpsteinZin}; the stochastic-weather extension is in notebook \texttt{04\_DICE\_DEQN\_EpsteinZin\_Weather}.  The optimization is subject to the budget constraint, capital accumulation, climate dynamics, and learning dynamics.  The key advantage of Epstein--Zin over standard CRRA is the separation of risk aversion ($\gamma$) from the intertemporal elasticity of substitution ($\psi$), which is crucial for long-run climate risk where agents must trade off consumption smoothing against catastrophic tail scenarios.

\subsection{Nonstationarity: Time as a State Variable}

Unlike the stationary models of Chapters~\ref{ch:deqn}--\ref{ch:olg}, the IAM is \emph{nonstationary}: total factor productivity $A_t$, population $L_t$, the carbon intensity $\sigma_t$, land-use emissions $E_{\mathrm{Land},t}$, and exogenous forcing $F^{\mathrm{EX}}_t$ all follow deterministic but time-varying paths.  Consequently, the policy function $\bm{p}(\bm{X}_t)$ cannot be time-invariant: even if the economic state $k_t$ returns to a previous value, the optimal decision differs because the exogenous trends have moved.

The solution strategy has two prongs: \emph{detrend what you can, and add time for what remains}.

\paragraph{Detrending.}
Variables that grow with the productivity--population product $A_t L_t$ are rescaled to per-effective-capita units:
\begin{equation}
c_t \;:=\; \frac{C_t}{A_t\,L_t}, \qquad
k_t \;:=\; \frac{K_t}{A_t\,L_t}.
\label{eq:iam_detrend}
\end{equation}
In these units the budget constraint, capital accumulation, and Euler equations take a form that is closer to stationary, but not fully, because the carbon intensity $\sigma_t$, land-use emissions $E_{\mathrm{Land},t}$, and exogenous forcing $F^{\mathrm{EX}}_t$ remain time-dependent.

\paragraph{Time transformation.}
To handle the residual nonstationarity, time $t \in [0,\infty)$ enters as an explicit state variable.  Because neural networks perform best with bounded inputs, we apply a strictly monotonic time-rescaling (a trick regularly used in the dynamic-programming and reduced-state DICE literature, e.g.\ by \citet{traeger4StatedDICEQuantitatively2014}),
\begin{equation}
...
\end{equation}
where $\vartheta > 0$ is a compression parameter that controls how fast $\tau$ saturates.  In practice the network receives $\tau$ rather than $t$, confining the time input to the unit interval regardless of the planning horizon (typically 300 years).

\begin{remarkbox}[Connection to the stationary case]
Setting $\tau = \mathrm{const}$ (equivalently, freezing the exogenous trends) recovers the stationary DEQN formulation of Chapters~\ref{ch:deqn}--\ref{ch:olg}.  Nonstationarity therefore adds exactly one new input dimension to the network; everything else in the DEQN framework remains unchanged.
\end{remarkbox}
...
\begin{equation}
\bm{X}_t = \bigl[\underbrace{k_t,\, M_{\mathrm{AT},t},\, M_{\mathrm{UO},t},\, M_{\mathrm{LO},t},\, T^{\mathrm{AT}}_t,\, T^{\mathrm{OC}}_t,\, \mu_{f,t},\, S_{f,t},\, \tau_t}_{9\text{ endogenous/exogenous states}};\; \underbrace{\theta}_{N\text{ pseudo-state parameters}}\bigr],
\label{eq:iam_state}
\end{equation}
where $\mu_{f,t}$ and $S_{f,t}$ are the posterior mean and variance of the climate feedback parameter (Bayesian learning states), and $\theta$ collects uncertain parameters treated as pseudo-states (see Section~\ref{sec:deep_uq}).

\subsection{Derivation of Euler Equations}

The derivation follows the standard Lagrangian approach.  Write the Lagrangian with multipliers for the budget constraint, each climate transition equation (carbon cycle and temperature dynamics), and the abatement bound $0 \leq \mu_t \leq 1$.
...
Over a 300-year horizon, $A_t$ and $L_t$ can move the natural scale of marginal utilities and multipliers substantially, with the direction and magnitude depending on the IES through $A_t^{1-1/\psi}L_t$.  Such scale drift makes network outputs and gradients harder to optimize stably.  Following the detrending logic of~\eqref{eq:iam_detrend}, all multipliers are divided by $A_t^{1-1/\psi}\,L_t$:
\begin{equation}
...
\end{equation}
and analogously for the remaining multipliers $\hat{\nu}^{\mathrm{LO}}_t$, $\hat{\eta}^{\mathrm{AT}}_t$, and $\hat{\eta}^{\mathrm{OC}}_t$.  The normalization also induces an \emph{effective discount factor} that absorbs the trend growth:
\begin{equation}
\hat{\beta}_t \;:=\; \exp\!\left(-\rho + \left(1-\frac{1}{\psi}\right)\!\bigl(g^A_t + g^L_t\bigr)\right),
\label{eq:iam_eff_beta}
\end{equation}
where $g^A_t := \ln(A_{t+1}/A_t)$ and $g^L_t := \ln(L_{t+1}/L_t)$.  All intertemporal equations below use $\hat{\beta}_t$ in place of $e^{-\rho}$.
If the model period is not one year and $\rho$ is quoted as an annual continuous-time rate, replace $\rho$ by $\rho\Delta_t$ in this expression.
...
\exp\!\bigl(g^A_t + g^L_t\bigr)\,\hat{\lambda}_t - \hat{\beta}_t\,\hat{\lambda}_{t+1}\bigl[\Omega(T_{\mathrm{AT},t+1})\bigl(1-\Theta(\mu_{t+1})\bigr)\alpha k_{t+1}^{\alpha-1} + (1-\delta)\bigr] \nonumber\\
&\quad - \hat{\nu}^{\mathrm{AT}}_{t+1}\,\sigma_{t+1}(1-\mu_{t+1})A_{t+1}L_{t+1}\alpha k_{t+1}^{\alpha-1} = 0,
\label{eq:iam_foc_k}\\[4pt]
\frac{\partial \mathcal{L}}{\partial \mu_t} = 0 \;&\Leftrightarrow\;
\hat{\lambda}_t\,\Omega(T_{\mathrm{AT},t})\Theta'(\mu_t)\,k_t^\alpha + \hat{\lambda}^\mu_t + \hat{\nu}^{\mathrm{AT}}_t\,\sigma_t\,A_t\,L_t\,k_t^\alpha = 0.
\label{eq:iam_foc_mu}
\end{align}
...
\label{eq:iam_fb}
\end{equation}
with the same regularization parameter $\varepsilon_{\mathrm{FB}} \geq 0$ used in Chapters~\ref{ch:irbc}--\ref{ch:olg}.  At $\varepsilon_{\mathrm{FB}} = 0$ the zero set of $\Psi^{\mathrm{FB}}$ coincides with the positive axes in the $(\hat{\lambda}^\mu_t,\, 1-\mu_t)$-plane, enforcing the original complementarity exactly but the function is non-differentiable at the origin; with $\varepsilon_{\mathrm{FB}} > 0$ the function is differentiable everywhere at the cost of a slight relaxation of exact complementarity.

\subsection{Bayesian Learning}

Temperature evolves with uncertain climate feedback $\tilde{f}_{t+1} \sim \mathcal{N}(\mu_{f,t}, S_{f,t})$, entering the atmospheric temperature equation through the feedback term $\varphi_{1C}\tilde{f}_{t+1}T^{\mathrm{AT}}_t$.  The planner observes temperature realizations and updates beliefs via Gaussian conjugate posteriors:
\begin{align}
\mu_{f,t+1} &= \frac{S_{\epsilon_T}\,\mu_{f,t} + \varphi_{1C}\,T^{\mathrm{AT}}_t\bigl(\varphi_{1C}\,T^{\mathrm{AT}}_t\,\tilde{f}_{t+1} + \tilde{\epsilon}_{T,t+1}\bigr)S_{f,t}}{S_{\epsilon_T} + (\varphi_{1C}\,T^{\mathrm{AT}}_t)^2\,S_{f,t}}, \label{eq:bayes_mean}\\
S_{f,t+1} &= \frac{S_{\epsilon_T} \cdot S_{f,t}}{S_{\epsilon_T} + (\varphi_{1C}\,T^{\mathrm{AT}}_t)^2\,S_{f,t}}. \label{eq:bayes_var}
\end{align}
Learning creates an insurance--information trade-off: higher near-term emissions generate more informative temperature signals but also risk irreversible damages.

\subsection{DEQN Implementation}

\paragraph{Policy network specification.}
The policy function approximated by the neural network outputs a 9-dimensional vector:
\begin{equation}
\mathcal{N}_\rho(\bm{x}_t) \;\in\; \mathbb{R}^9
\;:=\; \bigl(k_{t+1},\; \mu_t,\; \hat{\lambda}_t,\; \hat{\lambda}^\mu_t,\; \hat{\nu}^{\mathrm{AT}}_t,\; \hat{\nu}^{\mathrm{UO}}_t,\; \hat{\nu}^{\mathrm{LO}}_t,\; \hat{\eta}^{\mathrm{AT}}_t,\; \hat{\eta}^{\mathrm{OC}}_t\bigr).
\label{eq:iam_nn_output}
\end{equation}
This comprises 2 choice variables ($k_{t+1}$, $\mu_t$) and 7 normalized Lagrange/KKT multipliers.  A key difference from the stationary DEQN of Chapters~\ref{ch:deqn}--\ref{ch:irbc} is that the network must learn the shadow prices of all climate constraints, not just the economic choices.  Without the climate multipliers, the planner would have no gradient signal about how today's decisions propagate through the carbon cycle and temperature dynamics into future damages.
...
\end{equation}
so $c_t$ is not itself a network output.  This is why the loss list below contains the budget residual ($l_2$) but no separate consumption-FOC residual: the FOC is built into how $c_t$ is computed in the forward pass.

The network architecture uses 2 hidden layers with 1024 units each, SELU activation, and the Adam optimizer with learning rate $10^{-5}$.  Training alternates between broad sampling (Phase~1) and endogenous simulation (Phase~2), as described in Chapter~\ref{ch:irbc}.

\paragraph{The 8 loss components.}
Each equilibrium condition from the previous subsection becomes a residual $l_m = 0$.  The eight components are:
{\small
\begin{align}
l_1 &:= \exp\!\bigl(g^A_t + g^L_t\bigr)\,\hat{\lambda}_t - \hat{\beta}_t\,\hat{\lambda}_{t+1}\bigl[\Omega(T_{\mathrm{AT},t+1})\bigl(1-\Theta(\mu_{t+1})\bigr)\alpha k_{t+1}^{\alpha-1} + (1-\delta)\bigr] \nonumber\\
    &\quad - \hat{\nu}^{\mathrm{AT}}_{t+1}\,\sigma_{t+1}(1-\mu_{t+1})A_{t+1}L_{t+1}\alpha k_{t+1}^{\alpha-1}
    \tag*{\text{(capital Euler)}}\label{eq:iam_l1}\\[3pt]
l_2 &:= \Omega(T_{\mathrm{AT},t})\bigl(1-\Theta(\mu_t)\bigr)\,k_t^\alpha + (1-\delta)\,k_t - c_t - \exp\!\bigl(g^A_t + g^L_t\bigr)\,k_{t+1}
...
l_6 &:= \hat{\eta}^{\mathrm{AT}}_t - \hat{\beta}_t\!\Bigl[-\hat{\lambda}_{t+1}\,\Omega'(T_{\mathrm{AT},t+1})\bigl(1-\Theta(\mu_{t+1})\bigr)\,k_{t+1}^\alpha + \hat{\eta}^{\mathrm{AT}}_{t+1}\!\Bigl(1-c_1\,\tfrac{F_{\mathrm{2\times CO_2}}}{\Delta T_{\mathrm{AT},\times 2}}-c_1 c_3\Bigr) + \hat{\eta}^{\mathrm{OC}}_{t+1}\,c_4\Bigr]
...
l_8 &:= \hat{\lambda}^\mu_t + (1-\mu_t) - \sqrt{(\hat{\lambda}^\mu_t)^2 + (1-\mu_t)^2 + \varepsilon_{\mathrm{FB}}}
    \tag*{\text{(Fischer--Burmeister)}}\label{eq:iam_l8}
\end{align}
}
Loss components $l_1$--$l_2$ enforce intertemporal optimality and feasibility, $l_3$--$l_7$ are the envelope conditions that price the five climate state variables, and $l_8$ handles the abatement complementarity.
...
\label{eq:iam_loss}
\end{equation}
This is the same sum-of-squared-residuals structure as in Chapter~\ref{ch:deqn}, but with 8 equations per time step instead of $2N+1$.
...
\begin{equation}
\bm{x}_{t+1} = \bigl(k_{t+1},\; M^{\mathrm{AT}}_{t+1},\; M^{\mathrm{UO}}_{t+1},\; M^{\mathrm{LO}}_{t+1},\; T^{\mathrm{AT}}_{t+1},\; T^{\mathrm{OC}}_{t+1},\; \mu_{f,t+1},\; S_{f,t+1},\; t+1;\; \theta\bigr)^T,
...
\item $t$ is incremented by the time step $\Delta_t$, equivalently $\tau_{t+1} = 1 - \exp(-\vartheta(t+\Delta_t))$;
...
\item Smooth complementarity constraints via Fischer--Burmeister (Eq.~\ref{eq:iam_fb}).
\item Form the loss as sum of squared residuals over all 8 conditions (Eq.~\ref{eq:iam_loss}).
\item Train as in the stationary DEQN: simulate $\to$ record loss $\to$ backprop $\to$ repeat.
\end{enumerate}
\end{keyinsightbox}

\subsection{Stochastic Extension: AR(1) Productivity and Cai--Lontzek-Style Risk}
\label{sec:stochastic_dice}

The CDICE model derived above is deterministic: the only forces that move the economy are deterministic exogenous trends in TFP, population, abatement cost, and the carbon-intensity of output.  Real climate--economy interactions are of course shot through with stochastic shocks: to productivity, to the equilibrium climate sensitivity, to tipping-point thresholds.  \citet{caiSocialCostCarbon2019} show that adding a stochastic productivity shock and a tipping-point hazard to a DICE-class model can shift the SCC by tens of percent, with the bulk of the effect driven by the right tail of the temperature distribution.  This subsection sketches the minimal stochastic extension, adding an AR(1) shock to TFP, that already lets us reproduce the qualitative SCC fan-chart structure of \citeauthor{caiSocialCostCarbon2019} on a laptop.  The exposition follows the implementation in companion notebook \texttt{03\_Stochastic\_DICE\_DEQN}.

\paragraph{Augmented state.}
Let $z_t$ denote the deviation of log TFP from its deterministic trend, evolving as
\begin{equation}
z_{t+1} = \rho_z\, z_t + \sigma_z\, \varepsilon_{t+1}, \qquad \varepsilon_{t+1} \overset{\mathrm{i.i.d.}}{\sim} \mathcal{N}(0,1),
\label{eq:tfp_ar1}
\end{equation}
so that effective TFP is $A_t \exp(z_t)$ with $A_t$ the deterministic CDICE trend.  The state vector~\eqref{eq:iam_state} is augmented with $z_t$,
\begin{equation}
...
\end{equation}
and the policy network now reads $u_t = \mathcal{N}_\rho(\tilde{\bm{x}}_t)$.

\paragraph{Conditional FOCs.}
Each forward-looking FOC derived in Section~\ref{sec:dice_deqn} now carries a conditional expectation $\mathbb{E}_t[\cdot]$ over the next-period innovation $\varepsilon_{t+1}$.  Schematically the capital Euler~\eqref{eq:iam_foc_k} becomes
\begin{equation}
\begin{aligned}
e^{g^A_t + g^L_t}\,\hat{\lambda}_t \;=\; \mathbb{E}_t\Bigl[\;
&\hat{\beta}_t\,\hat{\lambda}_{t+1}\bigl(\Omega(T^{\mathrm{AT}}_{t+1})(1-\Theta(\mu_{t+1}))\,\alpha\,k_{t+1}^{\alpha-1} + (1-\delta)\bigr) \\
&\;+\; \hat{\nu}^{\mathrm{AT}}_{t+1}\,\sigma_{t+1}(1-\mu_{t+1})\,A_{t+1}L_{t+1}\,\alpha\,k_{t+1}^{\alpha-1}\,\Bigr],
\end{aligned}
\label{eq:iam_euler_stoch}
\end{equation}
and the five climate-state envelope conditions are recast in the same way.  The Fischer--Burmeister complementarity~\eqref{eq:iam_fb} on the abatement bound is unaffected because $\mu_t$ is a date-$t$ control.

\paragraph{Evaluating the expectation: Gauss--Hermite quadrature.}
Because $\varepsilon_{t+1}$ is Gaussian, the conditional expectation in~\eqref{eq:iam_euler_stoch} can be evaluated to high accuracy with a small number of Gauss--Hermite (GH) nodes.  Letting $\{(\xi_q, w_q)\}_{q=1}^{Q}$ denote the Gauss--Hermite nodes and weights for $\int e^{-\xi^2} f(\xi)\, \mathrm{d}\xi \approx \sum_q w_q f(\xi_q)$, the change of variables $\varepsilon = \sqrt{2}\,\xi$ gives
\begin{equation}
\mathbb{E}_t[f(\varepsilon_{t+1})] \;=\; \frac{1}{\sqrt{\pi}}\int_{-\infty}^\infty e^{-\xi^2} f(\sqrt{2}\,\xi)\,\mathrm{d}\xi \;\approx\; \frac{1}{\sqrt{\pi}}\sum_{q=1}^{Q} w_q\, f\bigl(\sqrt{2}\,\xi_q\bigr).
\label{eq:gh_quadrature}
\end{equation}
Each forward-looking residual $l_m$ is therefore replaced by
\begin{equation}
...
\label{eq:loss_gh}
\end{equation}
and the total DEQN loss aggregates these as in~\eqref{eq:iam_loss}.  In practice $Q=5$ nodes already drive the quadrature error well below the training-noise floor for this problem.  The GH evaluation is fully differentiable, so the entire pipeline still runs as a single autodiff backward pass over the mini-batch.

\paragraph{Reading the SCC distribution.}
Once the network is trained, forward-simulating $N_{\mathrm{MC}}$ trajectories of the AR(1) shock yields a Monte-Carlo distribution of every quantity of interest, including temperature paths, abatement effort, and the SCC, conditional on the same calibrated parameters.  The companion notebook~\texttt{03\_Stochastic\_DICE\_DEQN} is designed to produce the resulting fan charts and to show the same qualitative mechanism as \citet{caiSocialCostCarbon2019}: stochastic risk mostly matters through the right tail of the temperature and SCC distributions.  This is the entry point to the deep uncertainty quantification of Section~\ref{sec:deep_uq}, where uncertainty about \emph{parameters} is also pushed into the network as an additional pseudo-state.

\paragraph{Companion notebooks.}
The full computational pipeline of this section is in the Day~8 code directory.  The intended order is:
\begin{itemize}[itemsep=2pt,leftmargin=*]
\item \tpath{01_Climate_Exercise}: NumPy-only warm-up on the carbon cycle and damage module; no neural networks.
\item \tpath{02_DICE_DEQN_Library_Port}: deterministic CDICE-DEQN for Sections~\ref{sec:dice}--\ref{sec:dice_deqn}, including a verification gate against the reference solution.
\item \tpath{03_Stochastic_DICE_DEQN}: AR(1) stochastic extension from Section~\ref{sec:stochastic_dice}.
\item \tpath{04_DICE_DEQN_EpsteinZin_Weather}: recursive-preference extension with i.i.d.\ atmospheric weather shocks.
\item \tpath{05_DICE_DEQN_EpsteinZin}: deterministic Epstein--Zin walkthrough used in the slide deck.
\end{itemize}

\section{Deep Uncertainty Quantification}
\label{sec:deep_uq}

The SCC depends sensitively on parameters that are deeply uncertain: the equilibrium climate sensitivity (ECS), the curvature of the damage function, the pure rate of time preference, and the risk aversion coefficient.  Traditional uncertainty analysis proceeds by fixing a parameter vector, solving the model, computing the SCC, and repeating for each new parameter draw, a brute-force approach that is computationally infeasible when each model solution is expensive.

\subsection{The Pseudo-State Approach}

The key insight, developed in \citet{friedlDeep2023}, is to treat uncertain parameters as \emph{pseudo-states}: additional inputs to the policy network that are held fixed within each simulation episode but vary across episodes.  The augmented state vector becomes
\begin{equation}
\tilde{\bm{x}}_t = \bigl(\underbrace{\bm{x}_t}_{\text{economic + climate states}},\; \underbrace{\theta}_{\text{uncertain parameters}}\bigr),
\qquad u_t = \mathcal{N}_\rho(\tilde{\bm{x}}_t).
\label{eq:pseudo_state}
\end{equation}
One model training approximates policy functions for \emph{all} parameter values simultaneously.  After training, evaluating the model at any new $\theta$ requires only a forward pass of the network; the expensive global solution is amortized.  For very large pseudo-state dimensions (multi-region IAMs, sectoral detail), the active-subspace methods of Section~\ref{sec:active_subspaces} compress $\theta \mapsto \tilde\theta \in \mathbb{R}^m$ before the surrogate is fit; this is the natural composition of the Day~7 GP toolkit (Chapter~\ref{ch:gp}) with the Day~8 climate stack.

\subsection{GP Surrogates for Quantities of Interest}

While the DEQN provides a fast simulator, constructing a smooth mapping from the full parameter space to scalar quantities of interest (the SCC, welfare, temperature paths) benefits from an additional surrogate layer.  We fit a Gaussian process (GP):
\begin{equation}
q(\cdot) \sim \mathcal{GP}\bigl(m(\cdot),\, k(\cdot, \cdot)\bigr),
\end{equation}
where $q$ is any scalar output of interest (e.g., SCC at a given date).  Training data for the GP are generated by sampling parameter vectors, simulating the DEQN, and recording the output.  The GP provides both a predictive mean (fast approximation) and a predictive variance (uncertainty-aware), the latter being essential for guiding adaptive sampling.

It is important to distinguish three sources of uncertainty in this pipeline: (i) \emph{parameter uncertainty}, propagated through the sampled $\theta$; (ii) \emph{stochastic uncertainty} from the underlying climate and economic shocks, captured by the DEQN's expectation operators; and (iii) \emph{surrogate epistemic uncertainty}, which is what the GP posterior variance measures, conditional on the simulator and chosen kernel.  The GP variance alone does not quantify total climate-economy uncertainty.

\subsection{Bayesian Active Learning}

Rather than sampling parameter vectors on a fixed grid or by random Latin hypercube, we employ \emph{Bayesian active learning} (BAL): at each iteration, the next parameter vector is chosen where the GP predictive variance is highest (exploration) or where the relevant quantity of interest or policy objective is most informative (exploitation).  BAL converges to stable sensitivity conclusions with far fewer expensive model evaluations than naive sampling.

\subsection{Global Sensitivity Analysis}

With the GP surrogate in hand, global sensitivity analysis becomes computationally inexpensive.  The two standard decompositions are:

\paragraph{Sobol indices.}
The first-order Sobol index of parameter $\theta_i$ is the share of output variance explained by $\theta_i$ alone \citep{sobol1967distribution, saltelli2008global}:
\begin{equation}
S_i = \frac{\mathrm{Var}\bigl(\mathbb{E}[q(\theta) \mid \theta_i]\bigr)}{\mathrm{Var}(q(\theta))}.
\label{eq:sobol_first}
\end{equation}
For independent inputs, the first-order indices $\{S_i\}$ sum to at most one.  The total-effect index captures both direct and interaction effects:
\begin{equation}
S_i^{\mathrm{tot}} = 1 - \frac{\mathrm{Var}\bigl(\mathbb{E}[q(\theta) \mid \theta_{-i}]\bigr)}{\mathrm{Var}(q(\theta))}.
\label{eq:sobol_total}
\end{equation}
Total-effect indices can sum to more than one when interactions are present, because each interaction is counted in every contributing variable's total effect; equality $\sum_i S_i^{\mathrm{tot}} = 1$ holds if and only if the model is purely additive (no interaction terms).

\paragraph{Worked example.}
For $q(\theta_1, \theta_2) = \theta_1^2 + \theta_1\theta_2$ with $\theta_i \sim \mathcal{U}(0,1)$ i.i.d., direct calculation gives $\mathrm{Var}(q)=53/240$,
\[
S_1=\frac{139}{159}\approx 0.874,\qquad
S_2=\frac{5}{53}\approx 0.094.
\]
The corresponding total-effect indices are
\[
S_1^{\mathrm{tot}}=\frac{48}{53}\approx 0.906,\qquad
S_2^{\mathrm{tot}}=\frac{20}{159}\approx 0.126,
\]
so $\sum_i S_i^{\mathrm{tot}}=164/159>1$.  The interaction term $\theta_1\theta_2$ contributes to both totals, signalling that $\theta_1$ and $\theta_2$ jointly matter even though the first-order indices sum to less than one.

\paragraph{Shapley effects.}
For a clean additive variance decomposition under interactions, \citet{owen2014sobol} introduced \emph{Shapley effects}, which allocate $\mathrm{Var}(q)$ across parameters by averaging each parameter's marginal contribution over all possible subsets of other parameters, following the cooperative game theory axioms of \citet{shapley1953value}.  Raw Shapley values sum exactly to $\mathrm{Var}(q)$; normalized Shapley effects sum to one.  Together, Sobol indices and Shapley effects give a robust ranking of the drivers of SCC dispersion.

\paragraph{Results.}
\citet{friedlDeep2023} find that ECS uncertainty and the damage function specification dominate SCC variability, with the pure rate of time preference playing a secondary but nontrivial role.  The SCC distribution is wide and right-skewed, and Bayesian learning shifts mass away from extreme tails over time as the planner accumulates temperature observations.  These findings underscore that climate policy design should target tail insurance, not only mean outcomes.

\section{Constrained Pareto-Improving Carbon Tax Rules}
\label{sec:carbon_tax}

The representative-agent IAM of Sections~\ref{sec:dice}--\ref{sec:deep_uq} assumes a benevolent planner who aggregates welfare across time.  In reality, climate policy creates winners and losers: current generations bear abatement costs while future generations enjoy a cleaner climate.  This section extends the analysis to an OLG setting, following \citet{kotlikoff2021making} and \citet{kubler2025using}; the Pareto-improvement criterion is closely related to the social-security reform literature \citep{krueger2006pareto} and to recent work on intergenerational climate policy \citep{Karp_Peri_Rezai_2024}.

\subsection{The OLG-IAM Model}
\label{sec:olg_iam}

The model features 12 overlapping generations of selfish agents (ages 20--80 in 5-year periods), a competitive firm, and a simplified climate module based on \citet{dietz2019cumulative}:
\begin{itemize}[itemsep=2pt]
\item \textbf{Technology:} Output is $Y_t = \Omega_t(T_t)\,\Phi(\mu_t)\,K_t^\alpha L_t^{1-\alpha}$ with retained-output damage factor $\Omega_t$ and net-of-abatement-cost factor $\Phi(\mu_t) = 1 - \theta_1\mu_t^{\theta_2}$; the period resource constraint is $C_t + K_{t+1} = Y_t + (1-\delta)K_t$.
\item \textbf{Households:} Each agent maximizes $\mathbb{E}_t\sum_{j=1}^{12}\beta^{j-1}\, C_{t+j-1,j}^{1-\sigma_u}/(1-\sigma_u)$ (where $\sigma_u$ is the household CRRA risk-aversion coefficient, distinct from the climate-chapter notation $\sigma_t$ for emissions intensity) subject to the budget constraint $C_{t,j} + a_{t+1,j+1} = (1+r_t)\,a_{t,j} + w_t\,l_j + \mathbb{T}_{t,j}$, where $\mathbb{T}_{t,j}$ is the transfer from carbon tax revenue.
\item \textbf{Climate:} A near-linear relationship between cumulative emissions and temperature, $T^{\mathrm{AT}}_t \approx \sigma_{\mathrm{CCR}}\sum_{s=0}^t E_s$, with a stochastic tipping-point threshold $TP_t$.
\item \textbf{Stochastic shocks:} Carbon intensity $\kappa_t$ follows an AR(1) with time-varying persistence; the tipping parameter $TP_t$ follows a bounded random walk.
\end{itemize}
The household Euler equation takes the standard form $C_{t,j}^{-\sigma_u} = \beta\,\mathbb{E}_t[(1+r_{t+1})\,C_{t+1,j+1}^{-\sigma_u}]$ for $j = 1,\ldots,11$, and market clearing requires that aggregate savings equal the capital stock: $\sum_j a_{t,j} = K_t$.
...
The tax rule parameters $\vartheta = (\vartheta_0, \vartheta_E, \vartheta_\kappa, \vartheta_{TP})$ and the 11 transfer shares enter the neural network as pseudo-states alongside the economic and climate states.  The DEQN learns the equilibrium for \emph{all} tax rules at once, so that simulating any tax configuration requires only a forward pass.

\paragraph{Step 2: GP surrogate.}
For each of 500 design points (450 Latin hypercube + 50 Bayesian active learning), the DEQN is simulated to compute welfare for each of the 40 cohorts (11 current, 29 future).  A GP surrogate maps $\vartheta \mapsto \mathbb{E}[\mathcal{W}(\vartheta)]$ and $\vartheta \mapsto \tilde{U}_t(\vartheta)$ for each cohort.
...
\label{eq:pareto_opt}
\end{equation}
where $U_t$ is the business-as-usual (BAU) welfare of cohort $t$ and the Pareto constraint ensures that \emph{no generation is worse off}.  Because the optimization runs on the GP surrogate (milliseconds per evaluation), the full constrained policy search takes minutes rather than weeks.

\subsection{Results}

Three policy scenarios illustrate the power of the approach:
\begin{enumerate}[itemsep=2pt]
\item \textbf{Unconstrained welfare maximum with exogenous transfers:}  a linear tax rule $\tau_t = \vartheta_0 + \vartheta_E \cdot E_t$ with a fixed declining transfer scheme.  This achieves strong emission reduction but creates losers among current cohorts (welfare losses up to $\sim$5\% for transition generations).
\item \textbf{Pareto-improving with optimized transfers:}  the same tax rule with endogenous transfer shares.  The Pareto constraint binds for current generations, but all cohorts are weakly better off than under BAU.
\item \textbf{Richer tax function:}  $\tau_t = \vartheta_0 + \vartheta_E\,E_t + \vartheta_\kappa\,\kappa_t + \vartheta_{TP}(1 - D_{TP})$, where the tax responds to carbon intensity and tipping proximity, providing additional tipping-risk insurance.
\end{enumerate}

\begin{keyinsightbox}[Pareto improvement through tail insurance]
The main welfare channel of constrained optimal policy is \emph{disaster-risk reduction} (tail insurance), not maximal average abatement.  Transfers resolve the intergenerational conflict, and even simple linear rules capture most of the achievable welfare gain.  Richer rules add further tipping-risk insurance.
\end{keyinsightbox}
...
\item DICE (and the CDICE recalibration) provide the workhorse model; ACE provides an analytic benchmark that sanity-checks the numerical SCC.
\item Pareto-improving carbon-tax rules \citep{kotlikoff2021making, kubler2025using} demonstrate that the surrogate machinery has direct policy relevance: linear, intergenerationally fair, implementable rules can be designed via constrained optimization on the surrogate.
...
\item \citet{kotlikoff2021making, kubler2025using}, Pareto-improving carbon-tax design.
...
The methods presented in this course, namely DEQNs, PINNs, deep surrogates, and GP + BAL, share a common computational paradigm that can be summarized as:
...
\begin{tikzpicture}[
    ...
    \node[box] (nn) at (0,0) {\textbf{Neural network}\\as function\\approximator};
    \node[box] (phys) at (5,0) {\textbf{Economics}\\in the\\loss function};
    \node[box] (ad) at (10,0) {\textbf{Automatic}\\differentiation\\for training};
    \draw[arr] (nn) -- (phys);
    \draw[arr] (phys) -- (ad);
\end{tikzpicture}
\caption{The unifying paradigm shared by the four methods of this course (DEQNs, PINNs, deep surrogates, and GP + BAL): a neural network plays the role of a flexible function approximator, the economic or financial structure (equilibrium conditions, PDE residuals, or simulated targets) enters directly through the loss function, and automatic differentiation supplies the derivatives needed for both the loss itself and for gradient-based optimization.}
\label{fig:unifying_paradigm}
\end{figure}

Figure~\ref{fig:unifying_paradigm} makes explicit the common template.  In each case, the structural knowledge of the economic or financial model is embedded directly in the loss function rather than in labeled training data.  The network learns by minimizing the violation of economic equilibrium conditions (DEQNs), PDE residuals (PINNs), or approximation error on model-generated data (surrogates).  Automatic differentiation provides the gradients needed for efficient optimization, and the mesh-free nature of neural networks avoids explicit tensor-product state grids and so mitigates the practical curse of dimensionality, though it does not eliminate the underlying high-dimensional sample-complexity, approximation, optimization, and integration costs.
...
Key advantage & Scales to $d>100$ & Exact AD derivatives of the network & Instant re-evaluation & Built-in UQ + active learning \\
...
\noindent A few practical takeaways from the table.  \textbf{DEQNs} are the workhorse for \emph{discrete-time} general-equilibrium models with high-dimensional state spaces, where Euler equations and market-clearing residuals admit a clean residual-loss formulation.  \textbf{PINNs} are the natural analogue in \emph{continuous time}, replacing finite-difference HJB or BSDE solvers when one needs exact derivatives of the solution by automatic differentiation rather than by interpolation.  \textbf{Deep surrogates} differ in kind: they do not solve the model; they learn a fast emulator of the already-solved mapping from parameters to economic quantities, which is what makes structural estimation, sensitivity analysis, and policy design tractable at scale.  \textbf{Gaussian processes with Bayesian active learning} occupy the small-data, high-uncertainty corner: the right tool when each evaluation is expensive, the input dimension is moderate ($d \lesssim 50$ with active subspaces), and posterior uncertainty is itself part of the answer.  \textbf{EMINNs} (Chapter~\ref{ch:ct_theory}) are a specialization of the PINN paradigm to continuous-time heterogeneous-agent master equations: they keep the PDE-residual training logic but encode the cross-sectional distribution itself as part of the network input, which is currently the only feasible route to global solutions of Krusell--Smith-style models with aggregate shocks.
...
\section{Running Case: Brock--Mirman Through Every Lens}
\label{sec:running_case_bm}

The Brock--Mirman model has accompanied us from Chapter~\ref{ch:deqn} onward as the simplest economically meaningful test case.  Table~\ref{tab:bm_running_case} shows the same problem viewed through the four lenses developed in this script; reading across rows, one sees both what changes and what stays invariant when the methodology shifts.
...
PINN, cake-eating          & Value function $\hat V(a)$ on a 1D interval        & HJB residual $\rho V-\max_c[u(c)+V'(a)(ra-c)]$         & $\hat V$ matches closed form $V^\star$            & \tpath{04_Cake_Eating_HJB_PINN.ipynb}       \\
GP $+$ BAL                 & $\hat V(K,z,\theta)$ on irregular ergodic set       & GP marginal likelihood; BAL picks next $K_*$           & 95\% pred.\ band covers true $V$                  & \tpath{04_GP_Value_Function_Iteration.ipynb}             \\
SMM $+$ surrogate          & Policy $s(K,z,\beta)$ pseudo-state on $\beta$       & SMM moment objective in $\beta$                        & $\hat\beta$ within MCSE of truth                  & \tpath{03_Structural_Estimation_BM.ipynb} \\
\bottomrule
\end{tabular}
\caption{The Brock--Mirman model, treated four ways.  The economic content is identical (a stochastic neoclassical growth model with log utility and Cobb--Douglas production); the methodology determines what object is approximated, what residual is minimized, and what diagnostic certifies convergence.  Reading the table top to bottom is essentially a tour of this script.}
\label{tab:bm_running_case}
\end{table}

The takeaway is that no single methodology is the ``right'' one for Brock--Mirman.  Rather, the four lenses answer different research questions: a DEQN gives the policy function, a PINN gives the value function with exact AD derivatives, a GP gives uncertainty-quantified value-function estimates with guidance for the next sample point, and the surrogate-SMM pipeline estimates the deep parameter $\beta$ from data.  Real research applications typically combine at least two of these, and the implementation guidance throughout the chapters is designed to make those combinations cheap.

\section{A Unified View of the Course Methods}

Figure~\ref{fig:unified_view_methods} summarizes how the four main methods covered in this course relate to one another.  All share the core principle of embedding structural knowledge in the loss function and using neural networks as flexible function approximators:
...
\begin{tikzpicture}[
    ...
    linklabel/.style={fill=white, draw=gray!65, rounded corners=2pt,
                      inner sep=3pt, align=center, font=\small\bfseries, text=black}
]
    % Core principle
    \node[core] (core) at (6,4.5) {Neural Network $+$ Structure-in-Loss $+$ Autograd};
...
\node[method=softorange] (surr) at (8,0) {\textbf{Deep surrogate}\\(Ch.~\ref{ch:gp}--\ref{ch:estimation})\\[3pt]\scriptsize Parameters as inputs\\Instant re-evaluation\\Estimation, UQ};
    ...
    % Cross-connections
    \draw[{Stealth[length=2.2mm]}-{Stealth[length=2.2mm]}, dashed, gray!75, very thick]
...
\end{tikzpicture}
\caption{Unified view of the four main methods covered in the course, and how they relate to the common core principle of using a neural network as a function approximator with the economic structure embedded in the loss function and gradients computed by automatic differentiation. DEQNs (blue) handle discrete-time Euler equations in high-dimensional state spaces; PINNs (red) handle continuous-time HJB, Kolmogorov forward, and Black--Scholes PDEs through PDE residuals; deep surrogates (orange) make model parameters part of the input space and replace the expensive solve with an instant forward pass for estimation and uncertainty quantification; GP + BAL (green) layers calibrated uncertainty and sample-efficient active learning on top of the surrogate. The dashed grey arrows highlight the two main bridges between methods: discrete- versus continuous-time formulations on the left, and the deep-surrogate plus uncertainty-layer pairing on the right.}
...
\item \textbf{Heterogeneous-agent models and beyond.}  Extending the DEQN and PINN frameworks to heterogeneous-agent models with aggregate shocks, where the cross-sectional distribution of agents is itself a state variable, is a major frontier.  \citet{han2023deepham} propose DeepHAM, which represents the distribution via optimal generalized moments and uses deep neural networks to approximate value and policy functions globally.  \citet{payne2024deepsam} apply deep learning to search-and-matching models with heterogeneity.  These approaches demonstrate that deep learning can tackle the infinite-dimensional state spaces inherent in heterogeneous-agent economies.
...
\begin{enumerate}[itemsep=4pt]
\item \textbf{The state space is genuinely low-dimensional.}  For models with $d \lesssim 5$ continuous state variables and smooth policies, classical methods, projection on Chebyshev polynomials, value-function iteration on a fine grid, or perturbation around the steady state, typically deliver higher accuracy at lower cost than a DEQN \citep{judd1998numerical}.  The curse of dimensionality is real but it bites at $d > 10$, not at $d = 3$.
...
\item \textbf{Sample size is small and uncertainty is the answer.}  At $n \lesssim 10^3$ design points, GPs with active learning (Chapter~\ref{ch:gp}) provide both better point predictions and calibrated uncertainty than a deep network of comparable complexity.  Default to GPs when the simulator is expensive and posterior uncertainty matters.

\item \textbf{Reproducibility and audit-ability are paramount.}  Deep-learning solutions depend on initialization seeds, optimizer tuning, and floating-point order of operations on the GPU.  In central-bank or regulatory contexts where bit-exact reproducibility across machines is required, a deterministic finite-difference scheme remains preferable.  At minimum, pin every random seed and report hardware/software versions (cf.\ the reproducibility appendix).
...
\item \textbf{Use common random numbers (CRN).}  When comparing model outputs across parameter values (e.g., in surrogate-based estimation), always fix the shock sequence.  CRN can reduce variance by several orders of magnitude, making gradient-based optimization feasible.
\end{enumerate}
...
The methods presented in this course are not merely computational tricks; they represent a fundamental shift in how we approach quantitative economic modeling.  By embedding economic structure directly in the loss function and leveraging the universal approximation capability of neural networks, we can solve models that were previously beyond reach, estimate their parameters from rich datasets, and conduct policy analysis in near-real time.

The convergence of abundant compute, mature software ecosystems (TensorFlow, PyTorch), and increasingly complex economic questions ensures that deep learning methods will play an expanding role in economic research and central bank practice in the years ahead.  We hope that this course has provided the conceptual foundations and practical tools needed to engage with this frontier.

For further reading, we refer to the comprehensive survey by \citet{fernandezvillaverde2024taming}, the methodological foundations laid in \citet{azinovicDEEPEQUILIBRIUMNETS2022}, and the applications in \citet{rennerscheidegger_2018}, \citet{friedlDeep2023}, \citet{han2023deepham}, \citet{payne2024deepsam}, \citet{chen2025Deep}, and \citet{kubler2025using}.  The list of references in these notes is necessarily incomplete; for a full bibliography, we refer the reader to the cited papers and the references therein.
...
\item Knowing when \emph{not} to use deep learning is part of using it well: classical projection or perturbation often dominates in low dimension; exact likelihood beats simulation when both are available; GPs win at small $n$.
...
\addcontentsline{toc}{section}{Exercises}
\begin{enumerate}[itemsep=4pt, leftmargin=*]
\item\label{ex:ch12:1} \textbf{Method-choice scenario.}  You are handed (a) a 4-state monetary-policy DSGE with smooth shocks, (b) a 200-agent OLG with progressive taxation, (c) an option-pricing problem on an irregularly shaped exotic payoff, and (d) a climate-IAM where uncertainty in the SCC is the deliverable.  Justify which method you would use for each, and what hybrid you would consider.
\item\label{ex:ch12:2} \textbf{When NOT to use deep learning.}  Pick one of the six regimes from \S\ref{sec:when_not_to_use_dl} and write a one-page note for a colleague explaining why a classical method dominates.
\item\label{ex:ch12:3} \textbf{Reproducibility audit.}  Take any notebook from this script and document hardware, software versions, and seeds.  Re-run end-to-end and report the maximum deviation in any reported number.
\item\label{ex:ch12:4} \textbf{Open-ended.}  Identify one frontier from \S\ref{sec:open_challenges} (operator learning, hybrid global/local, real-time policy, large structural estimation, climate-economy IAMs, HA with aggregate shocks) and sketch a 6-month research project that combines two methods from this script.
\item\label{ex:ch12:5} \textbf{Hybrid pipeline: DEQN $+$ GP $+$ SMM end-to-end.}  Build a complete estimation pipeline on the Brock--Mirman model with two parameters $(\beta, \alpha)$.
\begin{enumerate}[label=(\roman*), itemsep=2pt]
\item Train a DEQN with \emph{pseudo-state augmentation}: extend the network input from $(K_t, z_t)$ to $(K_t, z_t, \beta, \alpha)$.
\item On a $20\times 20$ grid of $(\beta, \alpha) \in [0.94, 0.99] \times [0.30, 0.40]$, simulate the model and compute four moments: mean savings rate, mean consumption growth, $\mathrm{Var}(\log C_t)$, and $\mathrm{Var}(K_t)$.
\item Fit a GP surrogate $(\beta, \alpha) \mapsto \bm m$ on the resulting $400$-point dataset.
\item Generate observed moments at $(\beta^\star,\alpha^\star)=(0.96,0.36)$ from one $T=200$ simulation and solve the GP-based SMM problem.
\end{enumerate}
Report the wall-clock time of each step, total pipeline time, estimation errors, and a comparison to a naive SMM that re-solves the DEQN at each candidate $(\beta,\alpha)$.  Evaluate whether the surrogate-based optimization is faster after accounting for the one-time GP fitting overhead and whether its accuracy is comparable.  Bonus: bootstrap confidence intervals for $(\hat\beta,\hat\alpha)$ using the parametric bootstrap of Exercise~\ref{ex:ch10:6}.
\item\label{ex:ch12:6} \textbf{DeepONet for a parameterized HJB.}  Consider the cake-eating HJB
\[
\rho V(a; \gamma) = \max_c \left[\frac{c^{1-\gamma}}{1-\gamma} - V'(a; \gamma)c\right],
...
\item[\textbf{Functional derivative.}]  $\delta V/\delta g$, the kernel of the Fr\'echet derivative of $V$ with respect to a function-valued argument $g$; appears in the master equation (Chapter~\ref{ch:ct_theory}).
...
This appendix collects the stochastic-calculus background needed for Chapters~\ref{ch:pinn}--\ref{ch:ct_theory}.  For a full treatment, see \citet{shreve2004stochasticii}.
...
\]
The differential algebra is $dt\cdot dt = dt\cdot dB_t = 0$, $dB_t \cdot dB_t = dt$.
...
$\tfrac{1}{T}\int_0^T \varphi(X_t)\,dt \xrightarrow{T\to\infty} \int \varphi\,d\pi$ for every bounded measurable $\varphi$.  In economic models with bounded state spaces and aperiodic dynamics, ergodicity is what justifies replacing population integrals by simulation-time averages in DEQN training (Chapter~\ref{ch:deqn}).

\chapter{Fixed Points, Contraction Mappings, and the Bellman Operator}
\label{app:fixed_points}

The classical numerical-economics toolkit rests on Banach's contraction-mapping theorem; this appendix recalls the statement and one short proof sketch for completeness.  A reference is \citet{stokey1989recursive}.
...
DEQNs do \emph{not} iterate a Bellman operator.  Instead, they apply gradient descent to a residual loss that vanishes at the equilibrium policy.  The contraction property therefore disappears as a tool for proving convergence, and theoretical guarantees come from neural-network optimization theory rather than fixed-point analysis (Chapter~\ref{ch:outlook}).
...
\midrule
Random seeds   & Each notebook fixes \tpath{numpy.random.seed(0)}, \tpath{torch.manual_seed(0)}, and \tpath{tf.random.set_seed(0)} at the top; JAX-based notebooks additionally pin \tpath{jax.random.PRNGKey(0)}.  Per-cell seeds are documented inline. \\
...
Software stack  & Python $\geq 3.10$, TensorFlow $\geq 2.13$, PyTorch $\geq 2.0$, JAX $\geq 0.4.20$, GPyTorch $\geq 1.11$.  Pinned versions live in \tpath{requirements.txt} on the companion repository. \\
...
GPU determinism & \emph{Optional, bit-exact runs only}: set \tpath{CUBLAS_WORKSPACE_CONFIG=:4096:8} and \tpath{torch.use_deterministic_algorithms(True)}.  Default classroom and production runs leave these unset, accepting last-decimal nondeterminism in exchange for speed; small accuracy regression possible when the flags are on. \\
...
\paragraph{Exercise~\ref{ex:ch1:1} --- Backprop on a 2-layer net.}
...
\paragraph{Exercise~\ref{ex:ch1:2} --- MSE vs.\ MLE.}
...
\paragraph{Exercise~\ref{ex:ch1:3} --- Activation choice for a PINN.}
...
\paragraph{Exercise~\ref{ex:ch1:4} --- Adam vs.\ AdamW.}
...
\paragraph{Exercise~\ref{ex:ch1:5} --- RNN forward pass by hand.}
...
\]
Setting aside the bounded $\tanh'(\cdot) \in (0,1]$ factors, the chain has $T-1$ multiplicative copies of $W_h$.  With $\|W_h\|_2 = 0.5 < 1$, the gradient magnitude decays at least as fast as $0.5^{T-1}$, i.e.\ exponentially in the sequence length.  This is the canonical vanishing-gradient pathology of vanilla RNNs (\S\ref{sec:sequence_models}); LSTM/GRU gates and skip connections were introduced precisely to keep the eigenvalues of the recurrent Jacobian close to one without making training unstable.

\paragraph{Exercise~\ref{ex:ch1:6} --- Attention by hand.}
...
\paragraph{Exercise~\ref{ex:ch1:7} --- TensorBoard optimizer comparison.}
...
\paragraph{Exercise~\ref{ex:ch2:1} --- Closed-form Brock--Mirman.}
...
\paragraph{Exercise~\ref{ex:ch2:2} --- Hard vs.\ soft constraints.}
...
\paragraph{Exercise~\ref{ex:ch2:3} --- Path averaging vs.\ conditional expectation.}
...
\paragraph{Exercise~\ref{ex:ch2:4} --- Brock--Mirman with Gauss--Hermite.}
...
\paragraph{Exercise~\ref{ex:ch2:5} --- Monomial rule by hand (Stroud-3 at $d=4$).}
...
\paragraph{Exercise~\ref{ex:ch2:6} --- Loss-kernel selection.}
...
\paragraph{Exercises~\ref{ex:ch2:7} and~\ref{ex:ch2:8}.}
These are coding exercises (notebook \tpath{02_Brock_Mirman_Uncertainty_DEQN.ipynb}); reference outputs and timing curves are in the companion repository.

\section{Chapter \ref{ch:irbc} --- The International Real Business Cycle Model}
\label{sol:ch3}

\paragraph{Exercise~\ref{ex:ch3:1} --- Fischer--Burmeister.}
...
\paragraph{Exercise~\ref{ex:ch3:2} --- State-space scaling.}
...
\paragraph{Exercise~\ref{ex:ch3:3} --- Two-phase training.}
...
\paragraph{Exercise~\ref{ex:ch3:4} --- Adjustment-cost partials and Tobin's Q.}
...
\paragraph{Exercise~\ref{ex:ch3:5} --- Complete-markets risk sharing.}
...
\paragraph{Exercises~\ref{ex:ch3:6} and~\ref{ex:ch3:7}.}
These are coding exercises (notebook \texttt{05\_IRBC\_Exercise}); reference outputs and convergence curves are in the companion repository.

\section{Chapter \ref{ch:nas} --- Neural Architecture Search and Loss Normalization}
\label{sol:ch4}

\paragraph{Exercise~\ref{ex:ch4:1} --- Random vs.\ grid.}
...
\paragraph{Exercise~\ref{ex:ch4:2} --- Bayesian optimization toy problem.}
...
\paragraph{Exercise~\ref{ex:ch4:3} --- Hyperband budget allocation.}
...
\paragraph{Exercise~\ref{ex:ch4:4} --- Loss balancing.}
...
\paragraph{Exercise~\ref{ex:ch4:5} --- Pareto front geometry.}
...
\paragraph{Exercise~\ref{ex:ch4:6} --- ReLoBRaLo vs.\ GradNorm.}
...
\paragraph{Exercise~\ref{ex:ch4:7} --- HPO vs.\ full NAS decision.}
...
\paragraph{Exercise~\ref{ex:ch5:1} --- OLG market clearing for $A=3$.}
...
\paragraph{Exercise~\ref{ex:ch5:2} --- KKT under FB.}
...
\paragraph{Exercise~\ref{ex:ch5:3} --- Hump-shaped lifecycle.}
...
\paragraph{Exercise~\ref{ex:ch5:5} --- Bond pricing in equilibrium.}
...
\paragraph{Exercises~\ref{ex:ch5:6} and~\ref{ex:ch5:7}.}
...
\paragraph{Exercise~\ref{ex:ch6:1} --- Mean-preserving lottery.}
...
\paragraph{Exercise~\ref{ex:ch6:2} --- Closed-form bracketing on log-spaced grids.}
...
\paragraph{Exercise~\ref{ex:ch6:3} --- Approximate aggregation, scope.}
...
\paragraph{Exercise~\ref{ex:ch6:4} --- Sequence-space vs.\ histogram DEQN.}
...
\paragraph{Exercise~\ref{ex:ch6:5} --- DeepSets permutation invariance.}
...
\paragraph{Exercises~\ref{ex:ch6:6} and~\ref{ex:ch6:7}.}
...
\paragraph{Exercise~\ref{ex:ch7:1} --- Trial-function BC enforcement.}
...
\paragraph{Exercise~\ref{ex:ch7:2} --- ReLU pathology.}
...
\paragraph{Exercise~\ref{ex:ch7:3} --- Discrete $\to$ continuous bridge.}
Start with
\[
...
\paragraph{Exercises~\ref{ex:ch7:4}, \ref{ex:ch7:5}, \ref{ex:ch7:6}.}
...
\paragraph{Exercise~\ref{ex:ch7:7} --- Operator learning vs.\ PINN.}
...
\paragraph{Exercise~\ref{ex:ch8:1} --- Itô on GBM.}
...
\paragraph{Exercise~\ref{ex:ch8:2} --- KFE for an OU process.}
...
\paragraph{Exercise~\ref{ex:ch8:3} --- Functional derivative.}
...
\paragraph{Exercises~\ref{ex:ch8:6} and~\ref{ex:ch8:7}.}
...
\section{Chapter \ref{ch:gp} --- Deep Surrogate Models and Gaussian Processes}
\label{sol:ch9}

\paragraph{Exercise~\ref{ex:ch9:1} --- Posterior on three points.}
...
\paragraph{Exercise~\ref{ex:ch9:2} --- Marginal likelihood Occam.}
...
\paragraph{Exercise~\ref{ex:ch9:3} --- Active subspace by hand.}
...
\paragraph{Exercise~\ref{ex:ch9:4} --- Deep vs.\ linear active subspace.}
...
\paragraph{Exercise~\ref{ex:ch9:5} --- BAL on a 2D function.}
Coding exercise.  Pure-variance acquisition selects points purely by where the GP is most uncertain, ignoring the predicted mean.  The resulting design is usually more uniform across the input domain because the GP variance is high wherever data is sparse, regardless of function value.  Maximizing pure $\log\sigma^2(\x)$ gives exactly the same design as maximizing pure $\sigma^2(\x)$, since the logarithm is monotone.  Differences arise only once the acquisition is mixed with a mean term, for example $w_{\mathrm{exp}}\mu(\x)+\tfrac{w_{\mathrm{var}}}{2}\log\sigma^2(\x)$: then the design tilts toward regions that are both uncertain and high-valued under the current surrogate.  For global surrogate construction, pure variance is the cleaner baseline; for optimization-like goals, the mixed score may be useful.

\paragraph{Exercise~\ref{ex:ch9:6} --- Sobol sensitivity with a GP surrogate.}
...
\paragraph{Exercise~\ref{ex:ch9:7} --- GP overconfidence outside the convex hull.}
...
\emph{(iv)}~The danger: the GP returns a moderate (not infinite) posterior s.d.\ at $x = 3$, so naive uncertainty quantification reports a $\pm 2\sigma_f$ band that does \emph{not} reflect ignorance of the true function's behaviour outside the training hull.  In particular, the posterior does not know that $f$ continues oscillating; it just reverts to zero with a flat band.  Bayesian active learning algorithms that select points by maximum posterior variance can therefore miss extrapolation regions entirely.

Mitigations: use a Matérn kernel with $\nu = 1/2$ or $\nu = 3/2$ (heavier-tailed than RBF, posterior reverts to prior more slowly), incorporate a polynomial mean function in the GP prior (so extrapolation grows with $x$ instead of decaying), or use a boundary-aware acquisition function that explicitly penalises exploitation outside the convex hull.  In economic applications (e.g., extrapolating an estimated value function to wealth levels outside the training range), the safest practice is to flag any query outside the convex hull of training data and refuse to predict.

\section{Chapter \ref{ch:estimation} --- Structural Estimation via SMM}
\label{sol:ch10}

\paragraph{Exercise~\ref{ex:ch10:1} --- Identification.}
...
\paragraph{Exercise~\ref{ex:ch10:2} --- Optimal weighting.}
...
\paragraph{Exercise~\ref{ex:ch10:3} --- Common random numbers.}
...
\paragraph{Exercise~\ref{ex:ch10:4} --- SMM vs.\ SBI.}
...
\paragraph{Exercise~\ref{ex:ch10:5} --- $J$-statistic and overidentification.}
...
\paragraph{Exercise~\ref{ex:ch10:6} --- Bootstrap confidence intervals.}
...
\paragraph{Exercise~\ref{ex:ch10:7} --- ML vs.\ SMM efficiency.}
...
\paragraph{Exercise~\ref{ex:ch11:1} --- ECS sensitivity.}
...
\paragraph{Exercise~\ref{ex:ch11:2} --- Sobol decomposition.}
...
\paragraph{Exercise~\ref{ex:ch11:3} --- ACE benchmark.}
...
\paragraph{Exercises~\ref{ex:ch11:4}, \ref{ex:ch11:5}, \ref{ex:ch11:6}, \ref{ex:ch11:7}.}
Coding exercises.  Use the chapter discussion as qualitative guidance, but report the numerical answers from the notebook outputs because they depend on calibration choices, tolerances, and random seeds.

\paragraph{Exercise~\ref{ex:ch11:8} --- Tipping-point regime-switching damages.}
...
\paragraph{Exercise~\ref{ex:ch11:9} --- Real options value of waiting.}
...
\paragraph{Exercise~\ref{ex:ch12:2} --- When NOT to use deep learning.}
...
\paragraph{Exercise~\ref{ex:ch12:3} --- Reproducibility audit.}
Coding exercise.  Expected behavior: re-running notebook \tpath{02_Brock_Mirman_Uncertainty_DEQN} on the same machine with the same seeds should reproduce the reported diagnostics within the stated tolerance.  Bitwise equality of trained network parameters should be expected only when deterministic framework settings, hardware, BLAS/CUDA versions, and floating-point order of operations are all pinned.  Re-running on a different GPU, or with deterministic flags off, can produce small deviations in the last few printed digits of the savings-rate diagnostics while remaining well within the residual tolerance of the training.

\paragraph{Exercise~\ref{ex:ch12:4} --- Open-ended.}
...
\paragraph{Exercise~\ref{ex:ch12:5} --- Hybrid pipeline DEQN $+$ GP $+$ SMM.}
Coding exercise.  Illustrative outputs to benchmark against, not fixed targets:
\begin{itemize}[itemsep=2pt]
\item \emph{Step 1 (DEQN training).} Report the actual training time for the parameterized network.  Adding $(\beta,\alpha)$ as inputs increases the network's effective input dimension from $2$ to $4$, so convergence can be slower than in the single-calibration case.
...
\paragraph{Exercise~\ref{ex:ch12:6} --- DeepONet for a parameterized HJB.}
