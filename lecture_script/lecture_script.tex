...
\graphicspath{%
  {./}%
  {../assets/hero/}%
  {../lectures/lecture_03_B02_deep_equilibrium_nets/slides/figures/}%
  {../lectures/lecture_14_B12_surrogates_and_gps/slides/}%
  {../lectures/lecture_14_B12_surrogates_and_gps/slides/fig/}%
  {../lectures/lecture_15_B13_structural_estimation_smm/slides/fig/}%
}
\usepackage{bm}
...
\vfill
{\small Course materials: \url{https://github.com/sischei/Deep_Learning_for_Solving_And_Estimating_Dynamic_Economic_Models}\par}
\vspace{0.6cm}
{\footnotesize Last updated: April~2026\\[3pt]
\copyright{} 2026 Simon Scheidegger.  Code under MIT, text/figures under CC0 1.0 Universal:\\
...
\end{enumerate}
The unifying theme throughout is that \emph{economic structure} (equilibrium conditions, PDEs, Bellman equations) shapes how each method learns: in DEQNs, PINNs, and EMINNs the structure is embedded directly into a neural-network loss as residual terms, making the training objective itself unsupervised; in the surrogate and GP-based estimation chapters the structure instead appears upstream, in the model that generates the (input, output) training pairs the surrogate then learns in a standard supervised regression. Either way, the model's economic content drives the learning problem rather than being separated from it.

\paragraph{How these notes are organized.}
At a high level, Chapters~1--6 build the discrete-time toolkit (foundations, DEQNs, IRBC, architecture search and loss normalization, OLG, heterogeneous agents). Chapters~7--8 turn to continuous-time methods (PINNs, HJB--KFE for heterogeneous agents). Chapters~9--10 cover surrogate-based estimation and Gaussian processes. Chapter~11 applies the toolbox to climate economics under deep uncertainty, and Chapter~12 closes by comparing methods and highlighting open problems. The next chapter, \emph{How to Read This Script}, expands this overview into a full reading guide: cover-to-cover and selective paths, notation and cross-references, the role of the companion notebooks, and the visual conventions used throughout.
...
\paragraph{Cross-checking the autodiff loss.}  An important pedagogical point is that the autodiff residual~\eqref{eq:ad_euler} and the hand-derived residual of \S\ref{sec:bm} are the same mathematical object.  The companion autodiff notebooks implement both expressions on the same network and report the maximum absolute difference: $4.8\cdot 10^{-7}$ in the deterministic case and $8.1\cdot 10^{-6}$ in the stochastic case with Gauss--Hermite quadrature, both consistent with float32-level arithmetic, graph ordering, and quadrature accumulation.  Under full depreciation ($\delta = 1$) the trained policy from the autodiff loss matches the analytical closed-form $K_{t+1} = \alpha\beta K_t^{\alpha}$ (respectively $\alpha\beta z_t K_t^{\alpha}$) to mean relative error $\sim 10^{-4}$, confirming that minimizing the autodiff residual recovers the true policy when one is available.
...
\paragraph{Companion notebooks.}  Three notebooks (in the autodiff chapter's code folder) put the above into practice:
\begin{itemize}[itemsep=2pt]
...
Two NAS walkthroughs are provided alongside the ReLoBRaLo notebook.  All three live in the NAS chapter's code folder:
\begin{itemize}
...
Examples & OLG (Ch.~\ref{ch:olg}); \citet{maliarmaliarvalli2010} & \citet{krusell1998income}; \citet{young2010} \\
\bottomrule
\end{tabular}
...
\end{equation}
where $m_1 = \int k\, d\mu$ is mean capital and $a$ is the aggregate shock, achieves $R^2 > 0.9999$ in practice.
...
The companion agentic-programming workshop (taught as a separate hands-on module without an associated chapter in this script) introduces agentic programming as a tool for supervising and debugging research code at scale.  This chapter returns to numerical methods, this time applying neural networks to partial differential equations rather than equilibrium fixed-point conditions.  The PINN loss is structurally analogous to the DEQN loss of Chapter~\ref{ch:deqn}, but the equations are now in continuous time, which lets us solve a single PDE globally over the state space rather than time-step it.
...
The dynamic-programming perspective is classical in economics; see \citet{stokeylucas1989} for the standard textbook treatment.
...
\item \citet{kase2022estimating}, neural-network estimation of nonlinear heterogeneous-agent models; \citet{chen2025Deep}, deep surrogates for finance and option pricing.
...
\label{eq:scc}
\end{equation}
where $V_t$ is the value function, $E_t$ denotes a marginal pulse of contemporaneous emissions (interpreted as a one-unit shift in next-period atmospheric carbon $M^{\mathrm{AT}}_{t+1}$ via the carbon-cycle transition), and $C_t$ is consumption.  The closely related stock-based form $\mathrm{SCC}^M_t = -(\partial V_t/\partial M^{\mathrm{AT}}_t)/(\partial V_t/\partial C_t)$, derived in Section~\ref{sec:dice_deqn}, prices the existing carbon stock; the two definitions differ by the marginal loading of emissions into $M^{\mathrm{AT}}$ and the carbon-to-CO$_2$ unit conversion.  In a first-best allocation, the optimal carbon tax equals the SCC \citep{golosov2014optimal}.  The SCC is high when climate damages are steep, the climate response is strong, discounting is low, and tipping risks are material \citep{caiSocialCostCarbon2019, DIETZ20241}.
...
Following the convention in \citet{Folini_2021}, Online Appendix~D, we treat $\Omega(T_{\mathrm{AT}})$ as the \emph{damage fraction} of gross output (the fraction lost to climate damages, increasing in $T$), and $\Theta(\mu)$ as the \emph{abatement-cost fraction}.  Net (after-damages, after-abatement) output is therefore $\bigl(1 - \Omega(T_{\mathrm{AT}}) - \Theta(\mu)\bigr) K^\alpha (A L)^{1-\alpha}$.  This additive specification is what CDICE and the production-grade DEQN library port use; for an alternative multiplicative form $(1-\Omega^{\mathrm{ret}})(1-\Theta)$ with $\Omega^{\mathrm{ret}}$ a retained-output factor, see \citet{nordhaus2008question}.

\citet{nordhaus2008question}'s quadratic damage specification, in our convention, reads
\begin{equation}
...
\end{equation}
where $TP$ is a stochastic tipping-point threshold.  The degree of convexity of the damage function is one of the most important determinants of the optimal carbon tax.
...
\paragraph{How this section is organized (and why in this order).}
A reader new to DICE-style integrated assessment may find this section heavy.  It is layered deliberately so that each piece can be verified before the next one is added on top.  The spine of the section is the \emph{deterministic} CDICE benchmark: a single carbon-cycle calibration, a single deterministic productivity path, and an eight-residual DEQN loss derived from the Lagrangian.  Stochasticity, learning, and policy design are introduced afterward as small structural deltas on top of this spine.  Concretely:

\begin{enumerate}
  \item \textbf{Preferences.}  We replace CRRA with Epstein--Zin so that risk aversion ($\gamma$) and the intertemporal elasticity of substitution ($\psi$) can be set independently.  This is mandatory for credible long-horizon climate risk.
  \item \textbf{Nonstationarity.}  Trends are absorbed by detrending capital and consumption to per-effective-capita units, then time itself enters the network as a bounded state variable $\tau \in [0,1)$.  At this point the model is written in a form that the DEQN can train on.
  \item \textbf{State space.}  We list the inputs that the network actually sees, including a slot for pseudo-state parameters used later in deep uncertainty quantification.
  \item \textbf{Euler-equation derivation.}  The Lagrangian, the envelope conditions, and the Fischer--Burmeister complementarity slack are derived once, in the deterministic case.  Eight residuals fall out: capital Euler, budget, three carbon-stock envelopes, two temperature-layer envelopes, and one abatement bound.  These eight equations are the core of CDICE and follow Appendix~D.2 of \citet{Folini_2021}.
  \item \textbf{Bayesian learning over the climate-feedback parameter.}  A short extension that treats the posterior mean and variance of climate sensitivity as additional states.
  \item \textbf{DEQN implementation.}  The eight deterministic residuals are now wired into a single training loss.  The network is trained against the Nordhaus 2016 deterministic baseline (which has a known GAMS solution), and only \emph{then} is the deterministic CDICE-DEQN solver declared verified.  This deterministic anchor is what every later experiment sits on top of.
  \item \textbf{Stochastic extension.}  AR(1) productivity shocks and a Cai--Lontzek-style tipping risk are added.  Because the underlying loss kernel is the same eight residuals, the only structural changes are (a) the next-period state acquires an extra component $z_{t+1}$, (b) the capital and atmospheric-carbon residuals pick up an $\mathbb{E}_t[\cdot]$ via Gauss--Hermite quadrature, and (c) the discount factor becomes the Kreps--Porteus aggregator implied by Epstein--Zin.  No new equations need to be invented.
\end{enumerate}

The two later sections of the chapter then build on the verified stochastic CDICE solver: Section~\ref{sec:deep_uq} runs deep uncertainty quantification by treating uncertain calibration parameters as pseudo-states, and Section~\ref{sec:pareto_carbon_tax} designs Pareto-improving carbon-tax rules on top of an OLG variant.  A reader who only has time for the deterministic core can stop at the end of the DEQN-implementation subsection and still come away with a complete, working CDICE solver.
...
\textit{Convention.} The coefficients $b_{12}, b_{23}, c_1, c_3, c_4$ that appear below (and in the loss list further down) are \emph{per-period} coefficients of the calibrated CDICE tables, i.e.\ they already absorb the time step $\Delta_t$ that was visible in the raw carbon-cycle and temperature dynamics; they are \emph{not} continuous-time rates.  This is why no further $\Delta_t$ multipliers appear in the FOC residuals.
...
For each of 500 design points (450 Latin hypercube + 50 Bayesian active learning), the DEQN is simulated to compute welfare for each of the 40 tracked cohorts (12 alive at $t=0$ plus 28 future cohorts born during the policy horizon).  A GP surrogate maps $\vartheta \mapsto \mathbb{E}[\mathcal{W}(\vartheta)]$ and $\vartheta \mapsto \tilde{U}_t(\vartheta)$ for each cohort.
