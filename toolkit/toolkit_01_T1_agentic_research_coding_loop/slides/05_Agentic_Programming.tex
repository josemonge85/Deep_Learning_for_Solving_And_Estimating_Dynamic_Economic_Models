...
% ---------- Title ----------
\title[Toolkit T1]{Toolkit T1: Agentic Research-Coding Loop}
...
\begin{frame}{From the discrete-time DEQN block to this toolkit: From Models to the Workflow That Ships Them}
\small
\begin{columns}[T]
\column{0.50\textwidth}
\textbf{Days 1--4 built the models.}
\begin{itemize}\itemsep0pt
    \item Lectures 02--05: deep-learning fundamentals
    \item Lectures 06--10: Deep Equilibrium Nets (DEQNs)
    \item Lectures 11--12: IRBC, NAS, loss normalisation
    \item Lectures 10, 13--17: autodiff, sequence space, OLG
\end{itemize}

\vspace{0.3em}
{\scriptsize Each notebook ran for hours. Most of that time was \emph{not}
modelling: it was wiring data, debugging shapes, refactoring loss terms,
testing on toy problems, and rebuilding figures.}

\column{0.48\textwidth}
\textbf{This toolkit builds the \emphc{workflow}.}
\begin{itemize}\itemsep0pt
...
% --- Concrete use case ---
\begin{frame}{A Concrete Economics Workflow}
\small
\begin{columns}[T]
...
\column{0.48\textwidth}
\begin{enumerate}
    \item \textbf{Orientation \& Mental Models}\\
    {\scriptsize Why agentic AI matters; hierarchy of tools; context window.}
    \vspace{0.15em}
    \item \textbf{Production Environment Setup}\\
    {\scriptsize Terminal, Claude Code, Git, Python, MCP.}
    \vspace{0.15em}
    \item \textbf{The Core Interaction Loop}\\
    {\scriptsize Feedback loop; prompt quality; parallel sessions.}
    \vspace{0.15em}
    \item \textbf{Prompt Engineering for Research}\\
    {\scriptsize 6-component anatomy; templates; failure modes.}
\end{enumerate}

\column{0.48\textwidth}
\begin{itemize}
    \item[\textbf{5.}] \textbf{Project Memory \& CLAUDE.md}\\
    {\scriptsize Persistent context; self-improving memory; three-layer architecture.}
    \vspace{0.1em}
    \item[\textbf{6.}] \textbf{Skills, Subagents \& Hooks}\\
    {\scriptsize Custom commands; specialised agents; plan mode; hooks.}
    \vspace{0.1em}
    \item[\textbf{7.}] \textbf{Autonomous Loops with Ralph}\\
    {\scriptsize Wake-sleep pattern; \texttt{/ralph-loop} plugin; PRD-driven \texttt{ralph.sh}.}
    \vspace{0.1em}
    \item[\textbf{8.}] \textbf{From Data to Paper \& Replication}\\
    {\scriptsize Full pipeline; AI-assisted paper replication.}
    \vspace{0.1em}
    \item[\textbf{9.}] \textbf{Verification, Ethics \& Best Practices}\\
    {\scriptsize Failure modes; integrity; disclosure norms; Boris Cherny's 10 tips.}
\end{itemize}
\end{columns}
\end{frame}

% --- Roadmap ---
\begin{frame}{Roadmap (3-hour classroom slot)}
\scriptsize
...
\vspace{0.2em}
\textit{Coffee break (30 min)}

\vspace{0.2em}
\textbf{Block 2: Advanced \& Hands-On}~~(75 min, Parts V--IX)
\begin{itemize}\itemsep0pt
...
\vspace{0.3em}
This rewards \emphc{domain expertise}. Economists have a natural advantage over pure software engineers, because what is hard is knowing \emph{what question to ask}.
...
\begin{tikzpicture}[scale=0.7, every node/.style={font=\small}]
    ...
    \node[font=\tiny, text=uzhgreydark, align=center] at (0,0.1)
          {Your economics training\\is the competitive advantage};
\end{tikzpicture}
\end{center}
...
\scriptsize
\textbf{\textcolor{darkred}{Snapshot as of April 2026 \(\bullet\) semi-subjective \(\bullet\) re-check before re-using.}}\\
...
\toprule
\textbf{Task} & \textbf{ChatGPT} & \textbf{Claude} & \textbf{Gemini} \\
\midrule
Math \& analytical reasoning & \textcolor{darkgreen}{\checkmark\checkmark} & \checkmark & \checkmark \\
...
Long files / large context & \checkmark & \checkmark & \textcolor{softblue}{\checkmark\checkmark} \\
...
\toprule
\textbf{Level} & \textbf{Capability} & \textbf{Examples (April 2026)} \\
\midrule
...
\begin{columns}[T]
\column{0.48\textwidth}
An LLM has \emphc{no persistent memory}. Every message bundles the \textbf{entire conversation history}---your prompts, Claude's responses, every file read, every code output---into one large document sent to the model.
...
\vspace{0.1em}
{\tiny \boristip{1}~Boris Cherny: ``Context management is the single most critical resource.''}
\end{columns}
\end{frame}
...
\begin{itemize}\itemsep1pt
  ...
  \item Organises files, analyses spreadsheets, generates reports
...
\begin{enumerate}\itemsep1pt
    ...
    \item Copy \texttt{day5/code/generate\_synthetic\_data.py} and run it once
...
\vspace{0.3em}
{\scriptsize See \texttt{day5/code/exercise\_prompts.md} for copy-pasteable prompts.}
\end{columns}
\end{frame}
...
\begin{exampleblock}{\color{level4}\bfseries Turn 1 -- you}
\begin{lstlisting}[style=prompt, basicstyle=\tiny\ttfamily]
> What files are in this directory?
...
\begin{exampleblock}{\color{softgreen!80!black}\bfseries Turn 2 -- Claude}
\begin{lstlisting}[style=bash, basicstyle=\tiny\ttfamily]
...
\column{0.49\textwidth}
\begin{exampleblock}{\color{level4}\bfseries Turn 3 -- you}
\begin{lstlisting}[style=prompt, basicstyle=\tiny\ttfamily]
...
\begin{exampleblock}{\color{softgreen!80!black}\bfseries Turn 4 -- Claude}
\begin{lstlisting}[style=bash, basicstyle=\tiny\ttfamily]
...
\vspace{0.3em}
\begin{exampleblock}{\color{level4}\bfseries 1. You describe the task}
\begin{lstlisting}[style=prompt, basicstyle=\tiny\ttfamily]
...
\begin{exampleblock}{\color{softgreen!80!black}\bfseries 2. Claude reads, writes, runs}
\begin{lstlisting}[style=bash, basicstyle=\tiny\ttfamily]
...
\begin{exampleblock}{\color{softorange!80!black}\bfseries 3. You inspect, catch a bug, steer}
\begin{lstlisting}[style=prompt, basicstyle=\tiny\ttfamily]
...
\vspace{0.1em}
{\tiny Copy-pasteable version: \texttt{day5/code/CLAUDE\_md\_template.md}}
\end{columns}
\end{frame}
...
\scriptsize
From the course's \texttt{utils-agentic-support} framework (Scheidegger, 2026):
...
\begin{enumerate}\itemsep2pt
    ...
    \item Copy the template from \texttt{day5/code/CLAUDE\_md\_template.md}
...
\begin{frame}{Skills vs.\ Subagents vs.\ Hooks vs.\ MCP --- a Quick Map}
\scriptsize
Four extension mechanisms; each does one thing the others can't.

\vspace{0.3em}
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{p{1.6cm} p{3.7cm} p{2.9cm} p{3.4cm}}
\toprule
...
\emphc{Subagent} &
  Specialised assistant (own model, tools, prompt); spawned via \texttt{Task}. &
...
\vspace{0.2em}
{\scriptsize Example skill file: \texttt{day5/code/example\_skill/SKILL.md}}
\end{columns}
\end{frame}
...
\textbf{In course repo:} \texttt{day5/code/}\\
\phantom{xx}\texttt{example\_skill\_strategic\_revision/SKILL.md}
\end{columns}
...
\vspace{0.2em}
\textbf{7 ready-made examples in \texttt{day5/code/example\_subagent/}:}
...
\scriptsize
Three domain-specific subagents in \texttt{day5/code/example\_subagent/}.
...
\scriptsize
\boristip{11} \textbf{Hooks} are shell commands that run automatically before
...
\emphc{Full file:} \texttt{day5/code/example\_hooks/settings.json}.
\end{columns}
...
\begin{enumerate}\itemsep0pt
    \item Copy \texttt{example\_skill/SKILL.md} into \texttt{.claude/skills/data-diagnostics/}
    \item Copy \texttt{example\_subagent/verifier.md} into \texttt{.claude/agents/}
...
\textbf{Solutions:} \texttt{day5/code/exercise\_solutions.md}.
\end{frame}
...
\vspace{0.2em}
\boristip{10}~\textbf{Closed loops change the question.} You stop asking
...
\vspace{0.2em}
\emphc{Runnable reference:}~\selfstudy~\texttt{day5/code/mincer\_demo.py}
...
the two artifacts below (actual outputs of \texttt{day5/code/mincer\_demo.py}).
...
\small
Use \texttt{day5/code/generate\_synthetic\_data.py} to create data, then build a mini pipeline:
\begin{enumerate}\itemsep0pt
...
\begin{itemize}\itemsep0pt
    ...
    {\scriptsize From Days 1--4 notebooks}
\end{itemize}
...
\begin{itemize}\itemsep0pt
    ...
    \item[\textcolor{darkgreen}{\checkmark}] Hash / synthesise / aggregate first; send the derivative
...
\begin{enumerate}\itemsep1pt
    ...
    \item \textbf{AI is a fast second-year PhD student.} Excellent at known methods; mediocre at novel ones. Your judgement on identification, specification, and interpretation remains essential.
\end{enumerate}
\end{frame}
...
\begin{itemize}\itemsep0pt
    \item METR (2025). Early-2025 AI \& experienced OSS developer productivity --- found a 19\% \emph{slow-down}.
...
\begin{frame}{Next block --- PINNs \& Continuous-Time HA Models}
\small
\begin{columns}[T]
\column{0.55\textwidth}
...
\vspace{0.4em}
\textbf{The next block applies it.} The continuous-time block introduces:
\begin{itemize}\itemsep0pt
...
\column{0.43\textwidth}
\begin{block}{Suggested workflow for the continuous-time block}
\scriptsize
...
The continuous-time block swaps PDEs for the Bellman residual; the workflow stays the same.}
\end{columns}
\end{frame}
