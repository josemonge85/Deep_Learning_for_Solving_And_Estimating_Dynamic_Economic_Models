...
% ---------- Math commands ----------
\newcommand{\X}{\bm{X}}
\newcommand{\histE}{\mathcal{E}}  % shock-history symbol; reserve \mathbb{E} for expectations

% ---------- Title ----------
\title[Deep Learning for Dynamic Economic Models]
{Deep Learning for Economics and Finance}
\subtitle{Azinovic--Yang--Zemlicka: shock-history inputs for high-dimensional models}
...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Companion deck to \texttt{lecture\_09\_heterogeneous\_agents\_youngs}.}\\[4pt]
...
\begin{frame}[shrink=10]{Roadmap of This Lecture}
\small
...
\begin{block}{Notebooks for this lecture}
\texttt{05\_SequenceSpace\_BrockMirman.ipynb} -- the warm-up that isolates the history-to-policy logic.\\
\texttt{05b\_SequenceSpace\_IRBC.ipynb} -- bridge to the IRBC of Day~3 in sequence space.
\end{block}

\begin{block}{Heterogeneous-agent application -- self-study only}
Companion deck \texttt{lecture\_09\_heterogeneous\_agents\_youngs.pdf} (heterogeneous agents = self-study); reference notebook \texttt{06\_SequenceSpace\_KrusellSmith.ipynb} and the JAX implementation \texttt{KrusellSmith\_Tutorial\_CPU.ipynb}.
\end{block}
\end{frame}
...
\begin{itemize}
  \item Reformulate a state-space equilibrium as a truncated sequence-space equilibrium, mapping endogenous state into exogenous shock history
  \item Diagnose ergodicity and justify truncation length using decay rates of partial derivatives
  \item Implement residual minimization to train a neural-network policy on exogenous shock histories
  \item Compare state-space and sequence-space deep learning and recognize the stability advantages of exogenous inputs
  \item Build and validate a history-dependent policy network using the cloud method on Monte Carlo shock paths
\end{itemize}
\end{frame}
...
\[
x_t \;=\; \underbrace{\mathcal{H}}_{\substack{\text{iterated}\\ \text{law of motion}}}\!\bigl(\histE_t,\, x_0 \,\bigm|\, \Psi\bigr).
\]

\item \textbf{Equilibrium:} $\Psi$ satisfies an equivalent set of conditions
\[
\boxed{\;G(\Psi, \histE, x_0) \;=\; 0 \quad \forall \histE,\;\forall x_0\;.}
\]
\end{itemize}
\vspace{0.2em}
{\footnotesize Domain of $\Psi$: shock history $\histE_t = (\varepsilon_t, \varepsilon_{t-1}, \ldots)$ -- \emphc{exogenously driven}.}
\end{frame}
...
\]
Lag-$\tau$ weight is $\alpha^{\tau-1}$ (with $A_{t-1}$ at lag $1$).  At $\tau = 25$, $\alpha = 1/3$, the weight $\alpha^{24} \approx 3.5\!\times\!10^{-12}$.
\end{itemize}
\end{frame}
...
\begin{enumerate}\itemsep1pt
    \item Parameterise $\widehat\Psi$ using a neural network $\mathcal{N}_\theta(\histE)$.
    \item Train the network, i.e.\ find $\underbrace{\theta}_{\substack{\text{netw.}\\ \text{param.}}}$ such that $\mathcal{N}_\theta(\histE) \approx \widehat\Psi$.
\end{enumerate}
...
\item<1-> \textbf{Initialisation:} set $\text{iter} = 1$.
    ...
    \item<1-> sample initial state-history pairs $\bigl\{(x^0_j,\,\histE^0_j)\bigr\}_{j=1}^N$.
    \end{itemize}
...
\item<2-> \textbf{Training loop:}
    ...
    \item<2-> \emphc{Simulation.}  Fixing $\theta^{\text{iter}}$, sample shocks and simulate the policies $T$ periods forward.
        \begin{itemize}
        \item<2-> start with $\bigl\{(x^{\text{iter}-1}_j,\,\histE^{\text{iter}-1}_j)\bigr\}_{j=1}^N$;
        \item<2-> collect terminal pairs $\bigl\{(x^{\text{iter}}_j,\,\histE^{\text{iter}}_j)\bigr\}_{j=1}^N$.
        \end{itemize}
    ...
    \item<3-> \emphc{Policy improvement.}  Evaluate the mean squared residual and its gradient.
        ...
        \item<4-> use $y_j$ and $M$-point quadrature to construct $\bigl\{(x'_{j,m},\,\histE'_{j,m})\bigr\}_{m=1}^M$;
...
\begin{frame}[shrink=10]{Why State-Space DL Is Fragile -- and How Sequence Space Helps}
\small
...
\vspace{0.4em}
\textbf{Sequence space breaks the loop.}  The network input is the truncated shock history $\histE_t$, drawn from the \emphc{exogenous} aggregate-shock law of motion.  That distribution is fixed by model primitives and \emphc{does not move} with the policy update.
...
\begin{frame}[shrink=12]{Brock--Mirman: What Exactly Changes?}
\small
\begin{columns}[T]
\column{0.48\textwidth}
\begin{block}{State-space warm-up (discrete-time DEQN warm-up)}
...
\begin{itemize}\itemsep3pt
    \item \textbf{Input:} $\histE_t^T = (\varepsilon_{t-T+1}, \ldots, \varepsilon_t)$ -- 25 floats.
...
\vspace{-2pt}\begin{enumerate}\itemsep0pt
    ...
    \item \emphc{\S 12--13 loss core:} \texttt{Policy\_sequence}, then \texttt{E\_sequence} (Euler residual; network called twice -- at $\histE_t$ and at every quadrature node $\histE_{t+1}$).
...
\begin{itemize}\itemsep2pt
    ...
    \item Sequence-space DEQN feeds a \emphc{truncated shock history} ($\mathcal{O}(T)$ floats; $\sim$25 in the one-shock warm-up).
...
Companion deck \texttt{lecture\_09\_heterogeneous\_agents\_youngs.pdf}; teaching notebook \texttt{06\_SequenceSpace\_KrusellSmith.ipynb} (TF2); JAX reference \texttt{KrusellSmith\_Tutorial\_CPU.ipynb}.
\end{block}
\end{frame}
