...
\[
    J(\bm{\theta}) = \frac{1}{n}\sum_{i=1}^{n}\big(y^{(i)} - f(\x^{(i)};\bm{\theta})\big)^{2}
\]
...
Open and work through: \textbf{lectures/lecture\_02\_B01\_intro\_deep\_learning/code/lecture\_02\_B01\_02\_GradientDescent\_and\_StochasticGradientDescent.ipynb}
\end{block}
...
\vspace{0.4cm}
{\small \textbf{Adam} (Kingma \& Ba, 2015) is the most widely used default optimizer.  Typical hyperparameters: $\beta_1=0.9$, $\beta_2=0.999$, $\varepsilon=10^{-8}$.  \textbf{AdamW} (Loshchilov \& Hutter, 2019) decouples the weight-decay term from the adaptive update; it is the default we use for PINN training in later chapters.}
\end{frame}
...
\vspace{0.05cm}
{\footnotesize\emphc{Hands-on:} \texttt{lectures/lecture\_02\_B01\_intro\_deep\_learning/code/lecture\_02\_B01\_03\_Double\_Descent.ipynb} --- reproduce the curve with random Fourier features.}
\end{frame}
...
\begin{frame}{Outlook (not covered): Reinforcement Learning in Finance}
\begin{columns}[T]
\column{0.55\textwidth}
\textbf{The RL Loop:}
\begin{itemize}
...
\]
{\scriptsize Standard Jacobian convention; the transpose lives on the chain-rule product (matches script Eq.~\eqref*{eq:bptt} in \S1.10). For details see script \S1.10 ``Vanishing and Exploding Gradients''.}\\[2pt]
...
\vspace{0.3cm}
{\footnotesize\emphc{This chapter intentionally exercises both stacks.} Notebooks 04, 05, 07 use \emphc{TensorFlow/Keras}; notebooks 06, 08, 09 use \emphc{PyTorch}. The chapter's later DEQN/PINN materials (Chs.~2--7) are TensorFlow-based; the heterogeneous-agent and sequence-space chapters add JAX (Chs.~6 and~8).}
\end{frame}
