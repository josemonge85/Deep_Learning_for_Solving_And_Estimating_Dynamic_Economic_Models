...
\textbf{Session 1: April 20--23 (Discrete Time)}
\begin{itemize}
    \item Day 1~~Intro to ML \& Deep Learning
    \item Day 2~~Deep Equilibrium Nets (DEQNs)
    \item Day 3~~IRBC, NAS \& Loss Normalization
    \item Day 4~~OLG, Young's Method \& Sequence Space
\end{itemize}

\column{0.48\textwidth}
\textbf{Session 2: May 4--7 (Agentic AI, CT \& Apps)}
\begin{itemize}
    \item Day 5~~Agentic Programming (AI for Research)
    \item Day 6~~PINNs \& CT Heterogeneous Agents (HJB+KFE)
    \item Day 7~~Surrogates, GPs \& Deep Kernels
    \item Day 8~~Climate Economics, IAMs \& Deep UQ
\end{itemize}
\end{columns}
\normalsize
...
Open and work through: \textbf{lectures/day1/code/02\_GradientDescent\_and\_StochasticGradientDescent.ipynb}
\end{block}
...
\vspace{0.05cm}
{\footnotesize\emphc{Hands-on:} \texttt{lectures/day1/code/03\_Double\_Descent.ipynb} --- reproduce the curve with random Fourier features.}
\end{frame}
...
\vspace{1pt}
{\scriptsize Day 1 notebooks: \texttt{08\_MLP\_LSTM\_Transformer\_Edgeworth\_Cycles} (the three-way comparison on the same task) and \texttt{09\_Transformer\_InContext\_AR1} (advanced / optional).}
\end{frame}
