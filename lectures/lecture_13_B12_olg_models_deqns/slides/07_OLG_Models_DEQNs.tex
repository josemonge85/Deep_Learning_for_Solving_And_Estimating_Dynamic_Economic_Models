...
% ---------- Title ----------
\title[Lecture 13 (B12)]{Lecture 13 (B12): OLG Models with DEQNs}
\subtitle{OLG Models with Deep Equilibrium Nets}
...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Companion notebooks: \texttt{07\_OLG\_Analytic\_DEQN.ipynb}, \texttt{08\_OLG\_Benchmark\_DEQN.ipynb}, and the student exercise notebook \texttt{09\_OLG\_Exercise.ipynb}.}\\[3pt]
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    \item Same 9-step recipe at research scale: two assets, two borrowing constraints, capital adjustment costs, persistent shocks, hump-shaped labour.
...
\vspace{-3pt}\begin{itemize}\itemsep2pt
    ...
    \item \emphc{Demographics:} at any $t$, many cohorts coexist, each with its own asset holdings and labour income.
...
\begin{enumerate}\itemsep2pt
    ...
    \item Define the \emphc{equilibrium}: choices + prices that satisfy optimisation + market clearing.
    ...
    \item Sample training states $D$, minimise $\Loss(\rho)$ by SGD.
\end{enumerate}
\end{block}
...
\small
Agents supply their labour \emphc{inelastically} for a market wage $w_t$.
...
\vspace{0.3em}
Each period, \emphc{after the shock has realised}, the firm buys capital and hires labour to maximise profits, taking prices as given.
...
\vspace{0.4em}
There are \emphc{competitive spot markets} for the consumption good, capital, and labour.
...
\begin{enumerate}\itemsep1pt
    \item Given $(r_t, w_t)_{t\ge 0}$, household choices maximise the lifetime utility (1) subject to the savings-to-capital identity (2), the borrowing limit (3), and the per-period budget (4):\\[1pt]
...
% =====================================================================
% Part III -- Benchmark (AGS 2022 leading example), notebook 08
% =====================================================================
...
\begin{itemize}\itemsep1pt
    ...
    \item \emphc{Hump-shaped labour endowment} $\ell^s$ (Kotlikoff-style; right panel).
...
\begin{axis}[
    ...
    label style={font=\scriptsize}, ticklabel style={font=\tiny},
    title style={font=\scriptsize, uzhblue},
    title={Stylised lifecycle labour (schematic)}
]
...
\begin{enumerate}\itemsep1pt
    \item Households maximise $(1')$ subject to $(2')$--$(4')$.
    \item Firm maximises profits, same closed-form FOCs as Part II.
    \item \emphc{All three markets clear} every period: capital, labour, \emphc{bonds}.
\end{enumerate}
\end{block}
...
\begin{itemize}\itemsep1pt
    ...
    \item Practise the loop \emphc{specification $\to$ scaffold $\to$ run $\to$ diagnose $\to$ refactor}, with the agent doing the typing and you driving the economics.
