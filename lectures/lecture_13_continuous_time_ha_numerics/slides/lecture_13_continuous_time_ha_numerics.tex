...
\vspace{0.1em}
{\scriptsize\emph{This slide shows the simpler implicit-policy form ($c=(V_a)^{-1/\gamma}$) for intuition; the production notebook \texttt{lecture\_13\_06} uses width $96$, depth $3$, and the explicit $(V,c)$ output of the next slide.}}
\end{columns}
\end{frame}
...
\]
with mean-reversion speed $\theta$ (denoted $\eta$ in the script), long-run mean $\mu$, volatility $\sigma$.
...
\vspace{0.1em}
{\scriptsize\emph{Schematic architecture; the production notebook \texttt{lecture\_13\_07} uses width $96$, depth $3$, with the explicit $(V,c)$ output.}}
\end{columns}
\end{frame}
...
\toprule
& \textbf{PE Discrete (NB\,06)} & \textbf{PE Diffusion (NB\,07)} & \textbf{GE Aiyagari (NB\,08)} \\
\midrule
...
\begin{enumerate}\itemsep0pt
  \item \textbf{NB\,06}: isolate the HJB with simplest income process $\to$
        learn upwind FD and PINN mechanics
  \item \textbf{NB\,07}: extend to 2D with diffusion $\to$
        mesh-free PINN advantage ($V_{zz}$ via auto-diff)
  \item \textbf{NB\,08}: an upwind FD solver finds the GE prices; a PINN then re-solves the \emphc{coupled HJB+KFE system} at those prices --- a consistency / representation check, not price discovery
\end{enumerate}
\end{frame}
...
\begin{itemize}\itemsep2pt
    ...
    \item \textbf{NB 08} \textit{(in-class)}: \texttt{08\_Aiyagari\_Continuous\_Time\_FD\_and\_PINN\_PyTorch.ipynb} --- Aiyagari HJB+KFE: an upwind FD solver computes the GE prices, then a PINN re-solves the coupled system at those prices
    \item \textbf{NB 09} \textit{(homework warm-up)}: \texttt{09\_PINN\_Exercise.ipynb} --- toy 1D ODE $u''(x)=-\sin x$ on $[0,\pi]$ with zero BCs (unique solution $\sin x$); a low-stakes PINN scaffold before tackling the HJB exercises in NB~08
\end{itemize}
...
\begin{enumerate}\itemsep1pt
    ...
    \item Focus on NB~08 (in-class): the FD GE loop, then the PINN re-solve at the FD prices and the PINN-vs-FD agreement diagnostics
\end{enumerate}
\end{frame}
