...
% ---------- Title ----------
\title[Lecture 30 (B29)]{Lecture 30 (B29): Synthesis and Method Choice}
...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Final lecture: synthesis, climate economics, deep UQ, and policy design}\\[4pt]
...
\begin{enumerate}
    ...
    \item \textbf{Climate-block lessons for policy modeling} (6 min)
...
% --- Slide 4: Course arc ---
\begin{frame}{Course Arc: From Foundations to Climate Policy}
\small
\begin{center}
\begin{tabular}{ll}
\toprule
...
\node[phasebox, fill=softblue!25]   (d1) at (0,0)    {Foundations\\DL basics};
\node[phasebox, fill=softblue!25]   (d2) at (2.3,0)  {Days 2--4\\DEQN solvers};
\node[phasebox, fill=gray!20]       (d5) at (4.6,0)  {Toolkits\\Agents};
\node[phasebox, fill=softgreen!25]  (d6) at (6.9,0)  {PINNs\\CT HA};
\node[phasebox, fill=softorange!25] (d7) at (9.2,0)  {Surrogates\\GPs};
...
\begin{frame}{The Climate Block in One Slide}
\textbf{The climate block in context.}  Pseudo-states let one trained DEQN answer policy questions for \emph{all} plausible parameter values simultaneously.  A GP surrogate on top enables fast computation of SCC, temperature paths, and welfare across the parameter space.  Bayesian active learning then identifies which parameters drive policy recommendations, for example, climate sensitivity matters more than the pure rate of time preference for carbon-tax design.
...
\small
\textbf{Climate-block readings}
\begin{itemize}
    \item \textit{CDICE — Restud (2024)}
    \item \textit{CDICE online appendix}
    \item \textit{Deep UQ with an Application to IAM (working paper)}
    \item \textit{Constrained Optimal Carbon Tax Rules — JPE: Macro (forthcoming)}
\end{itemize}

\vspace{0.2cm}
\textbf{Climate-block notebooks} (suggested order)
\begin{itemize}
