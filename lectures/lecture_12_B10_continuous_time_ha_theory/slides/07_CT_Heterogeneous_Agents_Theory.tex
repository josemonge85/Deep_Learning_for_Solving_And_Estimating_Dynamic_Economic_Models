...
\draw[dashed] (axis cs:1,0) -- (axis cs:1,0.35);
\node[font=\tiny] at (axis cs:1.5,0.35) {$\bar{x}=1$};
\end{axis}
\end{tikzpicture}
\end{center}
...
% Low income - concentrated near constraint
\addplot[softblue, very thick, domain=-1.9:6, samples=80]
    {0.35*exp(-(x+1)^2/0.8) + 0.08*exp(-(x-1)^2/2)};
\addlegendentry{$g^*(b, n_1)$}
% High income - shifted right
\addplot[softorange, very thick, domain=-1.9:6, samples=80]
    {0.15*exp(-(x-0.5)^2/2) + 0.1*exp(-(x-2.5)^2/3)};
\addlegendentry{$g^*(b, n_2)$}
\draw[harvardcrimson, very thick, ->] (axis cs:-1.98,0) -- (axis cs:-1.98,0.45);
\node[font=\scriptsize\bfseries, harvardcrimson, fill=white, inner sep=1pt, anchor=west]
    at (axis cs:-0.95,0.46) {mass at $\ubar{b}$};
\draw[harvardcrimson, thick, -Stealth] (axis cs:-0.98,0.44) -- (axis cs:-1.96,0.44);
...
\addlegendentry{$g^*(a, n_2)$}
\draw[harvardcrimson, very thick, ->] (axis cs:0.1,0) -- (axis cs:0.1,0.1);
\node[font=\scriptsize\bfseries, harvardcrimson, fill=white, inner sep=1pt, anchor=west]
    at (axis cs:1.0,0.105) {mass at $\ubar{a}$};
\draw[harvardcrimson, thick, -Stealth] (axis cs:0.9,0.102) -- (axis cs:0.12,0.1);
\end{axis}
\end{tikzpicture}
\end{center}
...
\vspace{0.3em}
\textbf{The road ahead:}
\begin{enumerate}\itemsep1pt
    \item \textbf{Deck 2, Part I}: Solve coupled HJB+KFE by \emphc{finite differences} (benchmark)
    \item \textbf{Deck 2, Part II}: Solve with \emphc{PINNs} (mesh-free, auto-diff)
    \item \textbf{Deck 2, Part III}: Solve the \emphc{master equation} with EMINNs (deep learning)
\end{enumerate}
...
\footnotesize
\textbf{After this theory deck, implement directly in:}
\begin{itemize}\itemsep2pt
...
\vspace{0.2em}
\textbf{Mapping from this deck to notebook sections:}
\begin{itemize}\itemsep2pt
    \item HJB + KFE PDE system (Huggett/Aiyagari) $\rightarrow$ FD block (Section 1)
    \item Stationary distribution and boundary conditions $\rightarrow$ FD diagnostics + boundary plots
    \item PINN loss (HJB residual, KFE residual, BC terms) $\rightarrow$ PINN block (Section 2)
    \item Equilibrium moments and market clearing $\rightarrow$ FD/PINN comparison metrics
\end{itemize}
\end{frame}
...
\column{0.45\textwidth}
\begin{center}
\textbf{Covered today (Deck 1):}\\[8pt]
{\small
\begin{tabular}{rl}
