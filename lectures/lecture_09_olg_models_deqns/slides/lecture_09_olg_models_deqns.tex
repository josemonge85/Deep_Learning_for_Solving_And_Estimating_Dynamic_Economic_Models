...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Companion notebooks \texttt{lecture\_09\_07}\,--\,\texttt{lecture\_09\_11} (in \texttt{lecture\_09\_olg\_models\_deqns/code/}): \texttt{\_07}/\texttt{\_08} the analytic 6-agent model, \texttt{\_09}/\texttt{\_10} the 56-cohort benchmark, \texttt{\_11} the student exercise.  Full filenames appear on the Roadmap slide and in Parts II--III.}\\[3pt]
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{lecture\_09\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb}; companion ablation \texttt{lecture\_09\_07\_OLG\_Analytic\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\vspace{-2pt}
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{lecture\_09\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}; companion ablation \texttt{lecture\_09\_09\_OLG\_Benchmark\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\end{frame}
...
{\scriptsize $A=6 \Rightarrow 5$ outputs.  Notebook uses an extended state of dim $16+4A=40$ (pre-computed aggregates + per-agent income blocks + the transition-probability row) for ease of learning -- pure feature engineering.}
\end{frame}
...
\scriptsize
\emphc{Open notebook \texttt{lecture\_09\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb}} (the persistent-simulation primary; the exogenous-sampling companion is \texttt{lecture\_09\_07\_OLG\_Analytic\_DEQN\_exogenous.ipynb}).  It implements the model, equilibrium conditions, loss, FREE map, architecture, training, and validation covered in Part II.
\vspace{-2pt}
...
\vspace{2pt}
The actual training run, DEQN-vs-analytic comparison, and Euler-error histogram are produced by \texttt{lecture\_09\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb} (\S 8).  Production target in AGS (2022): mean relative Euler error $\lesssim 10^{-3}$.
\end{block}
\end{frame}
...
Open \texttt{lecture\_09\_08\_OLG\_Analytic\_DEQN\_persistent.ipynb}.  Set \texttt{RUN\_MODE = "teaching"} ($\sim 5$ min) for the classroom run; \texttt{"smoke"} (the default) is a $\sim$30-s sanity check; \texttt{"production"} chases AGS Table~3 accuracy.  Both notebooks expose a \texttt{SAMPLING\_MODE = "simulation" / "exogenous"} switch (the persistent primary uses \texttt{"simulation"}, the companion \texttt{lecture\_09\_07\_OLG\_Analytic\_DEQN\_exogenous.ipynb} uses \texttt{"exogenous"}); the model and loss are identical -- only the training cloud differs.
\end{block}
\end{frame}
...
\vspace{0.3em}
{\footnotesize With $A = 56$: minimal state dim $=113$; the notebook input is $240$ after engineered aggregates and transition probabilities.  Output dim $=4\times55+1=\bm{221}$.  Bond holdings are recovered as $\hat b'^h=(\hat q^h-\hat k'^h)/\kappa$, and softplus enforces $\hat k'^h,\hat\lambda_b^h,\hat q^h,\hat\mu^h\ge0$.}
\end{frame}
...
\begin{block}{Sizes from \texttt{lecture\_09\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}}
\centering
\begin{tabular}{ll}
\toprule
...
\]
\vspace{-4pt}
Same MLP template as Part II, plus a bond-price head; bonds recovered as $\hat b'^h=(\hat q^h-\hat k'^h)/\kappa$.
\end{exampleblock}
\end{frame}
...
\vspace{-3pt}
For each aggregate state $z \in \{1,2,3,4\}$ we obtain a lifecycle consumption profile $c^s(z)$ over ages $s=25,\ldots,80$, evaluated on a long ergodic simulation from \texttt{lecture\_09\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}.
\end{block}
...
\vspace{-2pt}
\textbf{Hands-on: \texttt{lecture\_09\_10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}} (persistent-simulation primary; \texttt{lecture\_09\_09\_OLG\_Benchmark\_DEQN\_exogenous.ipynb} is the feedback-free ablation, same model and loss, different training cloud).  \S 1--4 match III.1--III.5; \S 5--7 = training and policy stability; \S 8 = plots and diagnostics of III.10--III.12.  Teaching runtime (128-wide, 2\,000 episodes): $\approx 30$ min on CPU.
\end{frame}
...
\vspace{0.4em}
\textbf{Self-study extension (notebooks 10--12, deck 08).}  Heterogeneous agents on a histogram (Young's method), continuum-of-agents DEQN, and Krusell--Smith with deep learning -- visit any time before the PINN material.
\end{frame}
