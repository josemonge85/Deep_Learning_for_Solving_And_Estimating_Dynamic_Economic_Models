...
\vspace{-0.4cm}
\begin{center}
{\footnotesize Companion notebooks: \texttt{07\_OLG\_Analytic\_DEQN\_exogenous.ipynb} / \texttt{08\_OLG\_Analytic\_DEQN\_persistent.ipynb} (analytic 6-agent), \texttt{09\_OLG\_Benchmark\_DEQN\_exogenous.ipynb} / \texttt{10\_OLG\_Benchmark\_DEQN\_persistent.ipynb} (benchmark 56-agent), and the student exercise notebook \texttt{11\_OLG\_Exercise.ipynb}.}\\[3pt]
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{08\_OLG\_Analytic\_DEQN\_persistent.ipynb}; companion ablation \texttt{07\_OLG\_Analytic\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\vspace{-2pt}
...
\vspace{-3pt}\begin{itemize}\itemsep0pt
    ...
    \item Primary notebook: \texttt{10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}; companion ablation \texttt{09\_OLG\_Benchmark\_DEQN\_exogenous.ipynb}.
\end{itemize}
\end{block}
\end{frame}
...
\scriptsize
\emphc{Open notebook \texttt{08\_OLG\_Analytic\_DEQN\_persistent.ipynb}} (the persistent-simulation primary; the exogenous-sampling companion is \texttt{07\_OLG\_Analytic\_DEQN\_exogenous.ipynb}).  It implements the model, equilibrium conditions, loss, FREE map, architecture, training, and validation covered in Part II.
\vspace{-2pt}
...
\vspace{2pt}
The actual training run, DEQN-vs-analytic comparison, and Euler-error histogram are produced by \texttt{08\_OLG\_Analytic\_DEQN\_persistent.ipynb} (\S 8).  Production target in AGS (2022): mean relative Euler error $\lesssim 10^{-3}$.
\end{block}
\end{frame}
...
Open \texttt{08\_OLG\_Analytic\_DEQN\_persistent.ipynb}.  Set \texttt{RUN\_MODE = "teaching"} ($\sim 5$ min) for the classroom run; \texttt{"smoke"} (the default) is a $\sim$30-s sanity check; \texttt{"production"} chases AGS Table~3 accuracy.  Both notebooks expose a \texttt{SAMPLING\_MODE = "simulation" / "exogenous"} switch (the persistent primary uses \texttt{"simulation"}, the companion \texttt{07\_OLG\_Analytic\_DEQN\_exogenous.ipynb} uses \texttt{"exogenous"}); the model and loss are identical -- only the training cloud differs.
\end{block}
\end{frame}
...
\begin{block}{Sizes from \texttt{10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}}
\centering
\begin{tabular}{ll}
\toprule
Notebook input   & $240$ (minimal state: $113$) \\
Hidden layers    & $2 \times 128$ (teaching) \\
...
For each aggregate state $z \in \{1,2,3,4\}$ we obtain a lifecycle consumption profile $c^s(z)$ over ages $s=25,\ldots,80$, evaluated on a long ergodic simulation from \texttt{10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}.
\end{block}
...
\vspace{-2pt}
\textbf{Hands-on: \texttt{10\_OLG\_Benchmark\_DEQN\_persistent.ipynb}} (the persistent-simulation primary; \texttt{09\_OLG\_Benchmark\_DEQN\_exogenous.ipynb} is the feedback-free ablation).  Both notebooks expose a \texttt{SAMPLING\_MODE = "simulation" / "exogenous"} switch -- the model and loss are identical, only the training cloud differs.  \S 1--4 match III.1--III.5; \S 5--7 = training and policy stability; \S 8 = plots and diagnostics of III.10--III.12.  Teaching runtime (128-wide, 2\,000 episodes): $\approx 30$ min on CPU.
\end{frame}
