...
\begin{itemize}
  ...
  \item Diagnose the failure mode of state-space deep learning when the distribution shifts and explain how sequence space addresses it
  \item Deploy shape-preserving (I-spline) parameterizations to enforce marginal-propensity-to-consume monotonicity by construction
\end{itemize}
\end{frame}
...
\emphc{Teaching notebook \texttt{11\_Continuum\_of\_Agents\_DEQN.ipynb} handles the boundary inside \texttt{get\_weights\_eta()} via the histogram-redistribution math, not via a direct \texttt{tf.minimum} clamp.}}
\end{block}
\end{frame}
...
\addlinespace
Classroom MMW-style (this course) & \texttt{lecture\_10\_B08/\allowbreak code/\allowbreak 12\_KrusellSmith\_\allowbreak DeepLearning.ipynb} \\
