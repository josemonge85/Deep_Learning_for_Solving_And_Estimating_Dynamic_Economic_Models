...
\subtitle{Foundations, boundary conditions, applications to ODEs and economic PDEs}
...
{\scriptsize Based on: Raissi, Perdikaris \& Karniadakis (2019), \emph{J.\ Comput.\ Physics} 378, 686--707.}\\[1pt]
{\scriptsize Sirignano \& Spiliopoulos (2018), Deep Galerkin Method for PDEs in high dimensions.}\\[1pt]
{\scriptsize Al-Aradi, Correia, Naez, Hoshisashi \& Schwab (2018), DGM PDE solver.}\\[2pt]
{\scriptsize Materials: course GitHub repository}
\endgroup
\end{center}
\end{frame}
}
...
\begin{itemize}
  \item Build a PINN loss function combining PDE residuals and boundary conditions
  \item Implement automatic differentiation to compute derivatives of neural-network outputs with respect to spatial and temporal variables
  \item Construct trial solutions with hard boundary-condition enforcement for economic problems (HJB, cake-eating)
  \item Compare discrete-time (DEQN) and continuous-time (PINN) formulations and recognize when each is appropriate
  \item Diagnose training instabilities in PINNs and apply remedies (warm-start with Adam, L-BFGS polishing, double precision)
  \item Solve economic and finance PDEs (HJB, Black--Scholes) to high accuracy using PINNs with appropriate boundary conditions
\end{itemize}
\end{frame}
...
\textbf{In an earlier lecture, we trained} a neural network $\mathcal{N}_\theta$ to satisfy
...
\vspace{0.2em}
{\footnotesize \emphc{Used in notebooks 04 and 05} of this lecture (and again in the later continuous-time notebooks): L-BFGS is the deterministic polish step.  Final residuals remain problem-dependent, so report PDE residuals and solution diagnostics rather than relying on the optimizer label.}
\end{frame}
...
\begin{center}
\includegraphics[width=\linewidth]{fig/ext/pinn_ode_warmup.png}\\
{\scriptsize Trained PINN matches analytical solution; pointwise error $\sim 10^{-3}$ (teaching/production budget; the checked-in smoke run is coarser).}
\end{center}
\end{columns}
\end{frame}
...
\begin{center}
\includegraphics[width=0.90\linewidth]{fig/ext/pinn_softvshard.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.96\linewidth]{fig/ext/pinn_poisson2d_hard.png}
\end{center}

\vspace{0.1em}
\begin{center}
{\small Hard-BC PINN, analytical solution, and absolute error.\\Relative $L^2$ error $\approx 2.06\times 10^{-4}$ (representative run; the exact value depends on seed, budget, and hardware). Boundary conditions satisfied \emphc{exactly}.}
\end{center}
...
\vspace{0.15em}
\textbf{Consumption} via FOC, $\hat c(a) = (\widetilde V_a)^{-1/\gamma}$ with $\widetilde V_a := \operatorname{softplus}(\hat V'(a)) + \varepsilon$ (positivity safeguard); the same $\widetilde V_a$ enters the residual below.
...
\qquad
\mathcal{R}(a) = \rho\hat V - \Bigl[\tfrac{\hat c^{1-\gamma}}{1-\gamma} + \widetilde V_a\,(ra - \hat c)\Bigr].
\]

\vspace{0.15em}
{\small \emphc{Pedagogical point:} a small MLP with hard BCs beats a large gated net with a soft-BC penalty --- the boundary term no longer dominates the objective.}

\vspace{0.15em}
{\scriptsize \emphc{Well-posedness:} $V^\star$ requires $\kappa = (\rho - (1-\gamma) r)/\gamma > 0$, i.e.\ $\rho > (1-\gamma)r$ (automatic for $\gamma > 1$ with $\rho, r > 0$; a real restriction for $\gamma < 1$); otherwise the value is unbounded.}
\end{frame}
...
{\footnotesize\textbf{Deep Galerkin Method} (Sirignano \& Spiliopoulos, 2018), a heavier architecture that pays off only in higher dimensions or with sharp PDE features.  For the 1D cake problem, the trial-solution MLP on the previous slide is preferred.}
...
\vspace{0.1em}
\begin{center}
{\footnotesize \emph{Validation target:} closed form. Teaching/production run (Adam + FP64 L-BFGS): final HJB loss $\approx 3\!\times\!10^{-4}$, consumption rel.\ $L^2$ error $\approx 6\!\times\!10^{-3}$; the checked-in smoke notebook prints larger values.}
\end{center}
...
\vspace{0.2em}
\textbf{Network:} 3 hidden layers, 64 units, $\tanh$ activations; Adam warm start + FP64 L-BFGS polish (epoch/iteration budgets are \texttt{RUN\_MODE}-dependent).

\vspace{0.3em}
\emph{Schematic} of the call-option price curve. A longer teaching/production run reports max absolute error $\approx 0.13$ and mean $\approx 0.04$ against the closed form; the checked-in smoke notebook prints larger errors.
}
\end{columns}
