...
\vspace{0.3em}
\textbf{Consumption} via FOC, $\hat c(a) = (\widetilde V_a)^{-1/\gamma}$ with $\widetilde V_a := \operatorname{softplus}(\hat V'(a)) + \varepsilon$ (positivity safeguard).  The same $\widetilde V_a$ enters the HJB residual below; the printed $\hat V'$ is schematic.
...
\vspace{0.2em}
{\scriptsize \emphc{Well-posedness note:} the closed-form solution $V^\star(a) = \kappa^{-\gamma}\,a^{1-\gamma}/(1-\gamma)$ requires $\kappa = (\rho - (1-\gamma) r)/\gamma > 0$, i.e.\ $\rho > (1-\gamma) r$ (for $\gamma < 1$ a non-trivial parameter restriction; for $\gamma > 1$ automatic when $\rho, r > 0$). Absent this transversality, the value is unbounded.}
\end{frame}
