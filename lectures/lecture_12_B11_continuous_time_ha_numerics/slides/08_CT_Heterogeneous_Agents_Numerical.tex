% ============================================================================
% Lecture 21 (B20): Continuous-Time Heterogeneous Agents --- Numerics
...
% ---------- Title ----------
\title[Lecture 12 (B11)]{Lecture 12 (B11): Continuous-time heterogeneous agents, numerics}
...
\begin{itemize}\itemsep0pt
    ...
    \item Recall the PINN warm-up notebook
\end{itemize}
\end{block}
\end{columns}
...
(e.g., Young's method (Lecture 15) used $\sim 10{,}000$ agents).\\
...
\node[box] (d4) at (0,2.6) {Young's method (discrete)};
\node[box] (d5) at (0,1.7) {PINNs (single PDE)};
\node[current] (d6a) at (0,0.8) {PINNs (HJB+KFE)};
...
\centering\small
\vspace{0.1em}
\emphc{Mesh-free} solution of HJB + KFE via auto-diff. Same mesh-free philosophy.
\vspace{0.1em}
\end{block}
...
\vspace{0.1em}
\begin{center}
\textbf{Next block:} Surrogate models \& Gaussian Processes.
\end{center}
\end{frame}
...
\footnotesize
\textbf{Companion notebooks (self-contained, PyTorch):}
\begin{itemize}\itemsep2pt
