...
\small
Under standard regularity conditions (smoothness of $m$, stationarity, identification), as $T,S\to\infty$ with $T/S \to \tau \in [0,\infty)$,\footnote{\scriptsize Notation bridge to the script: the script parameterises $S$ independent simulated panels each of length $\tau T$ (so its $\tau$ is the relative panel-length factor), giving an asymptotic factor $(1+1/(\tau S))\Sigma_m$. Both conventions yield the same consistent estimator; the symbol $\tau$ plays inverse roles.}
\[
...
\begin{itemize}
  ...
  \item In practice, $\beta$ is calibrated to a target interest rate ($1/\beta \approx 1+r$), \emph{not} estimated from data.
  ...
  \item This is the textbook example of a \emphc{just-identified} parameter, which is exactly what we want for teaching the SMM workflow.
\end{itemize}
\vspace{0.2cm}
...
\begin{enumerate}
  ...
  \item Euler residual loss with Gauss--Hermite quadrature; $\varrho$ enters the \emph{transition} as well as the policy.
  {\small\[
...
\begin{block}{Goal}
Generate synthetic ``ex-post'' data at $\varrho_{\text{true}} = 0.90$ and compute three identifying moments.
\end{block}
\begin{center}\small
\begin{tabular}{cl}
\toprule
...
% --- New frame: Stacking a GP over the moment map ---
\begin{frame}{Stacking a GP over the Moment Map}
...
\vspace{-0.15cm}
\small
At fixed CRN, each candidate moment is monotone (or near-monotone) in $\varrho$ \emph{except} the mean savings rate (rightmost panel), which is essentially flat -- correctly diagnosed as uninformative for $\varrho$ identification.
\end{frame}
...
\begin{enumerate}
  ...
  \item Refine with \texttt{scipy.optimize.minimize} (bounded Nelder--Mead from the grid argmin).
...
\[
a(\theta) \;=\; \bigl[\,0.25 + \widetilde{\mathrm{UCB}}_{g}(\theta)\,\bigr] \cdot \Bigl\|\boldsymbol\sigma_m(\theta) / \widehat{\mathrm{sd}}(m)\Bigr\|_2,\quad
g(\theta)=-\log_{10}(Q(\theta)+\epsilon).
\]

\smallskip
\begin{itemize}\itemsep0pt
  \item First factor: \emphc{exploit} -- favor $\theta$ with small SMM criterion
  \item Second factor: \emphc{explore} -- favor $\theta$ where the moment GP is uncertain
  \item Constant $0.25$: keep exploration alive even where scaled UCB$=0$
\end{itemize}

\smallskip
\textbf{Three-way comparison.} Pilot grid / naive LHS / BoTorch BAL at the same budget: compare LOO error, criterion error, and recovered estimate.

\medskip
{\scriptsize Implementation: BoTorch \texttt{SingleTaskGP} + \texttt{UpperConfidenceBound}.}
\end{frame}
...
\vspace{0.10cm}\\
\textbf{Production pipeline:} Chen, Didisheim \& Scheidegger (2026, JFE), exactly this surrogate-and-SMM logic at scale, applied to finance.
\end{frame}
