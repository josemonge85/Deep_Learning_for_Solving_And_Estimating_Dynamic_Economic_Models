...
\small
\textit{Notation bridge to the script: this deck uses $G=\partial m/\partial\theta^\top$, $\Sigma_g$, and $\tau=T/S$; the script uses $M$, $\Omega$, and $\tau_{\mathrm{script}}$ = relative simulation panel length. The two conventions are related by $\tau = 1/(S\tau_{\mathrm{script}})$ so $(1+\tau)\Sigma_g = (1+1/(\tau_{\mathrm{script}}S))\Sigma_m$ -- both give the same consistent estimator.}

\smallskip
Under standard regularity conditions (smoothness of $m$, stationarity, identification), as $T,S\to\infty$ with $T/S \to \tau \in [0,\infty)$,
\[
