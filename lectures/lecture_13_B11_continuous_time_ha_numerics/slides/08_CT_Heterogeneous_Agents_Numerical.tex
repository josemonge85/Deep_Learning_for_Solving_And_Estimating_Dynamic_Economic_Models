...
\textbf{From Deck 1}: We derived the \emphc{theory} of CT heterogeneous agent models:
...
% --- Slide: FD Results ---
\begin{frame}[shrink=2]{Finite Differences: What They Produce}
\small
\begin{columns}[T]
...
\vspace{0.2em}
{\small \emphc{This is our ground truth} for validating deep learning methods.}
...
% --- Slide: FD primer ---
\begin{frame}{From Continuous HJB to a Linear System (Primer)}
\small
On a uniform grid $a_i$ with spacing $\Delta a$, replace the partial derivative $V_a$ by a finite-difference quotient:
\[
\underbrace{V_a^F(a_i) = \tfrac{V_{i+1}-V_i}{\Delta a}}_{\text{forward}}
\qquad
\underbrace{V_a^B(a_i) = \tfrac{V_i-V_{i-1}}{\Delta a}}_{\text{backward}}.
\]
\textbf{Upwind rule.} Use $V_a^F$ where the optimal drift $s_j(a_i) > 0$, $V_a^B$ where $s_j(a_i) < 0$. This is the standard stability fix for first-order PDEs (Courant-Isaacson-Rees / Barles-Souganidis).

\vspace{0.3em}
\textbf{Assemble the operator.} The choice of forward/backward at every grid point gives a sparse tridiagonal matrix $A_a$ acting on the vector $V \in \mathbb{R}^{I\times J}$. Coupling the discrete income states adds the block $\Lambda \otimes I_J$, so the full generator is
\[
A \;=\; A_a(\text{upwind}) + \Lambda \otimes I_J \;\in\; \mathbb{R}^{(IJ)\times(IJ)}.
\]
\textbf{Implicit step.} Rearrange the discretized HJB into $(\tfrac{1}{\Delta}+\rho)V^{n+1} - A V^{n+1} = u(c) + V^n/\Delta$ -- one sparse linear solve per outer iteration.

\vspace{0.3em}
{\scriptsize Goal of next slide: combine these three ingredients into the algorithm. The pseudocode mirrors Achdou et al.\ (2022, REStud), Appendix~B.}
\end{frame}
...
\begin{tabular}{@{}r@{\;}l@{}}
\textcolor{darkred}{\textbf{Left} ($\ubar a$):} & state-constraint $V_a \geq u'(\text{inc}\!\mid\!\ubar a)$ \\
\textcolor{darkred}{\textbf{Right} ($\bar a$):} & truncation Neumann $V_a = u'(\text{inc}\!\mid\!\bar a)$ \\
\textcolor{softblue}{\textbf{Top/bottom}} ($z$): & Reflecting $V_z = 0$ \\
\end{tabular}
\par}

\vspace{0.2em}
{\scriptsize At $\ubar a$ the inequality is the borrowing-constraint complementarity (binds when consumption hits $r\ubar a + w z$); at $\bar a$ we impose Neumann as a truncation choice on the artificial upper boundary.}
...
$\mathcal{L}_{\text{BC}}$  & Neumann on $a$-edges (FOC); reflecting $V_z=0$ on $z$ \\
...
No tensor-product blow-up --- sample cost grows \emphc{polynomially}, not exponentially, in $d$.}}
\end{columns}
\end{frame}
...
\begin{frame}[shrink=2]{Training Algorithm: Jointly Solving HJB + KFE}
\begin{columns}[T]
\column{0.52\textwidth}
\begin{block}{PINN Training (Stationary Equilibrium)}
\small
\begin{algorithmic}[1]
\STATE Initialize $V_\theta$, $g_\phi$, guess $r^{(0)}$
\STATE Optional functional-form initialization for $V_\theta$ and $g_\phi$; FD is reserved for validation
\FOR{$k = 1, \ldots, N_{\text{epochs}}$}
    ...
    \STATE Compute integrated KFE on a fixed asset grid
    \STATE Compute $\mathcal{L}_{\text{SC}}$, $\mathcal{L}_{\text{flux}}$, $\mathcal{L}_{\text{mass}}$, $\mathcal{L}_{\text{MC}}$
    \STATE Total loss: $\mathcal{L} = \sum \kappa_i \mathcal{L}_i$
    \STATE Update $\theta, \phi$ via Adam
    \IF{$k \mod N_r = 0$}
        \STATE Update $r$: $r \leftarrow r + \eta(K^s - K^d)$
	    \ENDIF
\ENDFOR
\STATE Freeze collocation grid and run FP64 L-BFGS polish
\end{algorithmic}
\end{block}
...
\begin{itemize}\itemsep1pt
    ...
    \item \textbf{Price update}: update $r$ slowly (every $N_r$ steps) to allow NN to adapt
    \item \textbf{Loss weights}: start with $\kappa_{\text{HJB}}=\kappa_{\text{KFE}}=1$, larger $\kappa_{\text{SC}},\kappa_{\text{flux}},\kappa_{\text{MC}}$
    \item \textbf{Optimizer}: Adam with lr $\sim 10^{-3}$, then deterministic FP64 L-BFGS
    \item \textbf{Validation}: compare against FD only after training
\end{itemize}

\vspace{0.2em}
\begin{center}
\colorbox{uzhgreylight}{\parbox{0.9\columnwidth}{\centering\small
Same philosophy as Day~5:\\
\emphc{economic equations = loss function}\\
...
\begin{center}
\begin{tikzpicture}[
    layer/.style={rectangle, rounded corners=3pt, draw=uzhblue, thick, minimum width=4cm, minimum height=0.45cm, align=center, font=\small},
    arrow/.style={-Stealth, thick, uzhblue}
]
\node[layer, fill=softblue!15] (input) at (0,4.2) {Input: $\omega = (a, n, z, \hat{\varphi}) \in \R^{d}$};
\node[layer, fill=uzhgreylight] (h1) at (0,3.5) {Hidden 1: $\bm{h}^{(1)} = \phi^{(1)}(W^{(1)}\omega + b^{(1)})$, \; 64, $\tanh$};
\node[layer, fill=uzhgreylight] (h2) at (0,2.8) {Hidden 2: $\bm{h}^{(2)} = \phi^{(2)}(W^{(2)}\bm{h}^{(1)} + b^{(2)})$, \; 64, $\tanh$};
\node at (0,2.3) {$\vdots$};
\node[layer, fill=uzhgreylight] (h5) at (0,1.8) {Hidden 5: $\bm{h}^{(5)} = \phi^{(5)}(W^{(5)}\bm{h}^{(4)} + b^{(5)})$, \; 64, $\tanh$};
\node[layer, fill=softorange!15] (output) at (0,1.1) {Output: $\hat{W} = \sigma(W^{(6)}\bm{h}^{(5)} + b^{(6)})$, \; softplus};

\draw[arrow] (input) -- (h1);
\draw[arrow] (h1) -- (h2);
\draw[arrow] (h5) -- (output);
\end{tikzpicture}
\end{center}
...
\footnotesize
\textbf{Companion notebooks for this lecture (self-contained, PyTorch):}
\begin{itemize}\itemsep2pt
