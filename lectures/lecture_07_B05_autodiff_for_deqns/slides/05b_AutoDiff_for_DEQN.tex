...
% University of Geneva -- Day 4 morning primer
...
\vspace{-0.4cm}
\begin{center}
{\footnotesize A Day~4 morning primer, to be used alongside the Day~4 autodiff notebooks (\texttt{02\_Brock\_Mirman\_AutoDiff\_DEQN}, \texttt{03\_Brock\_Mirman\_Uncertainty\_AutoDiff\_DEQN})\\
...
\footnotesize Notebook~01 (Brock--Mirman, Day~2; the autodiff version is Day~4 nb~02) needed \emph{two pages} of algebra before we could code the loss:
\begin{center}
...
\item \textbf{OLG} (Day~4 main lecture). A separate $\Pi$ per age cohort; gradients stack.
\item \textbf{Irreversibility constraints} (Day~3 IRBC). Fischer--Burmeister residuals sit alongside; the core Euler part still autodiffs.
\end{itemize}
\vspace{6pt}
...
\footnotesize
\begin{block}{Run the notebooks (Day 4)}
\begin{itemize}\itemsep1pt
...
\item \texttt{04\_IRBC\_AutoDiff\_DEQN} (\emph{further example, self-study}): IRBC under the autodiff template -- multi-country planner Lagrangian, machine-precision cross-check vs Day~3 nb 01.
\end{itemize}
