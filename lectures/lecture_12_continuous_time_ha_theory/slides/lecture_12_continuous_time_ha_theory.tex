...
\subtitle{HJB, Kolmogorov forward, Aiyagari in continuous time}
...
\node[done] (d1) {Discrete-time\\block};
\node[done, right=of d1] (d2) {DEQNs};
\node[done, right=of d2] (d3) {IRBC, NAS};
\node[done, right=of d3] (d4) {OLG, Young's};
\node[done, right=of d4] (d5) {Agents};
\node[current, right=of d5] (d6) {\textbf{This lecture}\\CT HA: HJB + KFE};
\node[future, right=of d6] (d7) {Surrogates};
\node[future, right=of d7] (d8) {Climate};
...
\small
The DEQN/Young's method block used \emph{discrete time}.\\
...
\textbf{Companion deck (numerical methods)}: finite differences, PINNs, and deep learning (EMINNs) for solving these coupled PDEs.}}
\end{center}
\end{frame}
...
\begin{itemize}
  \item Derive the Hamilton--Jacobi--Bellman equation from stochastic optimal-control problems with idiosyncratic risk
  \item Construct and solve the Kolmogorov forward (Fokker--Planck) equation for wealth-distribution dynamics given optimal policies
  \item Recognize how continuous-time stochastic calculus (Brownian motion, Ito's lemma, Poisson jumps) replaces discrete expectation operators
  \item Assemble coupled HJB + KFE + market-clearing systems to define competitive equilibrium in heterogeneous-agent economies (Huggett, Aiyagari, Krusell--Smith)
  \item Reformulate a coupled PDE system as a single master equation and interpret its solution structure
\end{itemize}
\end{frame}
...
\vspace{0.2em}
{\small This is exactly what PINNs solve (Chapter~7).}
\end{columns}
\end{frame}
...
\small
\textbf{Setting}: $dX_t = \eta(\bar{x} - X_t)\ud t + \sigma\ud B_t$ \quad (mean-reverting).

\vspace{0.2em}
\textbf{KFE}: $\partial_t g = \eta\,\partial_x\!\left[(x - \bar{x})\,g\right] + \frac{\sigma^2}{2}\partial_{xx}g$

\vspace{0.2em}
\textbf{Stationary distribution} ($\partial_t g = 0$):
\[
\boxed{g^*(x) = \frac{1}{\sqrt{2\pi\sigma^2/(2\eta)}}\exp\!\left(-\frac{(x-\bar{x})^2}{2\cdot\sigma^2/(2\eta)}\right) = \mathcal{N}\!\left(\bar{x},\;\frac{\sigma^2}{2\eta}\right)}
\]
...
\begin{itemize}\itemsep2pt
    \item Mean reversion ($\eta$) concentrates mass around $\bar{x}$
    \item Diffusion ($\sigma$) spreads mass
    \item Balance $\Rightarrow$ Gaussian steady state
    \item Larger $\eta$ $\Rightarrow$ tighter distribution
\end{itemize}
...
% --- Slide: Boundary Atom Decomposition ---
\begin{frame}{Boundary Atoms: Decomposition of $g$}
\small
When the borrowing constraint binds on a positive mass of agents, the stationary measure is \emphc{not absolutely continuous} with respect to Lebesgue measure: it carries a Dirac atom at $a=\ubar a$.
\[
   \boxed{\,g(a,n) \;=\; \underbrace{g_{\mathrm{ac}}(a,n)}_{\text{interior density}} \;+\; \underbrace{\alpha(n)\,\delta(a-\ubar a)}_{\text{constrained mass}}\,}
\]
\begin{itemize}\itemsep2pt
    \item $g_{\mathrm{ac}}(a,n)$ is the absolutely-continuous interior density.
    \item $\alpha(n)\geq 0$ is the constrained mass for income state $n$.
    \item The KFE introduced above governs only the interior part $g_{\mathrm{ac}}$; the atomic mass $\alpha(n)$ is determined by a \emph{separate flux-balance equation} that equates inflows from the no-flux boundary condition with the income-driven outflow back into the interior.
\end{itemize}

\vspace{0.2em}
{\footnotesize\textbf{Implementation.} Finite-difference solvers represent $\alpha(n)$ as the mass in the first grid cell.  PINN solvers either absorb the atom implicitly into a smooth density approximation (with corresponding accuracy loss near $\ubar a$) or explicitly parameterize $\alpha(n)$ alongside the interior network.}
\end{frame}
...
\end{itemize}
\vspace{0.1em}
{\scriptsize\emph{Schematic.}  The actual policy from NB~06 has a kink at the borrowing constraint where $c=wn+r\,\ubar a$ and $s(\ubar a, n)=0$ for the constrained type; see the ``Handling the Borrowing Constraint'' slide in the numerics deck.}
\end{frame}
...
\textbf{Companion (numerical) deck}: three approaches to approximate $g$ in finite dimensions:\\
...
\colorbox{uzhgreylight}{\parbox{0.6\textwidth}{\centering\small
\textbf{End of theory.}\\
The numerical methods deck picks up here.}}
\end{center}
\end{frame}

% --- Slide: Theory-to-Code Bridge ---
\begin{frame}{From Theory to Code (Companion Notebook)}
\footnotesize
\textbf{Companion notebook:}
\begin{itemize}\itemsep2pt
    \item \texttt{lecture\_13\_continuous\_time\_ha\_numerics/code/\allowbreak lecture\_13\_08\_Aiyagari\_Continuous\_Time\_FD\_and\_PINN\_PyTorch.ipynb}
\end{itemize}
