...
{\footnotesize Public draft: arXiv:2507.01704; companion code: \texttt{sischei/JPE\_Macro\_Using\_ML\_...}}\\[4pt]
...
\vspace{0.4em}
\begin{alertblock}{Bridge from Lecture 16}
...
\begin{frame}[shrink=12]{Climate Module: Cumulative Emissions $\rightarrow$ Temperature}
\small
\begin{columns}[T]
...
\]
where $\sigma_{\mathrm{CCR}}$ is the Transient Climate Response to cumulative carbon Emissions (TCRE), ranging from 1.0$^\circ$C to 2.3$^\circ$C per 1000 GtC.
...
\begin{alertblock}{Same lesson as Lecture 16 --- now in OLG}
...
\vspace{0.1cm}
\footnotesize
Emissions and temperature paths under no climate policy, compared with RCP scenarios.  Source: Kübler, Scheidegger \& Surbek (2026, forthcoming).
\end{frame}
...
\end{columns}
\vspace{0.05cm}
{\footnotesize Source: Kübler, Scheidegger \& Surbek (2026, forthcoming).}
\end{frame}
...
\vspace{0.15cm}
\footnotesize
Left: Pareto frontier in $(\vartheta_0, \vartheta_E)$-space. Right: welfare gains by cohort --- \emphc{all non-negative} under Pareto-improving policy.  Source: Kübler, Scheidegger \& Surbek (2026, forthcoming).
\end{frame}
...
the tax responds to carbon intensity and tipping proximity, providing tipping-risk insurance.  Source: Kübler, Scheidegger \& Surbek (2026, forthcoming).
\end{frame}

% --- Frame 37: Key economic insight ---
\begin{frame}{The Key Economic Insight}
\begin{block}{Main result}
Pareto-improving policies work through \emphc{disaster-risk reduction} (tail insurance), not maximum mean abatement \\ (empirical result from K\"ubler, Scheidegger \& Surbek 2026, forthcoming; not proven here).
\end{block}
...
\begin{frame}{Key Takeaways from Lecture 17}
\begin{enumerate}
...
\begin{frame}{Notebook Roadmap for Lecture 17}
\small
The slides here build on the deterministic CDICE-DEQN solver developed in Lecture 16.  Two notebooks are relevant:
\vspace{0.2cm}
\begin{itemize}
    \item \texttt{lectures/lecture\_17\_deep\_uq\_pareto\_improving\_policy/code/lecture\_17\_09\_DICE\_2P\_UQ\_Analysis.ipynb} -- this lecture's classroom notebook: the deep-UQ pipeline (per-year GP emulators over $(\rho,\pi_2)$, leave-one-out diagnostics, Saltelli Sobol indices, univariate effects, an SCC contour) on a 2-parameter DICE example, run on cached reference solutions.  CPU, $\sim$1 minute.
    \item \texttt{lectures/lecture\_16\_climate\_economics\_iams/code/lecture\_16\_03\_Stochastic\_DICE\_DEQN.ipynb} -- the upstream building block: an AR(1) productivity shock on the Lecture-16 deterministic CDICE-DEQN, Gauss--Hermite quadrature on the Euler expectation, Monte-Carlo SCC fan charts (parameter, stochastic, and epistemic uncertainty).
\end{itemize}
\vspace{0.2cm}
The full OLG-IAM with constrained Pareto-improving tax rules (K\"ubler, Scheidegger \& Surbek 2026, forthcoming, \textit{JPE: Macroeconomics}; arXiv:2507.01704) is implemented in production code that is not part of the classroom repository --- see the companion repository \texttt{sischei/JPE\_Macro\_Using\_ML\_to\_compute\_constrained\_optimal\_carbon\_tax\_rules}.
\end{frame}
...
\begin{itemize}
    \item K\"ubler, Scheidegger \& Surbek (2026, forthcoming, \textit{JPE: Macroeconomics}). \emph{Using Machine Learning to Compute Constrained Optimal Carbon Tax Rules.} arXiv:2507.01704.
...
\begin{itemize}
    \item \texttt{readings/links\_by\_lecture/lecture\_17.md}
\end{itemize}
