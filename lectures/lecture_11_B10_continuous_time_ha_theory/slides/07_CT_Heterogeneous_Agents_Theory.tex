% ============================================================================
% Lecture 20 (B19): Continuous-Time Heterogeneous Agents --- Theory
...
% ---------- Title ----------
\title[Lecture 11 (B10)]{Lecture 11 (B10): Continuous-time heterogeneous agents, theory}
...
\node[done] (d1) {DL Basics};
\node[done, right=of d1] (d2) {DEQNs};
\node[done, right=of d2] (d3) {IRBC, NAS};
\node[done, right=of d3] (d4) {OLG, Young's};
\node[done, right=of d4] (d5) {Agentic toolkit};
\node[current, right=of d5] (d6) {\textbf{Now}\\PINNs + CT HA};
...
\small
Earlier discrete-time lectures (DEQNs, Young's method) used \emph{discrete time}.\\
...
\vspace{0.2em}
{\small This is exactly what PINNs solve!}
\end{columns}
\end{frame}
...
\begin{center}
\colorbox{uzhgreylight}{\parbox{0.6\textwidth}{\centering\small
The next lecture (L21) covers the numerical methods for solving these models.}}
\end{center}
\end{frame}

% --- Slide: Theory-to-Code Bridge ---
\begin{frame}{From Theory to Code (numerics notebook)}
\footnotesize
\textbf{After this theory deck, implement directly in:}
\begin{itemize}\itemsep2pt
    \item \texttt{lecture\_21\_B20\_08\_Aiyagari\_Continuous\_Time\_FD\_and\_PINN\_PyTorch.ipynb}
\end{itemize}
