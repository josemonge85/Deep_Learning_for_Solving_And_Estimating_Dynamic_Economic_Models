...
% ---------- Title ----------
\title[Lecture 12 (B11)]{Lecture 12 (B11): Architecture Search and Loss Balancing}
\subtitle{Neural Architecture Search for Function Approximation}
...
% --- Slide 2: Roadmap ---
\begin{frame}{Roadmap of This Lecture (${\sim}20$ min)}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{I. Motivation}~~{\small(4 min)}
\begin{itemize}
...
\vspace{0.4em}
\textbf{II. Search Methods}~~{\small(8 min)}
\begin{itemize}
...
\column{0.48\textwidth}
\textbf{III. Implementing the Search}~~{\small(4 min)}
...
\vspace{0.4em}
\textbf{IV. Example \& Wrap-up}~~{\small(4 min)}
\begin{itemize}\itemsep0pt
...
\begin{itemize}
    \item Number of hidden layers $L \in \{1, \ldots, 10\}$
    \item Units per layer $n_\ell \in \{16, 32, \ldots, 512\}$
    \item Activation: ReLU, tanh, swish, \ldots
\end{itemize}

\vspace{0.3cm}
\textbf{Training hyperparameters:}
\begin{itemize}
    \item Learning rate $\eta \in [10^{-5}, 10^{-1}]$
...
\textbf{Hyperband} = SHA with multiple brackets (different $n/b$ trade-offs).  The usual default is $\eta=3$.
...
{\scriptsize
\begin{verbatim}
def random_search(n_trials=30, epochs=80, seed=0):
...
\begin{enumerate}
    ...
    \vspace{0.2cm}
    \item \textbf{DSGE policy functions:} Deep equilibrium nets (Lecture 02) use NNs to approximate policy/value functions. \emphc{Architecture search over the policy network} can improve convergence.
...
{\scriptsize
\begin{itemize}\itemsep0pt
    \item \emphc{Elsken, Metzen \& Hutter (2019)} --- \emph{NAS: A Survey}, JMLR 20(55); PDF in \texttt{day3/readings/}.
