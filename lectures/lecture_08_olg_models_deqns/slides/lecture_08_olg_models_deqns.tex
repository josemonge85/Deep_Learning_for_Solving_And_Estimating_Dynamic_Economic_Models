...
\vspace{0.3em}
{\footnotesize With $A = 56$: minimal state dim $=113$; the notebook input is $240$ after engineered aggregates and transition probabilities.  Output dim $=4\times55+1=\bm{221}$.  Bond holdings are recovered as $\hat b'^h=(\hat q^h-\hat k'^h)/\kappa$, and the output transforms enforce $\hat k'^h,\hat\lambda_b^h,\hat q^h,\hat\mu^h\ge0$.}
\end{frame}
...
Output layer     & softplus (multipliers, collateral slack) / bounded-exp (capital, bond price) \\
