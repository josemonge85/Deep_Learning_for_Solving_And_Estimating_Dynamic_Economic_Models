...
\[
u^j(c) \;=\; \begin{cases} \dfrac{c^{\,1 - 1/\gamma_j}}{1 - 1/\gamma_j}, & \gamma_j \neq 1, \\[4pt] \ln c, & \gamma_j = 1. \end{cases}
\]
...
\begin{itemize}
    ...
    \item $\gamma_{\min} = 0.25$, $\gamma_{\max} = 1.0$ (risk aversion $= 1/\gamma_j$ ranges from 1 to 4; the notebook uses the $\ln c$ branch when $\gamma_j = 1$)
\end{itemize}

\vspace{0.3em}
\begin{block}{Notation reminder}
{\footnotesize In this chapter $\gamma$ is the \textbf{IES}.  Later chapters on continuous-time HA and climate-economics re-purpose $\gamma$ for CRRA risk aversion (with $\psi$ then standing for the IES).  The convention is restated at the top of each chapter.}
\end{block}
...
\vspace{0.3em}
{\footnotesize $\sigma_e$ is the \emph{per-component} std., so the per-country marginal innovation std.\ is $\sigma_e\sqrt{2}$ and the cross-country innovation correlation is $1/2$ (hard-wired by the equal-weighted decomposition).}
...
\footnotesize
\[
\max_{\{c_t^j,\, k_{t+1}^j\}} \;\; \mathbb{E}_0\!\left[\sum_{t=0}^{\infty} \beta^{t} \sum_{j=1}^{N} \tau^j \, \frac{(c_t^j)^{1 - 1/\gamma_j}}{1 - 1/\gamma_j}\right]
\]
\textbf{subject to:}
\begin{align*}
...
\[
\mathcal{L} \;=\; \mathbb{E}_0\!\left[\,\sum_{t=0}^{\infty} \beta^{t}\!\left(\,
...
\begin{enumerate}
    ...
    \item Open notebook \texttt{lecture\_04\_B03\_01\_IRBC\_DEQN\_smooth.ipynb}, change $\delta$, and verify your prediction.
\end{enumerate}
\end{exampleblock}
\end{frame}
...
\begin{itemize}
    \item $\text{Euler}_j = \frac{\beta\,\E{\lambda' \text{MPK}' - (1-\delta)\mu'} + \mu}{\lambda(1+\Gamma_{k'})} - 1$ \quad (smooth: $\mu \equiv 0$)
...
\vspace{0.3em}
{\footnotesize \textbf{What the companion notebooks do.} Both 04a and 04b integrate the Euler expectation with the \emphc{Stroud-3 monomial rule} ($2(N+1)$ nodes; here $N=2$, so $6$ nodes), not the tensor-product Gauss--Hermite rule shown above. Gauss--Hermite is included here for pedagogical contrast and remains useful at very low $N$.}
\end{frame}
...
\textbf{Three notebooks accompany this lecture} (two in the L04 \texttt{code/} folder, one in the L05 folder):

\vspace{0.5em}
\begin{enumerate}
    \item \texttt{lecture\_04\_B03\_01\_IRBC\_DEQN\_smooth.ipynb} -- \emph{smooth benchmark, in-class walk-through (this lecture's \texttt{code/})}
    \begin{itemize}\itemsep1pt\vspace{2pt}
        \item Parameters \& steady state, monomial quadrature
    ...
    \vspace{0.3em}
    \item \texttt{lecture\_04\_B03\_02\_IRBC\_DEQN\_irreversible.ipynb} -- \emph{irreversible-investment extension (this lecture's \texttt{code/})}
    \begin{itemize}\itemsep1pt\vspace{2pt}
        \item KKT multipliers $\mu^j$ and feasibility-by-construction policy
        \item Fischer--Burmeister complementarity loss
        \item Same training loop and diagnostics as the smooth benchmark
    \end{itemize}

    \vspace{0.3em}
    \item \texttt{lecture\_05\_B04\_05\_IRBC\_Exercise.ipynb} -- \emph{hands-on exercise (lives in L05's \texttt{code/}, extends naturally into the NAS / loss-normalization material of Chapter~4)}
    \begin{itemize}\itemsep1pt\vspace{2pt}
...
\vspace{0.2em}
{\footnotesize Reconnects with the \emphc{consumption-correlation puzzle} flagged on slide~7 of this deck: complete markets predict $\rho(c^1,c^2)\!\approx\!1$, but the data give $\rho(c^1,c^2)<\rho(y^1,y^2)$.  Heterogeneous IES alone does \emph{not} resolve the puzzle; closing it with the data needs incomplete markets, frictions, or non-traded goods.}
