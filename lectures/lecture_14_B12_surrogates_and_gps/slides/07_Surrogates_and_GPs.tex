...
\centering
\includegraphics[height=0.65\textheight,keepaspectratio]{fig/gp_vfi_2d_active_benchmark.pdf}

\smallskip
{\scriptsize Separable benchmark $V_2(k_1,k_2) = V^*(k_1) + V^*(k_2)$ with $\delta = 1$ (full depreciation $\Rightarrow$ closed-form 1D Brock--Mirman, summed across sectors as a 2D ground truth). Workflow is identical for $\delta < 1$; we just lose the analytic check. Active design places points where $\sigma_{\text{GP}}$ is largest. (Notebook 04, cell 25.)}
\end{frame}
