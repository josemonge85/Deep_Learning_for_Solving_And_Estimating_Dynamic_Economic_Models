...
Training & \textcolor{softorange}{Posterior closed-form; hyperparams via marg.\ likelihood} & \textcolor{softorange}{SGD, many epochs} \\
...
\small
Workhorse testbed for computational macro (cf.\ the DEQN lectures). Model with $D$ sectors:
...
\textbf{Challenge:} approximate $V$ on $\R^D$ at each VFI step. Tensor-product grids become infeasible past $D \sim 20$. \emphc{GPs as interpolation} $\Rightarrow$ arbitrary geometries; with active-subspace reduction, scales to 500D in problems with low-dim active structure (Scheidegger \& Bilionis, 2019).
\end{columns}
...
\scriptsize
Standard GPs degrade past $D \gtrsim 10$ (kernel distances become uninformative without dimensionality reduction). \emphc{Active Subspaces} (AS) find a low-dim manifold of maximal response variation.
...
\toprule
 & \textbf{GP / ASGP} & \textbf{DEQNs (the discrete-time lectures)} \\
\midrule
...
\vspace{0.2em}
\begin{block}{Hands-on}
\small \textbf{Notebook:} \texttt{lecture\_14\_B12\_04\_GP\_Value\_Function\_Iteration.ipynb}.
\end{block}
\end{frame}
